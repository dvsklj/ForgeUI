...
function buildFullPrompt(): string {
  return [
    ...
    '',
    'forgeui_read_app_data — returns raw rows from permitted tables:',
    '  Input: { app_id, tables: ["tableName"], limit: 20, since: "2026-04-01" }',
    '  Output: { schema, data: { tableName: [...rows] }, rowCounts }',
    '',
    'forgeui_query_app_data — returns aggregated summaries (more token-efficient):',
    '  Input: { app_id, queries: [{ table, aggregate: "count|max|min|avg|trend", groupBy?, column?, where? }] }',
    '  Output: { results: [{ data: {...} }] }',
    '',
    'PREFER forgeui_query_app_data over forgeui_read_app_data.',
    'Summaries cost ~50-150 tokens vs. ~2,000+ for raw rows.',
    '',
    'THE DATA INTERACTION LOOP:',
    '1. Read — call forgeui_query_app_data to understand user data (trends, patterns, gaps)',
    '2. Reason — identify what should change in the app (adjust targets, add alerts, modify plans)',
    '3. Update — call forgeui_update_app with a manifest patch to modify the app structure',
    '',
