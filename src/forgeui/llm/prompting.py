...
def _system_prompt(request: GenerationRequest, policy: ManifestPolicy) -> str:
    ...
    return (
        ...
        "For fluid layouts, use grid columns auto with min_item_width sm/md/lg, or responsive "
        "counts {small: 1, medium: 2, large: 3} at renderer-owned container widths. "
        "Numeric columns (1-4) retain existing presets. Ratios main-start/main-end allow only "
        "one or two columns. Use grid-item children for bounded column_span/row_span (1-4); "
        "auto grids require column_span 1. Use gap_x/gap_y and padding tokens none/sm/md/lg, "
        "density inherit/compact/comfortable/spacious, and grid align or equal_height. "
        "Cards accept ordinary children or ordered card-header/card-body/card-footer slots; "
        "slot mode requires one body and permits one header/footer, without loose children. "
        "content-group attaches description/caption to exactly one child. disclosure requires "
        "title and permits summary and expanded; it works without actions or scripts. "
        "Remove empty decorative wrappers or provide meaningful content or empty-state. "
        "Layout props are static catalog choices, never expressions, CSS, or custom breakpoints. "
