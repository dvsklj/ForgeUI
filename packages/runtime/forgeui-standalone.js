...
`)}function ke(){return{type:"object",required:["manifest","id","elements"],properties:{manifest:{type:"string",const:"0.1.0"},id:{type:"string",pattern:"^[a-z0-9][a-z0-9-]*$"},root:{type:"string"},title:{type:"string"},schema:{type:"object",properties:{version:{type:"number"},tables:{type:"object",additionalProperties:{type:"object",properties:{columns:{type:"object",additionalProperties:{type:"object",properties:{type:{type:"string",enum:["string","number","boolean"]},default:{}},required:["type"]}}},required:["columns"]}}}},state:{type:"object",additionalProperties:{}},elements:{type:"object",additionalProperties:{type:"object",properties:{type:{type:"string",enum:ge},props:{type:"object"},children:{type:"array",items:{type:"string"}},visible:{type:"object",properties:{$when:{type:"object",properties:{path:{type:"string"},eq:{},neq:{},gt:{type:"number"},lt:{type:"number"},gte:{type:"number"},lte:{type:"number"},in:{type:"array"},notIn:{type:"array"},exists:{type:"boolean"}},required:["path"]}}}},required:["type"]}},actions:{type:"object",additionalProperties:{type:"object",properties:{type:{type:"string",enum:["mutateState","navigate","openDialog","closeDialog","callApi","toast","custom"]},path:{type:"string"},value:{},operation:{type:"string",enum:["set","append","update","delete","increment","decrement","toggle"]},set:{type:"object"},data:{type:"object"},key:{type:"string"},formId:{type:"string"},action:{type:"string"},target:{type:"string"},url:{type:"string"},method:{type:"string"},body:{type:"object"}},required:["type"]}},dataAccess:{type:"object",properties:{enabled:{type:"boolean",default:!1},readable:{type:"array",items:{type:"string"}},restricted:{type:"array",items:{type:"string"}},summaries:{type:"boolean",default:!1}}},persistState:{type:"boolean"},skipPersistState:{type:"boolean"}}}}var Sr={container:"Stack",row:"Stack",column:"Stack",text:"Text",heading:"Text",label:"Text",image:"Image",icon:"Icon",button:"Button",link:"Link",textInput:"TextInput",numberInput:"NumberInput",checkbox:"Checkbox",toggle:"Toggle",select:"Select",divider:"Divider",spacer:"Spacer",card:"Card",table:"Table",list:"List",alert:"Alert",progress:"Progress",badge:"Badge",avatar:"Avatar"};function _r(s){let t={},e=0;function o(n){let a=n.id||`a2ui-${e++}`,u=Sr[n.type];if(!u||!te(u))return t[a]={type:"Text",props:{content:`[Unsupported A2UI type: ${n.type}]`,variant:"caption",colorScheme:"secondary"}},a;let c=Ar(n.type,n.props||{}),d=(n.children||[]).map(p=>o(p));return t[a]={type:u,props:c,children:d.length>0?d:void 0},a}let r=(s.components||s.content||[]).map(n=>o(n)),l=r.length===1?r[0]:"a2ui-root";return r.length>1&&(t[l]={type:"Stack",props:{gap:"md"},children:r}),{manifest:"0.1.0",id:`a2ui-${Date.now()}`,root:l,elements:t,actions:{}}}function Ar(s,t){let e={...t};switch(s){case"heading":e.variant="heading",e.content=t.text||t.content||"",delete e.text;break;case"text":case"label":e.content=t.text||t.content||"",delete e.text;break;case"row":e.direction="row",e.gap=t.gap||t.spacing||"md";break;case"column":e.direction="column",e.gap=t.gap||t.spacing||"md";break;case"container":e.gap=t.gap||t.spacing||"md";break;case"image":e.src=t.src||t.url||"",e.alt=t.alt||"";break;case"button":e.label=t.text||t.label||"",e.variant=t.variant||t.style||"default";break;case"textInput":e.placeholder=t.placeholder||"",e.value=t.value||"",e.label=t.label||"";break;case"numberInput":e.placeholder=t.placeholder||"",e.value=t.value??0,e.label=t.label||"";break;case"select":t.options&&Array.isArray(t.options)&&(e.options=t.options.map(o=>typeof o=="string"?{value:o,label:o}:o));break;case"alert":e.variant=t.type||t.severity||"info",e.content=t.message||t.text||t.content||"",delete e.message,delete e.text;break;case"progress":e.value=t.value??t.percent??0;break}return e}function Dt(s){if(!s||typeof s!="object")return!1;let t=s;return!!(Array.isArray(t.components)||Array.isArray(t.content)||typeof t.type=="string"&&(t.type==="adaptive-card"||t.type.startsWith("a2ui"))||typeof t.version=="string"&&t.version.startsWith("a2ui")||!t.elements&&!t.manifest&&Array.isArray(t.content))}function Se(s){return Dt(s)?_r(s):s}function Lt(s){return`forgeui_${(s||"global").replace(/[^a-zA-Z0-9-]/g,"_")}`}var _e="f";function Pr(s){return new Promise((t,e)=>{let o=r=>e(r??new Error("IDB"));if(!globalThis.indexedDB)return o();let i=indexedDB.open(`${Lt(s)}_f`,1);i.onupgradeneeded=()=>{i.result.createObjectStore(_e)},i.onsuccess=()=>t(i.result),i.onerror=i.onblocked=()=>o(i.error)})}async function zt(s,t){if(s.length===0)return;let e=null;try{e=await Pr(t),await new Promise((o,i)=>{let r=e.transaction(_e,"readwrite"),l=r.objectStore(_e);for(let{file:n,id:a}of s)l.put(n,a);r.oncomplete=()=>o(),r.onerror=r.onabort=()=>i(r.error)})}catch{return}finally{e?.close()}}function Ot(s,t,e="none"){let o=Lt(t),i=e,r=null,l=!1,n=!1,a=null,u=null,c=null,d=[];function p(){let h=x();for(let y of d)try{y(h)}catch{}}function x(){return{mode:i,isPersisting:r!=null&&l,lastSaved:a,lastLoaded:u,error:c,dbName:i==="indexeddb"?o:null}}async function O(){if(r)return r;try{let{createIndexedDbPersister:h}=await import("tinybase/persisters/persister-indexed-db");return r=h(s,o),c=null,r}catch(h){throw c=`IndexedDB unavailable: ${h.message}`,p(),h}}async function Z(){if(i==="none")return;let h=await O();try{await h.load(),u=Date.now()}catch(y){c=`Load failed: ${y.message}`,p()}try{h.startAutoSave(),l=!0}catch(y){c=`Auto-save failed: ${y.message}`}try{h.startAutoLoad(1),n=!0}catch(y){c=`Auto-load failed: ${y.message}`}p()}async function $(){if(r){l&&(r.stopAutoSave(),l=!1),n&&(r.stopAutoLoad(),n=!1);try{await r.save(),a=Date.now()}catch(h){c=`Final save failed: ${h.message}`}p()}}async function L(){if(!(i==="none"||!r)){try{await r.save(),a=Date.now(),c=null}catch(h){c=`Save failed: ${h.message}`}p()}}async function R(){if(!(i==="none"||!r)){try{await r.load(),u=Date.now(),c=null}catch(h){c=`Load failed: ${h.message}`}p()}}async function P(){if(await $(),r){try{r.destroy()}catch{}r=null}d=[],p()}function E(h){if(h===i)return;let y=i;i=h,y==="indexeddb"&&h==="none"&&(r&&l&&(r.stopAutoSave(),l=!1),r&&n&&(r.stopAutoLoad(),n=!1)),h==="indexeddb"&&y==="none"&&Z().catch(()=>{}),p()}function k(h){return d.push(h),()=>{d=d.filter(y=>y!==h)}}return{start:Z,stop:$,save:L,load:R,setMode:E,getStatus:x,addListener:k,destroy:P}}var se=class{constructor(t=50){this._stack=[];this._position=-1;this._listeners=[];this._maxDepth=t}push(t){if(this._position<this._stack.length-1&&(this._stack=this._stack.slice(0,this._position+1)),this._position>=0){let e=this._stack[this._position];if(JSON.stringify(e)===JSON.stringify(t))return}this._stack.push(structuredClone(t)),this._stack.length>this._maxDepth?this._stack.shift():this._position++,this._notifyListeners()}undo(){return this._position<=0?null:(this._position--,this._notifyListeners(),structuredClone(this._stack[this._position]))}redo(){return this._position>=this._stack.length-1?null:(this._position++,this._notifyListeners(),structuredClone(this._stack[this._position]))}current(){return this._position<0?null:structuredClone(this._stack[this._position])}jumpTo(t){return t<0||t>=this._stack.length?null:(this._position=t,this._notifyListeners(),structuredClone(this._stack[this._position]))}getState(){return{canUndo:this._position>0,canRedo:this._position<this._stack.length-1,position:this._position,total:this._stack.length}}getTimeline(){return this._stack.map((t,e)=>({position:e,id:t.id}))}clear(){this._stack=[],this._position=-1,this._notifyListeners()}addListener(t){return this._listeners.push(t),()=>{this._listeners=this._listeners.filter(e=>e!==t)}}_notifyListeners(){let t=this.getState();for(let e of this._listeners)try{e(t)}catch{}}};function Ut(s){if(!G(s))return s;let t=typeof s.manifest=="string"?s.manifest:"";if(!(/^0\.0(?:\.|$)/.test(t)||!t&&("version"in s||"manifestVersion"in s||"components"in s||"rootElement"in s||Er(s.elements))))return s;let r={...s,manifest:"0.1.0"};if(delete r.version,delete r.manifestVersion,!r.elements&&G(r.components)&&(r.elements=r.components,delete r.components),!r.root&&typeof r.rootElement=="string"&&(r.root=r.rootElement,delete r.rootElement),G(r.elements)){let l={};for(let[n,a]of Object.entries(r.elements)){if(!G(a))continue;let u={...a};!u.type&&typeof u.component=="string"&&(u.type=u.component,delete u.component),l[n]=u}r.elements=l}return r}function Er(s){return G(s)?Object.values(s).some(t=>G(t)&&"component"in t):!1}function G(s){return s!==null&&typeof s=="object"&&!Array.isArray(s)}import{html as m,css as z,svg as _,nothing as w}from"lit";import{LitElement as Ir}from"lit";var ne=class ne extends Ir{constructor(){super(...arguments);this._instanceId=`forge-${++ne._instanceCounter}`;this.props={};this.store=null;this.onAction=null;this.itemContext=null}static get properties(){return{props:{type:Object}}}connectedCallback(){super.connectedCallback()}resolve(e){if(!this.store)return e;this.itemContext&&K(this.itemContext);try{return S(this.store,e)}finally{K(null)}}getProp(e){let o=this.props?.[e];return typeof o=="string"&&(o.startsWith("$state:")||o.startsWith("$computed:")||o.startsWith("$item:")||o.startsWith("$expr:")||o.includes("{{")&&o.includes("}}"))?this.resolve(o):o}getArray(e){let o=this.getProp(e);return Array.isArray(o)?o:o&&typeof o=="object"?Object.values(o):[]}getString(e,o=""){let i=this.getProp(e);return typeof i=="string"?i:String(i??o)}getNumber(e,o=0){let i=this.getProp(e);return typeof i=="number"?i:Number(i)||o}getBool(e,o=!1){let i=this.getProp(e);return typeof i=="boolean"?i:o}getBoundProp(e,o){let i=typeof this.props?.bind=="string"?this.props.bind:"";if(i){let l=this.resolve(i);if(l!==void 0)return l}let r=this.getProp(e);return r===void 0?o:r}dispatchAction(e,o){let i=typeof this.props?.bind=="string"?this.props.bind:"",r=i?{...o||{},bind:i}:o;this.onAction&&this.onAction(e,r),this.dispatchEvent(new CustomEvent("forgeui-action",{detail:{action:e,payload:r},bubbles:!0,composed:!0}))}handleAction(e){let o=this.getString("action");o&&this.dispatchAction(o,this.props)}prop(e){return this.getProp(e)}static get sharedStyles(){return[xe]}gapValue(e){let o={none:"0",0:"0","3xs":"var(--forgeui-space-3xs)","2xs":"var(--forgeui-space-2xs)",xs:"var(--forgeui-space-xs)",sm:"var(--forgeui-space-sm)",md:"var(--forgeui-space-md)",lg:"var(--forgeui-space-lg)",xl:"var(--forgeui-space-xl)","2xl":"var(--forgeui-space-2xl)"};if(e==null||e==="")return"var(--forgeui-space-md)";let i=String(e);return i in o?o[i]:/^\d+(\.\d+)?$/.test(i)?`${i}px`:/^\d+(\.\d+)?(px|rem|em|%|vw|vh|ch)$/.test(i)?i:"var(--forgeui-space-md)"}static get styles(){return[xe]}};ne._instanceCounter=0;var g=ne;import{html as Ie,css as Me,nothing as Mr}from"lit";var Ae=class extends g{static get styles(){return Me`
  ...
  `}render(){let t=this.getString("label","Button"),e=this.getString("variant","primary"),o=this.getString("size",""),i=this.getBool("disabled"),r=this.getProp("pressed");return Ie`<button class="${e} ${o}" ?disabled=${i}
      aria-pressed=${r==null?Mr:String(!!r)}
      @click=${l=>this.handleAction(l)}>${t}<slot></slot></button>`}};customElements.define("forgeui-button",Ae);var Pe=class extends g{static get styles(){return Me`
    :host { display:flex; gap:var(--forgeui-space-xs); }
  `}render(){return Ie`<slot></slot>`}};customElements.define("forgeui-button-group",Pe);var Ee=class extends g{static get styles(){return Me`
    :host { display:inline-flex; }
  ...
  `}render(){let t=this.getString("label",""),e=this.getString("href","#");return Ie`<a href="${e}">${t}<slot></slot></a>`}};customElements.define("forgeui-link",Ee);import{html as j,css as J,nothing as Nt}from"lit";var Te=class extends g{static get properties(){return{props:{type:Object}}}static get styles(){return J`
  ...
  `}render(){let t=this.getString("text","")||this.getString("label",""),e=this.getString("variant","");return j`<span class="badge" variant="${e}">${t}<slot></slot></span>`}};customElements.define("forgeui-badge",Re);var De=class extends g{static get styles(){return J`
    :host { display:inline-flex; }
  ...
  `}render(){let t=this.getString("src",""),e=this.getString("name","?"),o=e.split(" ").map(i=>i[0]).join("").toUpperCase().slice(0,2);return j`<div class="avatar">${t?j`<img src="${t}" alt="${e}">`:o}<slot></slot></div>`}};customElements.define("forgeui-avatar",De);var Le=class extends g{static get styles(){return J`
  ...
  `}render(){let t=this.getString("title","Nothing here"),e=this.getString("description","");return j`
      ...
      ${e?j`<div class="desc">${e}</div>`:Nt}
      <slot></slot>
    `}};customElements.define("forgeui-empty-state",Le);import{html as v,css as N,nothing as D}from"lit";var ze=class extends g{static get styles(){return N`
  ...
  `}render(){let t=this.getString("label",""),e=this.getString("placeholder",""),o=this.getString("hint",""),i=this.getString("error",""),r=this.getString("inputType","")||this.getString("type","text"),l=this.getBool("multiline"),n=String(this.getBoundProp("value","")??""),a=this._instanceId;return v`
    ...
    `}};customElements.define("forgeui-text-input",ze);var Oe=class extends g{static get styles(){return N`
    :host { display:block; flex:1 1 auto; min-width:0; max-width:100%; margin-bottom:var(--forgeui-space-sm); }
  ...
  `}render(){let t=this.getString("label",""),e=this.getProp("min"),o=this.getProp("max"),i=this.getProp("step"),r=this.getBoundProp("value"),l=this._instanceId;return v`
    ...
    `}};customElements.define("forgeui-number-input",Oe);var Ue=class extends g{static get styles(){return N`
    :host { display:block; flex:1 1 auto; min-width:0; max-width:100%; margin-bottom:var(--forgeui-space-sm); }
    label { display:block; font-size:var(--forgeui-text-sm); font-weight:var(--forgeui-weight-medium); margin-bottom:var(--forgeui-space-2xs); overflow-wrap:break-word; }
  ...
  `}render(){let t=this.getString("label",""),e=this.getProp("options")||[],o=String(this.getBoundProp("value","")??""),i=this._instanceId;return v`
    ...
    `}};customElements.define("forgeui-select",Ue);var Ne=class extends g{static get styles(){return N`
  ...
  `}render(){let t=this.getString("label",""),e=this.getProp("options")||[],o=this.getBoundProp("value",this.getProp("selected")??[]),i=Number(this.getProp("maxSelections")),r=Number.isFinite(i)&&i>=0?i:1/0,l=(Array.isArray(o)?o.map(c=>String(c)):[]).slice(0,r),n=this.getBool("disabled"),a=this._instanceId,u=c=>{let d=l.filter(p=>p!==c);this.dispatchAction("remove",{value:c}),this.dispatchAction("change",{value:d,selected:d})};return v`
    ...
    `}};customElements.define("forgeui-multi-select",Ne);var Fe=class extends g{static get styles(){return N`
  ...
  `}render(){let t=this.getString("label",""),e=!!this.getBoundProp("checked",this.getProp("value")??!1),o=this._instanceId;return v`
    ...
    `}};customElements.define("forgeui-checkbox",Fe);var Ve=class extends g{constructor(){super(...arguments);this._toggle=()=>{if(this.getBool("disabled"))return;let e=!!this.getBoundProp("on",this.getProp("value")??!1);this.dispatchAction("change",{value:!e,checked:!e})};this._onKeydown=e=>{(e.key==="Enter"||e.key===" "||e.key==="Spacebar")&&(e.preventDefault(),this._toggle())}}static get styles(){return N`
  ...
  `}render(){let e=!!this.getBoundProp("on",this.getProp("value")??!1),o=this.getString("label",""),i=this.getBool("disabled"),r=this._instanceId;return v`
    ...
    `}};customElements.define("forgeui-toggle",Ve);var Be=class extends g{static get styles(){return N`
    :host { display:block; flex:1 1 auto; min-width:0; max-width:100%; margin-bottom:var(--forgeui-space-sm); }
    label { display:block; font-size:var(--forgeui-text-sm); font-weight:var(--forgeui-weight-medium); margin-bottom:var(--forgeui-space-2xs); overflow-wrap:break-word; }
    input { width:100%; padding:var(--forgeui-space-xs) var(--forgeui-space-sm); border:1px solid var(--forgeui-color-border);
      border-radius:var(--forgeui-radius-md); font:inherit; height:var(--forgeui-input-height);
  ...
  `}render(){let t=this.getString("label",""),e=this.getString("value",""),o=this._instanceId;return v`
    ...
    `}};customElements.define("forgeui-date-picker",Be);var qe=class extends g{static get styles(){return N`
    :host { display:block; flex:1 1 auto; min-width:0; max-width:100%; margin-bottom:var(--forgeui-space-sm); }
    label { display:block; font-size:var(--forgeui-text-sm); font-weight:var(--forgeui-weight-medium); margin-bottom:var(--forgeui-space-2xs); overflow-wrap:break-word; }
  ...
  `}render(){let t=this.getString("label",""),e=this.getNumber("min",0),o=this.getNumber("max",100),i=this.getNumber("step",1),r=this.getBoundProp("value",e),l=Number(r);Number.isFinite(l)||(l=e);let n=this._instanceId;return v`
    ...
    `}};customElements.define("forgeui-slider",qe);var We=class extends g{constructor(){super(...arguments);this._dragging=!1;this._openFilePicker=()=>{this.shadowRoot?.querySelector('input[type="file"]')?.click()};this._onDropzoneKeydown=e=>{e.key!=="Enter"&&e.key!==" "||(e.preventDefault(),this._openFilePicker())};this._onFileChange=e=>{let o=Array.from(e.target.files??[]);this._processFiles(o)};this._onDragOver=e=>{e.preventDefault(),!this._dragging&&(this._dragging=!0,this.requestUpdate())};this._onDragLeave=e=>{e.currentTarget===e.target&&(this._dragging=!1,this.requestUpdate())};this._onDrop=e=>{e.preventDefault(),this._dragging=!1,this.requestUpdate(),this._processFiles(Array.from(e.dataTransfer?.files??[]))}}static get styles(){return N`
  ...
  `}_maxSizeBytes(){let e=this.getProp("maxSize");if(typeof e=="number"&&Number.isFinite(e)&&e>=0)return Math.floor(e);if(typeof e!="string")return null;let o=e.trim().match(/^(\d+(?:\.\d+)?)\s*(b|kb|mb|gb)?$/i);if(!o)return null;let i=Number(o[1]),r=(o[2]||"b").toLowerCase(),l=r==="gb"?1<<30:r==="mb"?1<<20:r==="kb"?1024:1,n=i*l;return Number.isFinite(n)?Math.floor(n):null}_newFileId(){return globalThis.crypto?.randomUUID?.()??`${Date.now()}_${Math.random().toString(36).slice(2)}`}_processFiles(e){let o=this.getBool("multiple"),i=this._maxSizeBytes(),r=(o?e:e.slice(0,1)).map(d=>{let p=this._newFileId(),x=i==null||d.size<=i,O={id:p,name:d.name,size:d.size,type:d.type,lastModified:d.lastModified,accepted:x,storageKey:x?p:null};return x||(O.error="maxSize"),[d,O]}),l=r.filter(([,d])=>d.accepted),n=r.map(([,d])=>d),a=l.map(([,d])=>d),u=o?a:a[0]??null,c=a[0]??null;this.dispatchAction("change",{id:c?.id??null,uuid:c?.id??null,name:c?.name??null,size:c?.size??null,type:c?.type??null,lastModified:c?.lastModified??null,storageKey:c?.storageKey??null,value:u,files:n,rejected:n.filter(d=>!d.accepted),multiple:o,maxSize:i}),zt(l.map(([d,p])=>({file:d,id:p.id})))}render(){let e=this.getString("label","Upload file"),o=this.getString("accept","*"),i=this.getBool("multiple");return v`
    ...
    `}};customElements.define("forgeui-file-upload",We);import{html as I,css as F,nothing as Ke}from"lit";var He=class extends g{static get properties(){return{props:{type:Object}}}static get styles(){return F`
  ...
  `}render(){let t=this.getProp("columns"),e;typeof t=="number"?e=String(t):typeof t=="string"&&t?e=t:e="1";let o=/^\d+$/.test(e)?`repeat(${e}, minmax(0, 1fr))`:e,i=this.getString("gap","md"),r=this.gapValue(i),l=this.getString("padding",""),n=l?this.gapValue(l):"0";return this.style.gridTemplateColumns=o,this.style.gap=r,this.style.padding=n,/^\d+$/.test(e)&&Number(e)>=2&&this.setAttribute("responsive",""),I`<slot></slot>`}};customElements.define("forgeui-grid",Ge);var Je=class extends g{static get properties(){return{props:{type:Object}}}static get styles(){return F`
  ...
  `}render(){let t=this.getString("variant",""),e=this.getString("title",""),o=this.getString("subtitle","");return t&&this.setAttribute("variant",t),I`
      ${e||o?I`
        <div class="header">
          ${e?I`<div class="title">${e}</div>`:Ke}
          ${o?I`<div class="subtitle">${o}</div>`:Ke}
        </div>
      `:Ke}
      <div class="body"><slot></slot></div>
    `}};customElements.define("forgeui-card",Je);var Ze=class extends g{static get properties(){return{props:{type:Object}}}static get styles(){return F`:host { display:block; margin-inline:auto; width:100%; box-sizing:border-box; }`}render(){let t=this.getString("maxWidth",""),e={sm:"640px",md:"768px",lg:"1024px",xl:"1280px","2xl":"1536px",full:"100%",none:"none","":""},o=t in e?e[t]:t,i=this.getString("padding","");return o&&o!=="none"?this.style.maxWidth=o:this.style.maxWidth="",this.style.padding=i?this.gapValue(i):"",I`<slot></slot>`}};customElements.define("forgeui-container",Ze);var Ye=class extends g{static get properties(){return{props:{type:Object}}}constructor(){super(),this._active=""}static get styles(){return F`
    :host { display:block; }
  ...
  `}_itemKey(t){return typeof t=="string"?t:String(t&&typeof t=="object"?t.id??t.key??t.value??t.label??"":t??"")}_itemLabel(t){return typeof t=="string"?t:String(t&&typeof t=="object"?t.label??t.title??t.value??"":t??"")}updated(){Array.from(this.children).filter(e=>!(e instanceof HTMLScriptElement)).forEach((e,o)=>{let i=(e.props||{}).slot??e.getAttribute("slot");String(o)===this._active||i===this._active?e.setAttribute("data-active",""):e.removeAttribute("data-active")})}_moveTo(t,e){let o=this._itemKey(e[t])||String(t);this._active=o,this.requestUpdate(),this.dispatchAction("tab-change",{active:o,value:o}),this.updateComplete.then(()=>{this.shadowRoot?.querySelector(`#${this._instanceId}-tab-${t}`)?.focus()})}render(){let t=this.getProp("items")||this.getProp("tabs")||[],e=Array.isArray(t)?t:[],o=this.getBoundProp("activeTab",this.getProp("value"));o!==void 0&&String(o)!==this._active&&(this._active=String(o)),!this._active&&e.length>0&&(this._active=this._itemKey(e[0])||"0");let i=e.findIndex((l,n)=>(this._itemKey(l)||String(n))===this._active),r=(l,n)=>{let a=-1;l.key==="ArrowRight"?a=(n+1)%e.length:l.key==="ArrowLeft"?a=(n-1+e.length)%e.length:l.key==="Home"?a=0:l.key==="End"&&(a=e.length-1),a!==-1&&(l.preventDefault(),this._moveTo(a,e))};return I`
    ...
    `}};customElements.define("forgeui-tabs",Ye);var Xe=class extends g{static get properties(){return{props:{type:Object}}}static get styles(){return F`
    :host { display:block; }
  ...
  `}render(){let t=this.getString("title","Section");return I`<details><summary>${t}</summary><div class="content"><slot></slot></div></details>`}};customElements.define("forgeui-accordion",Xe);var Qe=class extends g{static get styles(){return F`
    :host { display:block; }
    hr { border:none; border-top:1px solid var(--forgeui-color-border); margin:var(--forgeui-space-sm) 0; }
  `}render(){return I`<hr>`}};customElements.define("forgeui-divider",Qe);var et=class extends g{static get styles(){return F`:host { display:block; }`}render(){let t=this.getString("size","md"),e=this.getString("height",""),o=this.getString("width",""),i=e?this.gapValue(e):this.gapValue(t),r=o?/^\d+(\.\d+)?%$/.test(o)?o:this.gapValue(o):"";return I`<div style="height:${i};${r?`width:${r}`:""}"></div>`}};customElements.define("forgeui-spacer",et);var tt=class extends g{static get properties(){return{props:{type:Object}}}static get styles(){return F`
  ...
  `}render(){let t=this.getArray("data"),e=this.getString("emptyMessage",""),o=this.getString("direction","column");(o==="row"||o==="horizontal")&&this.setAttribute("direction","row");let i=this.getString("gap","md");return this.style.gap=this.gapValue(i),t.length===0&&e?I`<div class="empty">${e}</div>`:I`<slot></slot>`}};customElements.define("forgeui-repeater",tt);import{html as q,css as Ft,nothing as Tr}from"lit";var rt=class extends g{static get styles(){return Ft`
  ...
  `}render(){let t=this.getProp("items")||[];return q`${t.map((e,o)=>{let i=o===t.length-1,r=typeof e=="string"?e:e.label,l=typeof e=="string"?"#":e.href;return q`
        ${o>0?q`<span class="sep">/</span>`:Tr}
        ${i?q`<span class="current">${r}</span>`:q`<a href="${l}">${r}</a>`}
      `})}`}};customElements.define("forgeui-breadcrumb",rt);var ot=class extends g{static get styles(){return Ft`
  ...
  `}render(){let t=this.getProp("steps")||[],e=this.getBoundProp("active",this.getProp("activeStep")??0),o=Number(e)||0;return q`${t.map((i,r)=>{let l=typeof i=="string"?i:i.label||i.title||`Step ${r+1}`,n=r===o,a=r<o;return q`<div class="step" ?active=${n} ?completed=${a}>
        <div class="circle">${a?"\u2713":r+1}</div>
        <div class="label">${l}</div>
      </div>`})}`}};customElements.define("forgeui-stepper",ot);var it=class extends g{static get styles(){return z`
  ...
  `}_statusClass(t){let e=String(t??"").toLowerCase().trim();return["done","complete","completed","success","active","ok","approved","paid"].includes(e)?"success":["in progress","in-progress","pending","warning","waiting","review"].includes(e)?"warning":["to do","to-do","todo","backlog","draft","new","inactive"].includes(e)?"neutral":["high","urgent","critical"].includes(e)?"error":["medium","med"].includes(e)?"warning":["low"].includes(e)?"info":["failed","error","rejected","blocked","overdue"].includes(e)?"error":"neutral"}_renderCell(t,e){let o=typeof t=="string"?t:t.key,i=e[o],r=t&&typeof t=="object"?t.type:void 0;if(i==null||i==="")return m`<span style="color:var(--forgeui-color-text-tertiary)">—</span>`;if(r==="badge"||r==="status"){let l=(t.variant&&typeof t.variant=="object"?t.variant[String(i).toLowerCase()]:null)||this._statusClass(i);return m`<span class="badge ${l}">${String(i)}</span>`}if(r==="number")return typeof i=="number"?i.toLocaleString():String(i);if(r==="date"){let l=typeof i=="string"||typeof i=="number"?new Date(i):i;return l instanceof Date&&!isNaN(l.getTime())?l.toLocaleDateString():String(i)}if(r==="currency"){let l=Number(i);return isNaN(l)?String(i):l.toLocaleString(void 0,{style:"currency",currency:t.currency||"USD"})}return r==="boolean"?i?"\u2713":"\u2717":String(i)}render(){let t=this.getProp("data"),e=this.getProp("columns")||[],o=this.getString("emptyMessage","No data yet"),i=this.getString("rowAction",""),r=this.getString("caption","");if(!Array.isArray(t))return m`<div class="empty">${o}</div>`;let l=e.length>0?e:t.length>0?Object.keys(t[0]):[];return l.length===0?m`<div class="empty">${o}</div>`:m`
      <table>
        ${r?m`<caption>${r}</caption>`:w}
        <thead><tr>${l.map(n=>{let a=typeof n=="string"?n:n.label||n.key,u=typeof n=="object"?n.align:void 0,c=typeof n=="object"?n.width:void 0;return m`<th class="${u==="right"?"align-right":u==="center"?"align-center":""}" style="${c?`width:${c}`:""}">${a}</th>`})}</tr></thead>
        <tbody>${t.length===0?m`<tr><td colspan=${l.length} class="empty">${o}</td></tr>`:t.map((n,a)=>{let u=!!i,c=u?String(n[typeof l[0]=="string"?l[0]:l[0]?.key]??`Row ${a+1}`):"";return m`<tr class="${u?"row-action":""}"
                tabindex=${u?0:w}
                role=${u?"button":w}
                aria-label=${u?c:w}
              ...
              ${l.map(d=>{let p=typeof d=="object"?d.align:void 0;return m`<td class="${p==="right"?"align-right":p==="center"?"align-center":""}">${this._renderCell(d,n)}</td>`})}</tr>`})}</tbody>
      </table>
    `}};customElements.define("forgeui-table",it);var st=class extends g{static get styles(){return z`
    :host { display:block; }
  ...
  `}render(){let t=this.getProp("data"),e=this.getString("dataPath","");!("data"in(this.props||{}))&&e&&this.store?.hasTable(e)&&(t=Object.values(this.store.getTable(e)));let o=this.getString("emptyMessage","No items");return!Array.isArray(t)||t.length===0?m`<div class="empty">${o}</div>`:m`<div class="list">${t.map((i,r)=>m`
      <div class="item" data-index=${r}><slot name="item" .item=${i} .index=${r}>${JSON.stringify(i)}</slot></div>
    `)}</div>`}};customElements.define("forgeui-list",st);var nt=class extends g{constructor(){super(...arguments);this._palette=["var(--forgeui-color-primary)","var(--forgeui-color-success)","var(--forgeui-color-warning)","var(--forgeui-color-error)","var(--forgeui-color-info)","var(--forgeui-color-chart-6)","var(--forgeui-color-chart-7)","var(--forgeui-color-chart-8)","var(--forgeui-color-chart-9)","var(--forgeui-color-chart-10)"]}static get styles(){return z`
    :host { display:block; min-width:0; }
  ...
  `}_niceMax(e){if(e<=0)return 1;let o=Math.pow(10,Math.floor(Math.log10(e))),i=e/o;return(i<=1?1:i<=2?2:i<=5?5:10)*o}render(){let e=this.getString("chartType","bar"),o=this.getProp("data")||[],i=this.getString("title",""),r=this.getString("xKey","label")||this.getString("labelKey","label"),l=this.getString("yKey","value")||this.getString("valueKey","value"),n=this.getString("color","");if(!o||o.length===0)return m`
        ${i?m`<div class="title">${i}</div>`:w}
        <div class="empty">No data to display</div>
      `;let a=o.map($=>typeof $=="number"?{label:"",value:$}:$&&typeof $=="object"?{label:String($[r]??$.label??$.name??$.x??""),value:Number($[l]??$.value??$.y??0)||0,color:$.color}:{label:String($),value:0}),u=600,c=260,d={top:8,right:16,bottom:36,left:48},p=u-d.left-d.right,x=c-d.top-d.bottom,O,Z=w;if(e==="pie"||e==="donut"){let $=a.reduce((M,A)=>M+Math.max(0,A.value),0)||1,L=u/2,R=c/2,P=Math.min(p,x)/2-8,E=e==="donut"?P*.55:0,k=-Math.PI/2,h=[],y=[];a.forEach((M,A)=>{let V=Math.max(0,M.value)/$,T=k,C=k+V*Math.PI*2;k=C;let Y=C-T>Math.PI?1:0,ie=L+P*Math.cos(T),mt=R+P*Math.sin(T),ht=L+P*Math.cos(C),bt=R+P*Math.sin(C),ae=M.color||this._palette[A%this._palette.length];if(y.push(ae),E>0){let Vt=L+E*Math.cos(T),Bt=R+E*Math.sin(T),qt=L+E*Math.cos(C),Wt=R+E*Math.sin(C);h.push(_`<path class="slice" fill="${ae}" d="M ${ie} ${mt} A ${P} ${P} 0 ${Y} 1 ${ht} ${bt} L ${qt} ${Wt} A ${E} ${E} 0 ${Y} 0 ${Vt} ${Bt} Z"/>`)}else h.push(_`<path class="slice" fill="${ae}" d="M ${L} ${R} L ${ie} ${mt} A ${P} ${P} 0 ${Y} 1 ${ht} ${bt} Z"/>`)}),O=_`<g>${h}</g>`,Z=m`<div class="legend">${a.map((M,A)=>m`
        <span class="legend-item"><span class="swatch" style="background:${y[A]}"></span>${M.label} (${M.value})</span>
      `)}</div>`}else{let $=Math.max(...a.map(k=>k.value),0),L=this._niceMax($),R=k=>d.top+x-k/L*x,P=4,E=[];for(let k=0;k<=P;k++){let h=L*k/P,y=R(h);E.push(_`<line class="grid" x1="${d.left}" x2="${d.left+p}" y1="${y}" y2="${y}"/>`),E.push(_`<text class="tick-label" x="${d.left-6}" y="${y+3}" text-anchor="end">${h.toLocaleString()}</text>`)}if(e==="line"||e==="area"){let k=p/Math.max(1,a.length-1),h=a.map((A,V)=>{let T=d.left+V*k,C=R(A.value);return`${V===0?"M":"L"} ${T} ${C}`}).join(" "),y=e==="area"?h+` L ${d.left+p} ${d.top+x} L ${d.left} ${d.top+x} Z`:"",M=n||"var(--forgeui-color-primary)";O=m`
          <g>${E}</g>
          ${e==="area"?_`<path class="area" d="${y}" style="fill:${M};opacity:0.15"/>`:w}
          ${_`<path class="line" d="${h}" style="stroke:${M}"/>`}
          ${a.map((A,V)=>{let T=d.left+V*k,C=R(A.value);return _`<circle class="point" cx="${T}" cy="${C}" r="3" style="fill:${M}"/>
              <text class="tick-label" x="${T}" y="${d.top+x+14}" text-anchor="middle">${A.label}</text>`})}
        `}else{let k=a.length,h=p/k,y=Math.max(2,h*.7),M=h-y;O=m`
          <g>${E}</g>
          ${a.map((A,V)=>{let T=d.left+V*h+M/2,C=R(A.value),Y=Math.max(0,d.top+x-C),ie=A.color||n||"var(--forgeui-color-primary)";return _`<rect class="bar" x="${T}" y="${C}" width="${y}" height="${Y}" rx="2" style="fill:${ie}">
                <title>${A.label}: ${A.value}</title>
              </rect>
              <text class="tick-label" x="${T+y/2}" y="${d.top+x+14}" text-anchor="middle">${A.label}</text>`})}
        `}}return m`
      ${i?m`<div class="title">${i}</div>`:w}
      <div class="wrap">
    ...
    `}};customElements.define("forgeui-chart",nt);var at=class extends g{static get styles(){return z`
  ...
  `}render(){let t=this.getString("variant","info"),e=this.getString("title",""),o=this.getString("message","");return m`<div class="alert ${t}" role=${t==="error"||t==="warning"?"alert":"status"}>
      ${e?m`<strong>${e}</strong> `:w}${o}<slot></slot>
    </div>`}};customElements.define("forgeui-alert",lt);var ct=class extends g{constructor(){super(...arguments);this._priorFocus=null;this._keydownHandler=e=>this._onKeydown(e);this._close=()=>{this.dispatchAction("close")}}static get styles(){return z`
    :host { display:none; }
  ...
  `}render(){let e=this.getString("title",""),o=this.getBool("open"),i=`${this._instanceId}-title`;return o?this.setAttribute("open",""):this.removeAttribute("open"),o?m`
  ...
  `}render(){let t=this.getProp("value"),e=this.getNumber("max",100),o=t==null,i=o?0:Math.max(0,Math.min(Number(t),e)),r=o?0:i/e*100,l=this.getString("label",""),n=this.getBool("showValue");return m`
      ${l||n?m`
        <div class="meta">
          ${l?m`<span>${l}</span>`:m`<span></span>`}
          ${n?m`<span class="value">${Math.round(r)}%</span>`:w}
        </div>
      `:w}
      <div
    ...
    `}};customElements.define("forgeui-progress",ut);var dt=class extends g{static get styles(){return z`
  ...
  `}render(){let t=this.getString("message","");return t?m`<div class="toast">${t}</div>`:m`${w}`}};customElements.define("forgeui-toast",dt);var pt=class extends g{static get styles(){return z`
    :host { display:block; }
  ...
  `}render(){let t=this.getString("msg","Unknown error");return m`<div class="error" role="alert">⚠ ${t}</div>`}};customElements.define("forgeui-error",pt);var gt=class extends g{static get properties(){return{props:{type:Object}}}static get styles(){return z`
    :host { display:block; }
    svg { display:block; }
