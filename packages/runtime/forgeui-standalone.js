...
`;function ve(s="default"){switch(s){case"minimal":return Mt();case"default":return Tt();case"full":return wr()}}function Mt(){let t=Object.entries(yt).map(([e,o])=>`  ${e}: ${o.join(", ")}`).join(`
...
`)}function we(){return{type:"object",required:["manifest","id","elements"],properties:{manifest:{type:"string",const:"0.1.0"},id:{type:"string",pattern:"^[a-z0-9][a-z0-9-]*$"},root:{type:"string"},title:{type:"string"},schema:{type:"object",properties:{version:{type:"number"},tables:{type:"object",additionalProperties:{type:"object",properties:{columns:{type:"object",additionalProperties:{type:"object",properties:{type:{type:"string",enum:["string","number","boolean"]},default:{}},required:["type"]}}},required:["columns"]}}}},state:{type:"object",additionalProperties:{}},elements:{type:"object",additionalProperties:{type:"object",properties:{type:{type:"string",enum:ue},props:{type:"object"},children:{type:"array",items:{type:"string"}},visible:{type:"object",properties:{$when:{type:"object",properties:{path:{type:"string"},eq:{},neq:{},gt:{type:"number"},lt:{type:"number"},gte:{type:"number"},lte:{type:"number"},in:{type:"array"},notIn:{type:"array"},exists:{type:"boolean"}},required:["path"]}}}},required:["type"]}},actions:{type:"object",additionalProperties:{type:"object",properties:{type:{type:"string",enum:["mutateState","navigate","openDialog","closeDialog","callApi","toast","custom"]},path:{type:"string"},value:{},operation:{type:"string",enum:["set","append","update","delete","increment","decrement","toggle"]},set:{type:"object"},data:{type:"object"},key:{type:"string"},formId:{type:"string"},action:{type:"string"},target:{type:"string"},url:{type:"string"},method:{type:"string"},body:{type:"object"}},required:["type"]}},dataAccess:{type:"object",properties:{enabled:{type:"boolean",default:!1},readable:{type:"array",items:{type:"string"}},restricted:{type:"array",items:{type:"string"}},summaries:{type:"boolean",default:!1}}},persistState:{type:"boolean"},skipPersistState:{type:"boolean"}}}}var xr={container:"Stack",row:"Stack",column:"Stack",text:"Text",heading:"Text",label:"Text",image:"Image",icon:"Icon",button:"Button",link:"Link",textInput:"TextInput",numberInput:"NumberInput",checkbox:"Checkbox",toggle:"Toggle",select:"Select",divider:"Divider",spacer:"Spacer",card:"Card",table:"Table",list:"List",alert:"Alert",progress:"Progress",badge:"Badge",avatar:"Avatar"};function $r(s){let t={},e=0;function o(n){let a=n.id||`a2ui-${e++}`,u=xr[n.type];if(!u||!X(u))return t[a]={type:"Text",props:{content:`[Unsupported A2UI type: ${n.type}]`,variant:"caption",colorScheme:"secondary"}},a;let c=kr(n.type,n.props||{}),d=(n.children||[]).map(p=>o(p));return t[a]={type:u,props:c,children:d.length>0?d:void 0},a}let r=(s.components||s.content||[]).map(n=>o(n)),l=r.length===1?r[0]:"a2ui-root";return r.length>1&&(t[l]={type:"Stack",props:{gap:"md"},children:r}),{manifest:"0.1.0",id:`a2ui-${Date.now()}`,root:l,elements:t,actions:{}}}function kr(s,t){let e={...t};switch(s){case"heading":e.variant="heading",e.content=t.text||t.content||"",delete e.text;break;case"text":case"label":e.content=t.text||t.content||"",delete e.text;break;case"row":e.direction="row",e.gap=t.gap||t.spacing||"md";break;case"column":e.direction="column",e.gap=t.gap||t.spacing||"md";break;case"container":e.gap=t.gap||t.spacing||"md";break;case"image":e.src=t.src||t.url||"",e.alt=t.alt||"";break;case"button":e.label=t.text||t.label||"",e.variant=t.variant||t.style||"default";break;case"textInput":e.placeholder=t.placeholder||"",e.value=t.value||"",e.label=t.label||"";break;case"numberInput":e.placeholder=t.placeholder||"",e.value=t.value??0,e.label=t.label||"";break;case"select":t.options&&Array.isArray(t.options)&&(e.options=t.options.map(o=>typeof o=="string"?{value:o,label:o}:o));break;case"alert":e.variant=t.type||t.severity||"info",e.content=t.message||t.text||t.content||"",delete e.message,delete e.text;break;case"progress":e.value=t.value??t.percent??0;break}return e}function Ct(s){if(!s||typeof s!="object")return!1;let t=s;return!!(Array.isArray(t.components)||Array.isArray(t.content)||typeof t.type=="string"&&(t.type==="adaptive-card"||t.type.startsWith("a2ui"))||typeof t.version=="string"&&t.version.startsWith("a2ui")||!t.elements&&!t.manifest&&Array.isArray(t.content))}function xe(s){return Ct(s)?$r(s):s}function jt(s){return`forgeui_${(s||"global").replace(/[^a-zA-Z0-9-]/g,"_")}`}var $e="f";function Sr(s){return new Promise((t,e)=>{let o=r=>e(r??new Error("IDB"));if(!globalThis.indexedDB)return o();let i=indexedDB.open(`${jt(s)}_f`,1);i.onupgradeneeded=()=>{i.result.createObjectStore($e)},i.onsuccess=()=>t(i.result),i.onerror=i.onblocked=()=>o(i.error)})}async function Rt(s,t){if(s.length===0)return;let e=null;try{e=await Sr(t),await new Promise((o,i)=>{let r=e.transaction($e,"readwrite"),l=r.objectStore($e);for(let{file:n,id:a}of s)l.put(n,a);r.oncomplete=()=>o(),r.onerror=r.onabort=()=>i(r.error)})}catch{return}finally{e?.close()}}function Dt(s,t,e="none"){let o=jt(t),i=e,r=null,l=!1,n=!1,a=null,u=null,c=null,d=[];function p(){let h=w();for(let v of d)try{v(h)}catch{}}function w(){return{mode:i,isPersisting:r!=null&&l,lastSaved:a,lastLoaded:u,error:c,dbName:i==="indexeddb"?o:null}}async function L(){if(r)return r;try{let{createIndexedDbPersister:h}=await import("tinybase/persisters/persister-indexed-db");return r=h(s,o),c=null,r}catch(h){throw c=`IndexedDB unavailable: ${h.message}`,p(),h}}async function H(){if(i==="none")return;let h=await L();try{await h.load(),u=Date.now()}catch(v){c=`Load failed: ${v.message}`,p()}try{h.startAutoSave(),l=!0}catch(v){c=`Auto-save failed: ${v.message}`}try{h.startAutoLoad(1),n=!0}catch(v){c=`Auto-load failed: ${v.message}`}p()}async function x(){if(r){l&&(r.stopAutoSave(),l=!1),n&&(r.stopAutoLoad(),n=!1);try{await r.save(),a=Date.now()}catch(h){c=`Final save failed: ${h.message}`}p()}}async function D(){if(!(i==="none"||!r)){try{await r.save(),a=Date.now(),c=null}catch(h){c=`Save failed: ${h.message}`}p()}}async function R(){if(!(i==="none"||!r)){try{await r.load(),u=Date.now(),c=null}catch(h){c=`Load failed: ${h.message}`}p()}}async function P(){if(await x(),r){try{r.destroy()}catch{}r=null}d=[],p()}function E(h){if(h===i)return;let v=i;i=h,v==="indexeddb"&&h==="none"&&(r&&l&&(r.stopAutoSave(),l=!1),r&&n&&(r.stopAutoLoad(),n=!1)),h==="indexeddb"&&v==="none"&&H().catch(()=>{}),p()}function k(h){return d.push(h),()=>{d=d.filter(v=>v!==h)}}return{start:H,stop:x,save:D,load:R,setMode:E,getStatus:w,addListener:k,destroy:P}}var re=class{constructor(t=50){this._stack=[];this._position=-1;this._listeners=[];this._maxDepth=t}push(t){if(this._position<this._stack.length-1&&(this._stack=this._stack.slice(0,this._position+1)),this._position>=0){let e=this._stack[this._position];if(JSON.stringify(e)===JSON.stringify(t))return}this._stack.push(structuredClone(t)),this._stack.length>this._maxDepth?this._stack.shift():this._position++,this._notifyListeners()}undo(){return this._position<=0?null:(this._position--,this._notifyListeners(),structuredClone(this._stack[this._position]))}redo(){return this._position>=this._stack.length-1?null:(this._position++,this._notifyListeners(),structuredClone(this._stack[this._position]))}current(){return this._position<0?null:structuredClone(this._stack[this._position])}jumpTo(t){return t<0||t>=this._stack.length?null:(this._position=t,this._notifyListeners(),structuredClone(this._stack[this._position]))}getState(){return{canUndo:this._position>0,canRedo:this._position<this._stack.length-1,position:this._position,total:this._stack.length}}getTimeline(){return this._stack.map((t,e)=>({position:e,id:t.id}))}clear(){this._stack=[],this._position=-1,this._notifyListeners()}addListener(t){return this._listeners.push(t),()=>{this._listeners=this._listeners.filter(e=>e!==t)}}_notifyListeners(){let t=this.getState();for(let e of this._listeners)try{e(t)}catch{}}};function Lt(s){if(!W(s))return s;let t=typeof s.manifest=="string"?s.manifest:"";if(!(/^0\.0(?:\.|$)/.test(t)||!t&&("version"in s||"manifestVersion"in s||"components"in s||"rootElement"in s||_r(s.elements))))return s;let r={...s,manifest:"0.1.0"};if(delete r.version,delete r.manifestVersion,!r.elements&&W(r.components)&&(r.elements=r.components,delete r.components),!r.root&&typeof r.rootElement=="string"&&(r.root=r.rootElement,delete r.rootElement),W(r.elements)){let l={};for(let[n,a]of Object.entries(r.elements)){if(!W(a))continue;let u={...a};!u.type&&typeof u.component=="string"&&(u.type=u.component,delete u.component),l[n]=u}r.elements=l}return r}function _r(s){return W(s)?Object.values(s).some(t=>W(t)&&"component"in t):!1}function W(s){return s!==null&&typeof s=="object"&&!Array.isArray(s)}import{html as g,css as $,svg as _,nothing as y}from"lit";import{LitElement as Ar}from"lit";var oe=class oe extends Ar{constructor(){super(...arguments);this._instanceId=`forge-${++oe._instanceCounter}`;this.props={};this.store=null;this.onAction=null;this.itemContext=null}static get properties(){return{props:{type:Object}}}connectedCallback(){super.connectedCallback()}resolve(e){if(!this.store)return e;this.itemContext&&B(this.itemContext);try{return S(this.store,e)}finally{B(null)}}getProp(e){let o=this.props?.[e];return typeof o=="string"&&(o.startsWith("$state:")||o.startsWith("$computed:")||o.startsWith("$item:")||o.startsWith("$expr:")||o.includes("{{")&&o.includes("}}"))?this.resolve(o):o}getArray(e){let o=this.getProp(e);return Array.isArray(o)?o:o&&typeof o=="object"?Object.values(o):[]}getString(e,o=""){let i=this.getProp(e);return typeof i=="string"?i:String(i??o)}getNumber(e,o=0){let i=this.getProp(e);return typeof i=="number"?i:Number(i)||o}getBool(e,o=!1){let i=this.getProp(e);return typeof i=="boolean"?i:o}getBoundProp(e,o){let i=typeof this.props?.bind=="string"?this.props.bind:"";if(i){let l=this.resolve(i);if(l!==void 0)return l}let r=this.getProp(e);return r===void 0?o:r}dispatchAction(e,o){let i=typeof this.props?.bind=="string"?this.props.bind:"",r=i?{...o||{},bind:i}:o;this.onAction&&this.onAction(e,r),this.dispatchEvent(new CustomEvent("forgeui-action",{detail:{action:e,payload:r},bubbles:!0,composed:!0}))}handleAction(e){let o=this.getString("action");o&&this.dispatchAction(o,this.props)}prop(e){return this.getProp(e)}static get sharedStyles(){return[ye]}gapValue(e){let o={none:"0",0:"0","3xs":"var(--forgeui-space-3xs)","2xs":"var(--forgeui-space-2xs)",xs:"var(--forgeui-space-xs)",sm:"var(--forgeui-space-sm)",md:"var(--forgeui-space-md)",lg:"var(--forgeui-space-lg)",xl:"var(--forgeui-space-xl)","2xl":"var(--forgeui-space-2xl)"};if(e==null||e==="")return"var(--forgeui-space-md)";let i=String(e);return i in o?o[i]:/^\d+(\.\d+)?$/.test(i)?`${i}px`:/^\d+(\.\d+)?(px|rem|em|%|vw|vh|ch)$/.test(i)?i:"var(--forgeui-space-md)"}static get styles(){return[ye]}};oe._instanceCounter=0;var f=oe;import{html as Ae,css as Pe,nothing as Pr}from"lit";var ke=class extends f{static get styles(){return Pe`
  ...
  `}render(){let t=this.getString("label","Button"),e=this.getString("variant","primary"),o=this.getString("size",""),i=this.getBool("disabled"),r=this.getProp("pressed");return Ae`<button class="${e} ${o}" ?disabled=${i}
      aria-pressed=${r==null?Pr:String(!!r)}
      @click=${l=>this.handleAction(l)}>${t}<slot></slot></button>`}};customElements.define("forgeui-button",ke);var Se=class extends f{static get styles(){return Pe`
    :host { display:flex; gap:var(--forgeui-space-xs); }
  `}render(){return Ae`<slot></slot>`}};customElements.define("forgeui-button-group",Se);var _e=class extends f{static get styles(){return Pe`
    :host { display:inline-flex; }
  ...
  `}render(){let t=this.getString("label",""),e=this.getString("href","#");return Ae`<a href="${e}">${t}<slot></slot></a>`}};customElements.define("forgeui-link",_e);import{html as j,css as K,nothing as zt}from"lit";var Ee=class extends f{static get properties(){return{props:{type:Object}}}static get styles(){return K`
  ...
  `}render(){let t=this.getString("content",""),e=this.getString("variant","body"),i={h1:"heading1",h2:"heading2",h3:"heading3",title:"heading2",subtitle:"subheading",paragraph:"body",text:"body",secondary:"muted",tertiary:"caption"}[e]||e,r=this.getString("colorScheme",""),l=this.getString("align",""),n=this.getString("weight",""),a={primary:"var(--forgeui-color-primary)",secondary:"var(--forgeui-color-text-secondary)",tertiary:"var(--forgeui-color-text-tertiary)",success:"var(--forgeui-color-success)",warning:"var(--forgeui-color-warning)",error:"var(--forgeui-color-error)",info:"var(--forgeui-color-info)"},u={normal:"var(--forgeui-weight-normal)",medium:"var(--forgeui-weight-medium)",semibold:"var(--forgeui-weight-semibold)",bold:"var(--forgeui-weight-bold)"},c=[];r&&a[r]&&c.push(`color:${a[r]}`),n&&u[n]&&c.push(`font-weight:${u[n]}`);let d=l?`align-${l}`:"",p=j`${t}<slot></slot>`;return i==="heading1"?j`<h1 class="${i} ${d}" style="${c.join(";")}">${p}</h1>`:i==="heading2"?j`<h2 class="${i} ${d}" style="${c.join(";")}">${p}</h2>`:i==="heading3"?j`<h3 class="${i} ${d}" style="${c.join(";")}">${p}</h3>`:j`<div class="${i} ${d}" style="${c.join(";")}">${t}<slot></slot></div>`}};customElements.define("forgeui-text",Ee);var Ie=class extends f{static get styles(){return K`
    :host { display:block; }
    img { max-width:100%; height:auto; display:block; border-radius:var(--forgeui-radius-md); }
  `}render(){let t=this.getString("src",""),e=this.getString("alt",""),o=this.getString("fit","contain");return t?j`<img src="${t}" alt="${e}" style="object-fit:${o}" loading="lazy">`:j`${zt}`}};customElements.define("forgeui-image",Ie);var Me=class extends f{static get styles(){return K`
    :host { display:inline-flex; align-items:center; justify-content:center; }
    svg { width:var(--forgeui-icon-md); height:var(--forgeui-icon-md); fill:currentColor; }
  `}render(){let t=this.getString("name","circle"),e={check:"M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z",x:"M6 18L18 6M6 6l12 12",plus:"M12 4v16m8-8H4",minus:"M20 12H4",chevron:"M9 5l7 7-7 7",arrow:"M13 7l5 5m0 0l-5 5m5-5H6",star:"M11.049 2.927c.3-.921 1.603-.921 1.902 0l1.286 3.96a1 1 0 00.95.69h4.162c.969 0 1.371 1.24.588 1.81l-3.37 2.448a1 1 0 00-.364 1.118l1.287 3.96c.3.921-.755 1.688-1.54 1.118l-3.37-2.448a1 1 0 00-1.176 0l-3.37 2.448c-.784.57-1.838-.197-1.539-1.118l1.287-3.96a1 1 0 00-.364-1.118L2.063 8.387c-.783-.57-.38-1.81.588-1.81h4.162a1 1 0 00.95-.69l1.286-3.96z",circle:"M12 2a10 10 0 100 20 10 10 0 000-20z",alert:"M12 9v2m0 4h.01M4.93 4.93l14.14 14.14M12 2a10 10 0 100 20 10 10 0 000-20z"},o=e[t]||e.circle;return j`<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="${o}"/></svg>`}};customElements.define("forgeui-icon",Me);var Te=class extends f{static get styles(){return K`
  ...
  `}render(){let t=this.getString("text","")||this.getString("label",""),e=this.getString("variant","");return j`<span class="badge" variant="${e}">${t}<slot></slot></span>`}};customElements.define("forgeui-badge",Te);var Ce=class extends f{static get styles(){return K`
    :host { display:inline-flex; }
  ...
  `}render(){let t=this.getString("src",""),e=this.getString("name","?"),o=e.split(" ").map(i=>i[0]).join("").toUpperCase().slice(0,2);return j`<div class="avatar">${t?j`<img src="${t}" alt="${e}">`:o}<slot></slot></div>`}};customElements.define("forgeui-avatar",Ce);var je=class extends f{static get styles(){return K`
  ...
  `}render(){let t=this.getString("title","Nothing here"),e=this.getString("description","");return j`
      ...
      ${e?j`<div class="desc">${e}</div>`:zt}
      <slot></slot>
    `}};customElements.define("forgeui-empty-state",je);import{html as I,css as O,nothing as Re}from"lit";var De=class extends f{static get properties(){return{props:{type:Object}}}static get styles(){return O`
  ...
  `}render(){let t=this.getProp("columns"),e;typeof t=="number"?e=String(t):typeof t=="string"&&t?e=t:e="1";let o=/^\d+$/.test(e)?`repeat(${e}, minmax(0, 1fr))`:e,i=this.getString("gap","md"),r=this.gapValue(i),l=this.getString("padding",""),n=l?this.gapValue(l):"0";return this.style.gridTemplateColumns=o,this.style.gap=r,this.style.padding=n,/^\d+$/.test(e)&&Number(e)>=2&&this.setAttribute("responsive",""),I`<slot></slot>`}};customElements.define("forgeui-grid",Le);var ze=class extends f{static get properties(){return{props:{type:Object}}}static get styles(){return O`
  ...
  `}render(){let t=this.getString("variant",""),e=this.getString("title",""),o=this.getString("subtitle","");return t&&this.setAttribute("variant",t),I`
      ${e||o?I`
        <div class="header">
          ${e?I`<div class="title">${e}</div>`:Re}
          ${o?I`<div class="subtitle">${o}</div>`:Re}
        </div>
      `:Re}
      <div class="body"><slot></slot></div>
    `}};customElements.define("forgeui-card",ze);var Oe=class extends f{static get properties(){return{props:{type:Object}}}static get styles(){return O`:host { display:block; margin-inline:auto; width:100%; box-sizing:border-box; }`}render(){let t=this.getString("maxWidth",""),e={sm:"640px",md:"768px",lg:"1024px",xl:"1280px","2xl":"1536px",full:"100%",none:"none","":""},o=t in e?e[t]:t,i=this.getString("padding","");return o&&o!=="none"?this.style.maxWidth=o:this.style.maxWidth="",this.style.padding=i?this.gapValue(i):"",I`<slot></slot>`}};customElements.define("forgeui-container",Oe);var Ue=class extends f{static get properties(){return{props:{type:Object}}}constructor(){super(),this._active=""}static get styles(){return O`
    :host { display:block; }
  ...
  `}_itemKey(t){return typeof t=="string"?t:String(t&&typeof t=="object"?t.id??t.key??t.value??t.label??"":t??"")}_itemLabel(t){return typeof t=="string"?t:String(t&&typeof t=="object"?t.label??t.title??t.value??"":t??"")}updated(){Array.from(this.children).filter(e=>!(e instanceof HTMLScriptElement)).forEach((e,o)=>{let i=(e.props||{}).slot??e.getAttribute("slot");String(o)===this._active||i===this._active?e.setAttribute("data-active",""):e.removeAttribute("data-active")})}_moveTo(t,e){let o=this._itemKey(e[t])||String(t);this._active=o,this.requestUpdate(),this.dispatchAction("tab-change",{active:o,value:o}),this.updateComplete.then(()=>{this.shadowRoot?.querySelector(`#${this._instanceId}-tab-${t}`)?.focus()})}render(){let t=this.getProp("items")||this.getProp("tabs")||[],e=Array.isArray(t)?t:[],o=this.getBoundProp("activeTab",this.getProp("value"));o!==void 0&&String(o)!==this._active&&(this._active=String(o)),!this._active&&e.length>0&&(this._active=this._itemKey(e[0])||"0");let i=e.findIndex((l,n)=>(this._itemKey(l)||String(n))===this._active),r=(l,n)=>{let a=-1;l.key==="ArrowRight"?a=(n+1)%e.length:l.key==="ArrowLeft"?a=(n-1+e.length)%e.length:l.key==="Home"?a=0:l.key==="End"&&(a=e.length-1),a!==-1&&(l.preventDefault(),this._moveTo(a,e))};return I`
    ...
    `}};customElements.define("forgeui-tabs",Ue);var Ne=class extends f{static get properties(){return{props:{type:Object}}}static get styles(){return O`
    :host { display:block; }
  ...
  `}render(){let t=this.getString("title","Section");return I`<details><summary>${t}</summary><div class="content"><slot></slot></div></details>`}};customElements.define("forgeui-accordion",Ne);var Fe=class extends f{static get styles(){return O`
    :host { display:block; }
    hr { border:none; border-top:1px solid var(--forgeui-color-border); margin:var(--forgeui-space-sm) 0; }
  `}render(){return I`<hr>`}};customElements.define("forgeui-divider",Fe);var Ve=class extends f{static get styles(){return O`:host { display:block; }`}render(){let t=this.getString("size","md"),e=this.getString("height",""),o=this.getString("width",""),i=e?this.gapValue(e):this.gapValue(t),r=o?/^\d+(\.\d+)?%$/.test(o)?o:this.gapValue(o):"";return I`<div style="height:${i};${r?`width:${r}`:""}"></div>`}};customElements.define("forgeui-spacer",Ve);var Be=class extends f{static get properties(){return{props:{type:Object}}}static get styles(){return O`
  ...
  `}render(){let t=this.getArray("data"),e=this.getString("emptyMessage",""),o=this.getString("direction","column");(o==="row"||o==="horizontal")&&this.setAttribute("direction","row");let i=this.getString("gap","md");return this.style.gap=this.gapValue(i),t.length===0&&e?I`<div class="empty">${e}</div>`:I`<slot></slot>`}};customElements.define("forgeui-repeater",Be);import{html as F,css as Ot,nothing as Er}from"lit";var qe=class extends f{static get styles(){return Ot`
  ...
  `}render(){let t=this.getProp("items")||[];return F`${t.map((e,o)=>{let i=o===t.length-1,r=typeof e=="string"?e:e.label,l=typeof e=="string"?"#":e.href;return F`
        ${o>0?F`<span class="sep">/</span>`:Er}
        ${i?F`<span class="current">${r}</span>`:F`<a href="${l}">${r}</a>`}
      `})}`}};customElements.define("forgeui-breadcrumb",qe);var We=class extends f{static get styles(){return Ot`
  ...
  `}render(){let t=this.getProp("steps")||[],e=this.getBoundProp("active",this.getProp("activeStep")??0),o=Number(e)||0;return F`${t.map((i,r)=>{let l=typeof i=="string"?i:i.label||i.title||`Step ${r+1}`,n=r===o,a=r<o;return F`<div class="step" ?active=${n} ?completed=${a}>
        <div class="circle">${a?"\u2713":r+1}</div>
        <div class="label">${l}</div>
      </div>`})}`}};customElements.define("forgeui-stepper",We);var Ke=class extends f{static get styles(){return $`
  ...
  `}render(){let t=this.getString("label",""),e=this.getString("placeholder",""),o=this.getString("hint",""),i=this.getString("error",""),r=this.getString("inputType","")||this.getString("type","text"),l=this.getBool("multiline"),n=String(this.getBoundProp("value","")??""),a=this._instanceId;return g`
      ${t?g`<label for="${a}">${t}</label>`:y}
      ${l?g`<textarea id="${a}" placeholder="${e}" .value=${n} @input=${u=>this.dispatchAction("change",{value:u.target.value})}></textarea>`:g`<input id="${a}" type="${r}" placeholder="${e}" .value=${n} @input=${u=>this.dispatchAction("change",{value:u.target.value})}>`}
      ${o&&!i?g`<div class="hint">${o}</div>`:y}
      ${i?g`<div class="error">${i}</div>`:y}
    `}};customElements.define("forgeui-text-input",Ke);var He=class extends f{static get styles(){return $`
    :host { display:block; flex:1 1 auto; min-width:0; max-width:100%; margin-bottom:var(--forgeui-space-sm); }
  ...
  `}render(){let t=this.getString("label",""),e=this.getProp("min"),o=this.getProp("max"),i=this.getProp("step"),r=this.getBoundProp("value"),l=this._instanceId;return g`
      ${t?g`<label for="${l}">${t}</label>`:y}
      <input id="${l}" type="number" min=${e} max=${o} step=${i} .value=${r??""}
        @input=${n=>this.dispatchAction("change",{value:Number(n.target.value)})}>
    `}};customElements.define("forgeui-number-input",He);var Ge=class extends f{static get styles(){return $`
    :host { display:block; flex:1 1 auto; min-width:0; max-width:100%; margin-bottom:var(--forgeui-space-sm); }
    label { display:block; font-size:var(--forgeui-text-sm); font-weight:var(--forgeui-weight-medium); margin-bottom:var(--forgeui-space-2xs); overflow-wrap:break-word; }
  ...
  `}render(){let t=this.getString("label",""),e=this.getProp("options")||[],o=String(this.getBoundProp("value","")??""),i=this._instanceId;return g`
      ${t?g`<label for="${i}">${t}</label>`:y}
      <select id="${i}" .value=${o} @change=${r=>this.dispatchAction("change",{value:r.target.value})}>
        ${e.map(r=>g`<option value=${typeof r=="string"?r:r.value} ?selected=${(typeof r=="string"?r:r.value)===o}>
  ...
  `}render(){let t=this.getString("label",""),e=!!this.getBoundProp("checked",this.getProp("value")??!1),o=this._instanceId;return g`
      <input id="${o}" type="checkbox" ?checked=${e} @change=${i=>this.dispatchAction("change",{checked:i.target.checked})}>
      ${t?g`<label for="${o}">${t}</label>`:y}
    `}};customElements.define("forgeui-checkbox",Ze);var Ye=class extends f{constructor(){super(...arguments);this._toggle=()=>{if(this.getBool("disabled"))return;let e=!!this.getBoundProp("on",this.getProp("value")??!1);this.dispatchAction("change",{value:!e,checked:!e})};this._onKeydown=e=>{(e.key==="Enter"||e.key===" "||e.key==="Spacebar")&&(e.preventDefault(),this._toggle())}}static get styles(){return $`
  ...
  `}render(){let e=!!this.getBoundProp("on",this.getProp("value")??!1),o=this.getString("label",""),i=this.getBool("disabled"),r=this._instanceId;return g`
      <label for="${r}" class="toggle-label">
        ...
        ></button>
        ${o?g`<span class="toggle-text">${o}</span>`:y}
      </label>
    `}};customElements.define("forgeui-toggle",Ye);var Xe=class extends f{static get styles(){return $`
    :host { display:block; flex:1 1 auto; min-width:0; max-width:100%; margin-bottom:var(--forgeui-space-sm); }
    label { display:block; font-size:var(--forgeui-text-sm); font-weight:var(--forgeui-weight-medium); margin-bottom:var(--forgeui-space-2xs); overflow-wrap:break-word; }
    input { width:100%; padding:var(--forgeui-space-xs) var(--forgeui-space-sm); border:1px solid var(--forgeui-color-border);
      border-radius:var(--forgeui-radius-md); font:inherit; height:var(--forgeui-input-height);
      background:var(--forgeui-color-surface); color:var(--forgeui-color-text); box-sizing:border-box; min-width:0; }
    input:focus { outline:none; border-color:var(--forgeui-color-primary); box-shadow:0 0 0 3px var(--forgeui-color-primary-subtle); }
  `}render(){let t=this.getString("label",""),e=this.getString("value",""),o=this._instanceId;return g`
      ${t?g`<label for="${o}">${t}</label>`:y}
      <input id="${o}" type="date" .value=${e} @change=${i=>this.dispatchAction("change",{value:i.target.value})}>
    `}};customElements.define("forgeui-date-picker",Xe);var Qe=class extends f{static get styles(){return $`
    :host { display:block; flex:1 1 auto; min-width:0; max-width:100%; margin-bottom:var(--forgeui-space-sm); }
    label { display:block; font-size:var(--forgeui-text-sm); font-weight:var(--forgeui-weight-medium); margin-bottom:var(--forgeui-space-2xs); overflow-wrap:break-word; }
  ...
  `}render(){let t=this.getString("label",""),e=this.getNumber("min",0),o=this.getNumber("max",100),i=this.getNumber("step",1),r=this.getBoundProp("value",e),l=Number(r);Number.isFinite(l)||(l=e);let n=this._instanceId;return g`
      ${t?g`<label for="${n}">${t}</label>`:y}
      <input id="${n}" type="range" min=${e} max=${o} step=${i} .value=${l}
        @input=${a=>this.dispatchAction("change",{value:Number(a.target.value)})}>
      <div class="value">${l}</div>
    `}};customElements.define("forgeui-slider",Qe);var et=class extends f{constructor(){super(...arguments);this._dragging=!1;this._openFilePicker=()=>{this.shadowRoot?.querySelector('input[type="file"]')?.click()};this._onDropzoneKeydown=e=>{e.key!=="Enter"&&e.key!==" "||(e.preventDefault(),this._openFilePicker())};this._onFileChange=e=>{let o=Array.from(e.target.files??[]);this._processFiles(o)};this._onDragOver=e=>{e.preventDefault(),!this._dragging&&(this._dragging=!0,this.requestUpdate())};this._onDragLeave=e=>{e.currentTarget===e.target&&(this._dragging=!1,this.requestUpdate())};this._onDrop=e=>{e.preventDefault(),this._dragging=!1,this.requestUpdate(),this._processFiles(Array.from(e.dataTransfer?.files??[]))}}static get styles(){return $`
    :host { display:block; margin-bottom:var(--forgeui-space-sm); }
    label { display:block; font-size:var(--forgeui-text-sm); font-weight:var(--forgeui-weight-medium); margin-bottom:var(--forgeui-space-2xs); }
  ...
  `}_maxSizeBytes(){let e=this.getProp("maxSize");if(typeof e=="number"&&Number.isFinite(e)&&e>=0)return Math.floor(e);if(typeof e!="string")return null;let o=e.trim().match(/^(\d+(?:\.\d+)?)\s*(b|kb|mb|gb)?$/i);if(!o)return null;let i=Number(o[1]),r=(o[2]||"b").toLowerCase(),l=r==="gb"?1<<30:r==="mb"?1<<20:r==="kb"?1024:1,n=i*l;return Number.isFinite(n)?Math.floor(n):null}_newFileId(){return globalThis.crypto?.randomUUID?.()??`${Date.now()}_${Math.random().toString(36).slice(2)}`}_processFiles(e){let o=this.getBool("multiple"),i=this._maxSizeBytes(),r=(o?e:e.slice(0,1)).map(d=>{let p=this._newFileId(),w=i==null||d.size<=i,L={id:p,name:d.name,size:d.size,type:d.type,lastModified:d.lastModified,accepted:w,storageKey:w?p:null};return w||(L.error="maxSize"),[d,L]}),l=r.filter(([,d])=>d.accepted),n=r.map(([,d])=>d),a=l.map(([,d])=>d),u=o?a:a[0]??null,c=a[0]??null;this.dispatchAction("change",{id:c?.id??null,uuid:c?.id??null,name:c?.name??null,size:c?.size??null,type:c?.type??null,lastModified:c?.lastModified??null,storageKey:c?.storageKey??null,value:u,files:n,rejected:n.filter(d=>!d.accepted),multiple:o,maxSize:i}),Rt(l.map(([d,p])=>({file:d,id:p.id})))}render(){let e=this.getString("label","Upload file"),o=this.getString("accept","*"),i=this.getBool("multiple");return g`
    ...
    `}};customElements.define("forgeui-file-upload",et);var tt=class extends f{static get styles(){return $`
  ...
  `}_statusClass(t){let e=String(t??"").toLowerCase().trim();return["done","complete","completed","success","active","ok","approved","paid"].includes(e)?"success":["in progress","in-progress","pending","warning","waiting","review"].includes(e)?"warning":["to do","to-do","todo","backlog","draft","new","inactive"].includes(e)?"neutral":["high","urgent","critical"].includes(e)?"error":["medium","med"].includes(e)?"warning":["low"].includes(e)?"info":["failed","error","rejected","blocked","overdue"].includes(e)?"error":"neutral"}_renderCell(t,e){let o=typeof t=="string"?t:t.key,i=e[o],r=t&&typeof t=="object"?t.type:void 0;if(i==null||i==="")return g`<span style="color:var(--forgeui-color-text-tertiary)">—</span>`;if(r==="badge"||r==="status"){let l=(t.variant&&typeof t.variant=="object"?t.variant[String(i).toLowerCase()]:null)||this._statusClass(i);return g`<span class="badge ${l}">${String(i)}</span>`}if(r==="number")return typeof i=="number"?i.toLocaleString():String(i);if(r==="date"){let l=typeof i=="string"||typeof i=="number"?new Date(i):i;return l instanceof Date&&!isNaN(l.getTime())?l.toLocaleDateString():String(i)}if(r==="currency"){let l=Number(i);return isNaN(l)?String(i):l.toLocaleString(void 0,{style:"currency",currency:t.currency||"USD"})}return r==="boolean"?i?"\u2713":"\u2717":String(i)}render(){let t=this.getProp("data"),e=this.getProp("columns")||[],o=this.getString("emptyMessage","No data yet"),i=this.getString("rowAction",""),r=this.getString("caption","");if(!Array.isArray(t))return g`<div class="empty">${o}</div>`;let l=e.length>0?e:t.length>0?Object.keys(t[0]):[];return l.length===0?g`<div class="empty">${o}</div>`:g`
      <table>
        ${r?g`<caption>${r}</caption>`:y}
        <thead><tr>${l.map(n=>{let a=typeof n=="string"?n:n.label||n.key,u=typeof n=="object"?n.align:void 0,c=typeof n=="object"?n.width:void 0;return g`<th class="${u==="right"?"align-right":u==="center"?"align-center":""}" style="${c?`width:${c}`:""}">${a}</th>`})}</tr></thead>
        <tbody>${t.length===0?g`<tr><td colspan=${l.length} class="empty">${o}</td></tr>`:t.map((n,a)=>{let u=!!i,c=u?String(n[typeof l[0]=="string"?l[0]:l[0]?.key]??`Row ${a+1}`):"";return g`<tr class="${u?"row-action":""}"
                tabindex=${u?0:y}
                role=${u?"button":y}
                aria-label=${u?c:y}
              ...
              ${l.map(d=>{let p=typeof d=="object"?d.align:void 0;return g`<td class="${p==="right"?"align-right":p==="center"?"align-center":""}">${this._renderCell(d,n)}</td>`})}</tr>`})}</tbody>
      </table>
    `}};customElements.define("forgeui-table",tt);var rt=class extends f{static get styles(){return $`
    :host { display:block; }
  ...
  `}render(){let t=this.getProp("data"),e=this.getString("dataPath","");!("data"in(this.props||{}))&&e&&this.store?.hasTable(e)&&(t=Object.values(this.store.getTable(e)));let o=this.getString("emptyMessage","No items");return!Array.isArray(t)||t.length===0?g`<div class="empty">${o}</div>`:g`<div class="list">${t.map((i,r)=>g`
      <div class="item" data-index=${r}><slot name="item" .item=${i} .index=${r}>${JSON.stringify(i)}</slot></div>
    `)}</div>`}};customElements.define("forgeui-list",rt);var ot=class extends f{constructor(){super(...arguments);this._palette=["var(--forgeui-color-primary)","var(--forgeui-color-success)","var(--forgeui-color-warning)","var(--forgeui-color-error)","var(--forgeui-color-info)","var(--forgeui-color-chart-6)","var(--forgeui-color-chart-7)","var(--forgeui-color-chart-8)","var(--forgeui-color-chart-9)","var(--forgeui-color-chart-10)"]}static get styles(){return $`
    :host { display:block; min-width:0; }
  ...
  `}_niceMax(e){if(e<=0)return 1;let o=Math.pow(10,Math.floor(Math.log10(e))),i=e/o;return(i<=1?1:i<=2?2:i<=5?5:10)*o}render(){let e=this.getString("chartType","bar"),o=this.getProp("data")||[],i=this.getString("title",""),r=this.getString("xKey","label")||this.getString("labelKey","label"),l=this.getString("yKey","value")||this.getString("valueKey","value"),n=this.getString("color","");if(!o||o.length===0)return g`
        ${i?g`<div class="title">${i}</div>`:y}
        <div class="empty">No data to display</div>
      `;let a=o.map(x=>typeof x=="number"?{label:"",value:x}:x&&typeof x=="object"?{label:String(x[r]??x.label??x.name??x.x??""),value:Number(x[l]??x.value??x.y??0)||0,color:x.color}:{label:String(x),value:0}),u=600,c=260,d={top:8,right:16,bottom:36,left:48},p=u-d.left-d.right,w=c-d.top-d.bottom,L,H=y;if(e==="pie"||e==="donut"){let x=a.reduce((M,A)=>M+Math.max(0,A.value),0)||1,D=u/2,R=c/2,P=Math.min(p,w)/2-8,E=e==="donut"?P*.55:0,k=-Math.PI/2,h=[],v=[];a.forEach((M,A)=>{let U=Math.max(0,M.value)/x,T=k,C=k+U*Math.PI*2;k=C;let G=C-T>Math.PI?1:0,te=D+P*Math.cos(T),pt=R+P*Math.sin(T),gt=D+P*Math.cos(C),ft=R+P*Math.sin(C),ie=M.color||this._palette[A%this._palette.length];if(v.push(ie),E>0){let Ut=D+E*Math.cos(T),Nt=R+E*Math.sin(T),Ft=D+E*Math.cos(C),Vt=R+E*Math.sin(C);h.push(_`<path class="slice" fill="${ie}" d="M ${te} ${pt} A ${P} ${P} 0 ${G} 1 ${gt} ${ft} L ${Ft} ${Vt} A ${E} ${E} 0 ${G} 0 ${Ut} ${Nt} Z"/>`)}else h.push(_`<path class="slice" fill="${ie}" d="M ${D} ${R} L ${te} ${pt} A ${P} ${P} 0 ${G} 1 ${gt} ${ft} Z"/>`)}),L=_`<g>${h}</g>`,H=g`<div class="legend">${a.map((M,A)=>g`
        <span class="legend-item"><span class="swatch" style="background:${v[A]}"></span>${M.label} (${M.value})</span>
      `)}</div>`}else{let x=Math.max(...a.map(k=>k.value),0),D=this._niceMax(x),R=k=>d.top+w-k/D*w,P=4,E=[];for(let k=0;k<=P;k++){let h=D*k/P,v=R(h);E.push(_`<line class="grid" x1="${d.left}" x2="${d.left+p}" y1="${v}" y2="${v}"/>`),E.push(_`<text class="tick-label" x="${d.left-6}" y="${v+3}" text-anchor="end">${h.toLocaleString()}</text>`)}if(e==="line"||e==="area"){let k=p/Math.max(1,a.length-1),h=a.map((A,U)=>{let T=d.left+U*k,C=R(A.value);return`${U===0?"M":"L"} ${T} ${C}`}).join(" "),v=e==="area"?h+` L ${d.left+p} ${d.top+w} L ${d.left} ${d.top+w} Z`:"",M=n||"var(--forgeui-color-primary)";L=g`
          <g>${E}</g>
          ${e==="area"?_`<path class="area" d="${v}" style="fill:${M};opacity:0.15"/>`:y}
          ${_`<path class="line" d="${h}" style="stroke:${M}"/>`}
          ${a.map((A,U)=>{let T=d.left+U*k,C=R(A.value);return _`<circle class="point" cx="${T}" cy="${C}" r="3" style="fill:${M}"/>
              <text class="tick-label" x="${T}" y="${d.top+w+14}" text-anchor="middle">${A.label}</text>`})}
        `}else{let k=a.length,h=p/k,v=Math.max(2,h*.7),M=h-v;L=g`
          <g>${E}</g>
          ${a.map((A,U)=>{let T=d.left+U*h+M/2,C=R(A.value),G=Math.max(0,d.top+w-C),te=A.color||n||"var(--forgeui-color-primary)";return _`<rect class="bar" x="${T}" y="${C}" width="${v}" height="${G}" rx="2" style="fill:${te}">
                <title>${A.label}: ${A.value}</title>
              </rect>
              <text class="tick-label" x="${T+v/2}" y="${d.top+w+14}" text-anchor="middle">${A.label}</text>`})}
        `}}return g`
      ${i?g`<div class="title">${i}</div>`:y}
      <div class="wrap">
    ...
    `}};customElements.define("forgeui-chart",ot);var it=class extends f{static get styles(){return $`
  ...
  `}render(){let t=this.getString("variant","info"),e=this.getString("title",""),o=this.getString("message","");return g`<div class="alert ${t}" role=${t==="error"||t==="warning"?"alert":"status"}>
      ${e?g`<strong>${e}</strong> `:y}${o}<slot></slot>
    </div>`}};customElements.define("forgeui-alert",st);var nt=class extends f{constructor(){super(...arguments);this._priorFocus=null;this._keydownHandler=e=>this._onKeydown(e);this._close=()=>{this.dispatchAction("close")}}static get styles(){return $`
    :host { display:none; }
  ...
  `}render(){let e=this.getString("title",""),o=this.getBool("open"),i=`${this._instanceId}-title`;return o?this.setAttribute("open",""):this.removeAttribute("open"),o?g`
  ...
  `}render(){let t=this.getProp("value"),e=this.getNumber("max",100),o=t==null,i=o?0:Math.max(0,Math.min(Number(t),e)),r=o?0:i/e*100,l=this.getString("label",""),n=this.getBool("showValue");return g`
      ${l||n?g`
        <div class="meta">
          ${l?g`<span>${l}</span>`:g`<span></span>`}
          ${n?g`<span class="value">${Math.round(r)}%</span>`:y}
        </div>
      `:y}
      <div
    ...
    `}};customElements.define("forgeui-progress",at);var lt=class extends f{static get styles(){return $`
  ...
  `}render(){let t=this.getString("message","");return t?g`<div class="toast">${t}</div>`:g`${y}`}};customElements.define("forgeui-toast",lt);var ct=class extends f{static get styles(){return $`
    :host { display:block; }
  ...
  `}render(){let t=this.getString("msg","Unknown error");return g`<div class="error" role="alert">⚠ ${t}</div>`}};customElements.define("forgeui-error",ct);var ut=class extends f{static get properties(){return{props:{type:Object}}}static get styles(){return $`
    :host { display:block; }
    svg { display:block; }
