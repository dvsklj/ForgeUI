...
`)}function ie(){return{type:"object",required:["manifest","id","elements"],properties:{manifest:{type:"string",const:"0.1.0"},id:{type:"string",pattern:"^[a-z0-9][a-z0-9-]*$"},root:{type:"string"},title:{type:"string"},schema:{type:"object",properties:{version:{type:"number"},tables:{type:"object",additionalProperties:{type:"object",properties:{columns:{type:"object",additionalProperties:{type:"object",properties:{type:{type:"string",enum:["string","number","boolean"]},default:{}},required:["type"]}}},required:["columns"]}}}},state:{type:"object",additionalProperties:{}},elements:{type:"object",additionalProperties:{type:"object",properties:{type:{type:"string",enum:Qe},props:{type:"object"},children:{type:"array",items:{type:"string"}},visible:{type:"object",properties:{$when:{type:"object",properties:{path:{type:"string"},eq:{},neq:{},gt:{type:"number"},lt:{type:"number"},gte:{type:"number"},lte:{type:"number"},in:{type:"array"},notIn:{type:"array"},exists:{type:"boolean"}},required:["path"]}}}},required:["type"]}},actions:{type:"object",additionalProperties:{type:"object",properties:{type:{type:"string",enum:["mutateState","setState","navigate","submitForm"]},path:{type:"string"},value:{},operation:{type:"string",enum:["append","update","delete"]},data:{type:"object"},key:{type:"string"},formId:{type:"string"},action:{type:"string"}},required:["type"]}},dataAccess:{type:"object",properties:{enabled:{type:"boolean",default:!1},readable:{type:"array",items:{type:"string"}},restricted:{type:"array",items:{type:"string"}},summaries:{type:"boolean",default:!1}}},persistState:{type:"boolean"},skipPersistState:{type:"boolean"}}}}var Ft={container:"Stack",row:"Stack",column:"Stack",text:"Text",heading:"Text",label:"Text",image:"Image",icon:"Icon",button:"Button",link:"Link",textInput:"TextInput",numberInput:"NumberInput",checkbox:"Checkbox",toggle:"Toggle",select:"Select",divider:"Divider",spacer:"Spacer",card:"Card",table:"Table",list:"List",alert:"Alert",progress:"Progress",badge:"Badge",avatar:"Avatar"};function qt(s){let e={},t=0;function o(a){let n=a.id||`a2ui-${t++}`,u=Ft[a.type];if(!u||!V(u))return e[n]={type:"Text",props:{content:`[Unsupported A2UI type: ${a.type}]`,variant:"caption",colorScheme:"secondary"}},n;let c=Bt(a.type,a.props||{}),d=(a.children||[]).map(f=>o(f));return e[n]={type:u,props:c,children:d.length>0?d:void 0},n}let r=(s.components||s.content||[]).map(a=>o(a)),l=r.length===1?r[0]:"a2ui-root";return r.length>1&&(e[l]={type:"Stack",props:{gap:"md"},children:r}),{manifest:"0.1.0",id:`a2ui-${Date.now()}`,root:l,elements:e,actions:{}}}function Bt(s,e){let t={...e};switch(s){case"heading":t.variant="heading",t.content=e.text||e.content||"",delete t.text;break;case"text":case"label":t.content=e.text||e.content||"",delete t.text;break;case"row":t.direction="row",t.gap=e.gap||e.spacing||"md";break;case"column":t.direction="column",t.gap=e.gap||e.spacing||"md";break;case"container":t.gap=e.gap||e.spacing||"md";break;case"image":t.src=e.src||e.url||"",t.alt=e.alt||"";break;case"button":t.label=e.text||e.label||"",t.variant=e.variant||e.style||"default";break;case"textInput":t.placeholder=e.placeholder||"",t.value=e.value||"",t.label=e.label||"";break;case"numberInput":t.placeholder=e.placeholder||"",t.value=e.value??0,t.label=e.label||"";break;case"select":e.options&&Array.isArray(e.options)&&(t.options=e.options.map(o=>typeof o=="string"?{value:o,label:o}:o));break;case"alert":t.variant=e.type||e.severity||"info",t.content=e.message||e.text||e.content||"",delete t.message,delete t.text;break;case"progress":t.value=e.value??e.percent??0;break}return t}function ft(s){if(!s||typeof s!="object")return!1;let e=s;return!!(Array.isArray(e.components)||Array.isArray(e.content)||typeof e.type=="string"&&(e.type==="adaptive-card"||e.type.startsWith("a2ui"))||typeof e.version=="string"&&e.version.startsWith("a2ui")||!e.elements&&!e.manifest&&Array.isArray(e.content))}function se(s){return ft(s)?qt(s):s}function Wt(s){return`forgeui_${(s||"global").replace(/[^a-zA-Z0-9-]/g,"_")}`}function gt(s,e,t="none"){let o=Wt(e),i=t,r=null,l=!1,a=!1,n=null,u=null,c=null,d=[];function f(){let b=x();for(let v of d)try{v(b)}catch{}}function x(){return{mode:i,isPersisting:r!=null&&l,lastSaved:n,lastLoaded:u,error:c,dbName:i==="indexeddb"?o:null}}async function R(){if(r)return r;try{let{createIndexedDbPersister:b}=await import("tinybase/persisters/persister-indexed-db");return r=b(s,o),c=null,r}catch(b){throw c=`IndexedDB unavailable: ${b.message}`,f(),b}}async function N(){if(i==="none")return;let b=await R();try{await b.load(),u=Date.now()}catch(v){c=`Load failed: ${v.message}`,f()}try{b.startAutoSave(),l=!0}catch(v){c=`Auto-save failed: ${v.message}`}try{b.startAutoLoad(1),a=!0}catch(v){c=`Auto-load failed: ${v.message}`}f()}async function w(){if(r){l&&(r.stopAutoSave(),l=!1),a&&(r.stopAutoLoad(),a=!1);try{await r.save(),n=Date.now()}catch(b){c=`Final save failed: ${b.message}`}f()}}async function I(){if(!(i==="none"||!r)){try{await r.save(),n=Date.now(),c=null}catch(b){c=`Save failed: ${b.message}`}f()}}async function j(){if(!(i==="none"||!r)){try{await r.load(),u=Date.now(),c=null}catch(b){c=`Load failed: ${b.message}`}f()}}async function A(){if(await w(),r){try{r.destroy()}catch{}r=null}d=[],f()}function _(b){if(b===i)return;let v=i;i=b,v==="indexeddb"&&b==="none"&&(r&&l&&(r.stopAutoSave(),l=!1),r&&a&&(r.stopAutoLoad(),a=!1)),b==="indexeddb"&&v==="none"&&N().catch(()=>{}),f()}function $(b){return d.push(b),()=>{d=d.filter(v=>v!==b)}}return{start:N,stop:w,save:I,load:j,setMode:_,getStatus:x,addListener:$,destroy:A}}var W=class{constructor(e=50){this._stack=[];this._position=-1;this._listeners=[];this._maxDepth=e}push(e){if(this._position<this._stack.length-1&&(this._stack=this._stack.slice(0,this._position+1)),this._position>=0){let t=this._stack[this._position];if(JSON.stringify(t)===JSON.stringify(e))return}this._stack.push(structuredClone(e)),this._stack.length>this._maxDepth?this._stack.shift():this._position++,this._notifyListeners()}undo(){return this._position<=0?null:(this._position--,this._notifyListeners(),structuredClone(this._stack[this._position]))}redo(){return this._position>=this._stack.length-1?null:(this._position++,this._notifyListeners(),structuredClone(this._stack[this._position]))}current(){return this._position<0?null:structuredClone(this._stack[this._position])}jumpTo(e){return e<0||e>=this._stack.length?null:(this._position=e,this._notifyListeners(),structuredClone(this._stack[this._position]))}getState(){return{canUndo:this._position>0,canRedo:this._position<this._stack.length-1,position:this._position,total:this._stack.length}}getTimeline(){return this._stack.map((e,t)=>({position:t,id:e.id}))}clear(){this._stack=[],this._position=-1,this._notifyListeners()}addListener(e){return this._listeners.push(e),()=>{this._listeners=this._listeners.filter(t=>t!==e)}}_notifyListeners(){let e=this.getState();for(let t of this._listeners)try{t(e)}catch{}}};import{html as p,css as y,svg as k,nothing as h}from"lit";import{LitElement as Ht}from"lit";var H=class H extends Ht{constructor(){super(...arguments);this._instanceId=`forge-${++H._instanceCounter}`;this.props={};this.store=null;this.onAction=null;this.itemContext=null}static get properties(){return{props:{type:Object}}}connectedCallback(){super.connectedCallback()}resolve(t){if(!this.store)return t;this.itemContext&&L(this.itemContext);try{return M(this.store,t)}finally{L(null)}}getProp(t){let o=this.props?.[t];return typeof o=="string"&&(o.startsWith("$state:")||o.startsWith("$computed:")||o.startsWith("$item:")||o.startsWith("$expr:")||o.includes("{{")&&o.includes("}}"))?this.resolve(o):o}getArray(t){let o=this.getProp(t);return Array.isArray(o)?o:o&&typeof o=="object"?Object.values(o):[]}getString(t,o=""){let i=this.getProp(t);return typeof i=="string"?i:String(i??o)}getNumber(t,o=0){let i=this.getProp(t);return typeof i=="number"?i:Number(i)||o}getBool(t,o=!1){let i=this.getProp(t);return typeof i=="boolean"?i:o}dispatchAction(t,o){this.onAction&&this.onAction(t,o),this.dispatchEvent(new CustomEvent("forgeui-action",{detail:{action:t,payload:o},bubbles:!0,composed:!0}))}handleAction(t){let o=this.getString("action");o&&this.dispatchAction(o,this.props)}prop(t){return this.getProp(t)}static get sharedStyles(){return[re]}gapValue(t){let o={none:"0",0:"0","3xs":"var(--forgeui-space-3xs)","2xs":"var(--forgeui-space-2xs)",xs:"var(--forgeui-space-xs)",sm:"var(--forgeui-space-sm)",md:"var(--forgeui-space-md)",lg:"var(--forgeui-space-lg)",xl:"var(--forgeui-space-xl)","2xl":"var(--forgeui-space-2xl)"};if(t==null||t==="")return"var(--forgeui-space-md)";let i=String(t);return i in o?o[i]:/^\d+(\.\d+)?$/.test(i)?`${i}px`:/^\d+(\.\d+)?(px|rem|em|%|vw|vh|ch)$/.test(i)?i:"var(--forgeui-space-md)"}static get styles(){return[re]}};H._instanceCounter=0;var m=H;var ae=class extends m{static get properties(){return{props:{type:Object}}}static get styles(){return y`
  ...
  `}render(){let e=this.getProp("columns"),t;typeof e=="number"?t=String(e):typeof e=="string"&&e?t=e:t="1";let o=/^\d+$/.test(t)?`repeat(${t}, minmax(0, 1fr))`:t,i=this.getString("gap","md"),r=this.gapValue(i),l=this.getString("padding",""),a=l?this.gapValue(l):"0";return this.style.gridTemplateColumns=o,this.style.gap=r,this.style.padding=a,/^\d+$/.test(t)&&Number(t)>=2&&this.setAttribute("responsive",""),p`<slot></slot>`}};customElements.define("forgeui-grid",ne);var le=class extends m{static get properties(){return{props:{type:Object}}}static get styles(){return y`
  ...
  `}render(){let e=this.getString("title","Section");return p`<details><summary>${e}</summary><div class="content"><slot></slot></div></details>`}};customElements.define("forgeui-accordion",de);var pe=class extends m{static get styles(){return y`
    :host { display:block; }
    hr { border:none; border-top:1px solid var(--forgeui-color-border); margin:var(--forgeui-space-sm) 0; }
  `}render(){return p`<hr>`}};customElements.define("forgeui-divider",pe);var fe=class extends m{static get styles(){return y`:host { display:block; }`}render(){let e=this.getString("size","md"),t=this.getString("height",""),o=this.getString("width",""),i=t?this.gapValue(t):this.gapValue(e),r=o?/^\d+(\.\d+)?%$/.test(o)?o:this.gapValue(o):"";return p`<div style="height:${i};${r?`width:${r}`:""}"></div>`}};customElements.define("forgeui-spacer",fe);var ge=class extends m{static get properties(){return{props:{type:Object}}}static get styles(){return y`
  ...
  `}render(){let e=this.getArray("data"),t=this.getString("emptyMessage",""),o=this.getString("direction","column");(o==="row"||o==="horizontal")&&this.setAttribute("direction","row");let i=this.getString("gap","md");return this.style.gap=this.gapValue(i),e.length===0&&t?p`<div class="empty">${t}</div>`:p`<slot></slot>`}};customElements.define("forgeui-repeater",ge);var me=class extends m{static get properties(){return{props:{type:Object}}}static get styles(){return y`
  ...
  `}render(){let e=this.getString("content",""),t=this.getString("variant","body"),i={h1:"heading1",h2:"heading2",h3:"heading3",title:"heading2",subtitle:"subheading",paragraph:"body",text:"body",secondary:"muted",tertiary:"caption"}[t]||t,r=this.getString("colorScheme",""),l=this.getString("align",""),a=this.getString("weight",""),n={primary:"var(--forgeui-color-primary)",secondary:"var(--forgeui-color-text-secondary)",tertiary:"var(--forgeui-color-text-tertiary)",success:"var(--forgeui-color-success)",warning:"var(--forgeui-color-warning)",error:"var(--forgeui-color-error)",info:"var(--forgeui-color-info)"},u={normal:"var(--forgeui-weight-normal)",medium:"var(--forgeui-weight-medium)",semibold:"var(--forgeui-weight-semibold)",bold:"var(--forgeui-weight-bold)"},c=[];r&&n[r]&&c.push(`color:${n[r]}`),a&&u[a]&&c.push(`font-weight:${u[a]}`);let d=l?`align-${l}`:"",f=p`${e}<slot></slot>`;return i==="heading1"?p`<h1 class="${i} ${d}" style="${c.join(";")}">${f}</h1>`:i==="heading2"?p`<h2 class="${i} ${d}" style="${c.join(";")}">${f}</h2>`:i==="heading3"?p`<h3 class="${i} ${d}" style="${c.join(";")}">${f}</h3>`:p`<div class="${i} ${d}" style="${c.join(";")}">${e}<slot></slot></div>`}};customElements.define("forgeui-text",me);var he=class extends m{static get styles(){return y`
    :host { display:block; }
    img { max-width:100%; height:auto; display:block; border-radius:var(--forgeui-radius-md); }
  `}render(){let e=this.getString("src",""),t=this.getString("alt",""),o=this.getString("fit","contain");return e?p`<img src="${e}" alt="${t}" style="object-fit:${o}" loading="lazy">`:p`${h}`}};customElements.define("forgeui-image",he);var ye=class extends m{static get styles(){return y`
  ...
  `}render(){let e=this.getString("text","")||this.getString("label",""),t=this.getString("variant","");return p`<span class="badge" variant="${t}">${e}<slot></slot></span>`}};customElements.define("forgeui-badge",be);var ve=class extends m{static get styles(){return y`
  ...
  `}render(){let e=this.getString("src",""),t=this.getString("name","?"),o=t.split(" ").map(i=>i[0]).join("").toUpperCase().slice(0,2);return p`<div class="avatar">${e?p`<img src="${e}" alt="${t}">`:o}<slot></slot></div>`}};customElements.define("forgeui-avatar",ve);var xe=class extends m{static get styles(){return y`
  ...
  `}render(){let e=this.getString("title","Nothing here"),t=this.getString("description","");return p`
      ...
      <slot></slot>
    `}};customElements.define("forgeui-empty-state",xe);var we=class extends m{static get styles(){return y`
  ...
  `}render(){let e=this.getString("label",""),t=this.getString("placeholder",""),o=this.getString("hint",""),i=this.getString("error",""),r=this.getString("inputType","")||this.getString("type","text"),l=this.getBool("multiline"),a=this.getString("value",""),n=this._instanceId;return p`
      ${e?p`<label for="${n}">${e}</label>`:h}
      ${l?p`<textarea id="${n}" placeholder="${t}" .value=${a} @input=${u=>this.dispatchAction("change",{value:u.target.value})}></textarea>`:p`<input id="${n}" type="${r}" placeholder="${t}" .value=${a} @input=${u=>this.dispatchAction("change",{value:u.target.value})}>`}
      ${o&&!i?p`<div class="hint">${o}</div>`:h}
      ${i?p`<div class="error">${i}</div>`:h}
    `}};customElements.define("forgeui-text-input",we);var $e=class extends m{static get styles(){return y`
    :host { display:block; flex:1 1 auto; min-width:0; max-width:100%; margin-bottom:var(--forgeui-space-sm); }
  ...
  `}render(){let e=this.getString("label",""),t=this.getProp("min"),o=this.getProp("max"),i=this.getProp("step"),r=this.getProp("value"),l=this._instanceId;return p`
    ...
    `}};customElements.define("forgeui-number-input",$e);var ke=class extends m{static get styles(){return y`
    :host { display:block; flex:1 1 auto; min-width:0; max-width:100%; margin-bottom:var(--forgeui-space-sm); }
    label { display:block; font-size:var(--forgeui-text-sm); font-weight:var(--forgeui-weight-medium); margin-bottom:var(--forgeui-space-2xs); overflow-wrap:break-word; }
  ...
  `}render(){let e=this.getString("label",""),t=this.getProp("options")||[],o=this.getString("value",""),i=this._instanceId;return p`
  ...
  `}render(){let t=this.getBool("on"),o=this.getString("label",""),i=this.getBool("disabled"),r=this._instanceId;return p`
  ...
  `}render(){let e=this.getString("label",""),t=this.getNumber("min",0),o=this.getNumber("max",100),i=this.getNumber("step",1),r=this.getNumber("value",t);return p`
      ${e?p`<label>${e}</label>`:h}
  ...
  `}render(){let e=this.getString("label","Button"),t=this.getString("variant","primary"),o=this.getString("size",""),i=this.getBool("disabled"),r=this.getProp("pressed");return p`<button class="${t} ${o}" ?disabled=${i}
      aria-pressed=${r==null?h:String(!!r)}
      @click=${l=>this.handleAction(l)}>${e}<slot></slot></button>`}};customElements.define("forgeui-button",je);var Ie=class extends m{static get styles(){return y`
    :host { display:flex; gap:var(--forgeui-space-xs); }
  `}render(){return p`<slot></slot>`}};customElements.define("forgeui-button-group",Ie);var Me=class extends m{static get styles(){return y`
    :host { display:inline-flex; }
  ...
  `}render(){let e=this.getString("label",""),t=this.getString("href","#");return p`<a href="${t}">${e}<slot></slot></a>`}};customElements.define("forgeui-link",Me);var Te=class extends m{static get styles(){return y`
  ...
  `}_statusClass(e){let t=String(e??"").toLowerCase().trim();return["done","complete","completed","success","active","ok","approved","paid"].includes(t)?"success":["in progress","in-progress","pending","warning","waiting","review"].includes(t)?"warning":["to do","to-do","todo","backlog","draft","new","inactive"].includes(t)?"neutral":["high","urgent","critical"].includes(t)?"error":["medium","med"].includes(t)?"warning":["low"].includes(t)?"info":["failed","error","rejected","blocked","overdue"].includes(t)?"error":"neutral"}_renderCell(e,t){let o=typeof e=="string"?e:e.key,i=t[o],r=e&&typeof e=="object"?e.type:void 0;if(i==null||i==="")return p`<span style="color:var(--forgeui-color-text-tertiary)">—</span>`;if(r==="badge"||r==="status"){let l=(e.variant&&typeof e.variant=="object"?e.variant[String(i).toLowerCase()]:null)||this._statusClass(i);return p`<span class="badge ${l}">${String(i)}</span>`}if(r==="number")return typeof i=="number"?i.toLocaleString():String(i);if(r==="date"){let l=typeof i=="string"||typeof i=="number"?new Date(i):i;return l instanceof Date&&!isNaN(l.getTime())?l.toLocaleDateString():String(i)}if(r==="currency"){let l=Number(i);return isNaN(l)?String(i):l.toLocaleString(void 0,{style:"currency",currency:e.currency||"USD"})}return r==="boolean"?i?"\u2713":"\u2717":String(i)}render(){let e=this.getProp("data"),t=this.getProp("columns")||[],o=this.getString("emptyMessage","No data yet"),i=this.getString("rowAction",""),r=this.getString("caption","");if(!Array.isArray(e))return p`<div class="empty">${o}</div>`;let l=t.length>0?t:e.length>0?Object.keys(e[0]):[];return l.length===0?p`<div class="empty">${o}</div>`:p`
      <table>
        ${r?p`<caption>${r}</caption>`:h}
        <thead><tr>${l.map(a=>{let n=typeof a=="string"?a:a.label||a.key,u=typeof a=="object"?a.align:void 0,c=typeof a=="object"?a.width:void 0;return p`<th class="${u==="right"?"align-right":u==="center"?"align-center":""}" style="${c?`width:${c}`:""}">${n}</th>`})}</tr></thead>
        <tbody>${e.length===0?p`<tr><td colspan=${l.length} class="empty">${o}</td></tr>`:e.map((a,n)=>{let u=!!i,c=u?String(a[typeof l[0]=="string"?l[0]:l[0]?.key]??`Row ${n+1}`):"";return p`<tr class="${u?"row-action":""}"
                tabindex=${u?0:h}
                role=${u?"button":h}
                aria-label=${u?c:h}
                @click=${u?()=>this.dispatchAction(i,{row:a,index:n}):void 0}
                @keydown=${u?d=>{(d.key==="Enter"||d.key===" ")&&(d.preventDefault(),this.dispatchAction(i,{row:a,index:n}))}:void 0}>
              ${l.map(d=>{let f=typeof d=="object"?d.align:void 0;return p`<td class="${f==="right"?"align-right":f==="center"?"align-center":""}">${this._renderCell(d,a)}</td>`})}</tr>`})}</tbody>
      </table>
  ...
  `}render(){let e=this.getProp("data"),t=this.getString("emptyMessage","No items");return!Array.isArray(e)||e.length===0?p`<div class="empty">${t}</div>`:p`<div class="list">${e.map((o,i)=>p`
      <div class="item" data-index=${i}><slot name="item" .item=${o} .index=${i}>${JSON.stringify(o)}</slot></div>
    `)}</div>`}};customElements.define("forgeui-list",Re);var ze=class extends m{constructor(){super(...arguments);this._palette=["var(--forgeui-color-primary)","var(--forgeui-color-success)","var(--forgeui-color-warning)","var(--forgeui-color-error)","var(--forgeui-color-info)","var(--forgeui-color-chart-6)","var(--forgeui-color-chart-7)","var(--forgeui-color-chart-8)","var(--forgeui-color-chart-9)","var(--forgeui-color-chart-10)"]}static get styles(){return y`
    :host { display:block; min-width:0; }
  ...
  `}_niceMax(t){if(t<=0)return 1;let o=Math.pow(10,Math.floor(Math.log10(t))),i=t/o;return(i<=1?1:i<=2?2:i<=5?5:10)*o}render(){let t=this.getString("chartType","bar"),o=this.getProp("data")||[],i=this.getString("title",""),r=this.getString("xKey","label")||this.getString("labelKey","label"),l=this.getString("yKey","value")||this.getString("valueKey","value"),a=this.getString("color","");if(!o||o.length===0)return p`
      ...
      `;let n=o.map(w=>typeof w=="number"?{label:"",value:w}:w&&typeof w=="object"?{label:String(w[r]??w.label??w.name??w.x??""),value:Number(w[l]??w.value??w.y??0)||0,color:w.color}:{label:String(w),value:0}),u=600,c=260,d={top:8,right:16,bottom:36,left:48},f=u-d.left-d.right,x=c-d.top-d.bottom,R,N=h;if(t==="pie"||t==="donut"){let w=n.reduce((P,S)=>P+Math.max(0,S.value),0)||1,I=u/2,j=c/2,A=Math.min(f,x)/2-8,_=t==="donut"?A*.55:0,$=-Math.PI/2,b=[],v=[];n.forEach((P,S)=>{let T=Math.max(0,P.value)/w,C=$,E=$+T*Math.PI*2;$=E;let O=E-C>Math.PI?1:0,q=I+A*Math.cos(C),He=j+A*Math.sin(C),Ke=I+A*Math.cos(E),Ge=j+A*Math.sin(E),K=P.color||this._palette[S%this._palette.length];if(v.push(K),_>0){let mt=I+_*Math.cos(C),ht=j+_*Math.sin(C),yt=I+_*Math.cos(E),bt=j+_*Math.sin(E);b.push(k`<path class="slice" fill="${K}" d="M ${q} ${He} A ${A} ${A} 0 ${O} 1 ${Ke} ${Ge} L ${yt} ${bt} A ${_} ${_} 0 ${O} 0 ${mt} ${ht} Z"/>`)}else b.push(k`<path class="slice" fill="${K}" d="M ${I} ${j} L ${q} ${He} A ${A} ${A} 0 ${O} 1 ${Ke} ${Ge} Z"/>`)}),R=k`<g>${b}</g>`,N=p`<div class="legend">${n.map((P,S)=>p`
        <span class="legend-item"><span class="swatch" style="background:${v[S]}"></span>${P.label} (${P.value})</span>
      `)}</div>`}else{let w=Math.max(...n.map($=>$.value),0),I=this._niceMax(w),j=$=>d.top+x-$/I*x,A=4,_=[];for(let $=0;$<=A;$++){let b=I*$/A,v=j(b);_.push(k`<line class="grid" x1="${d.left}" x2="${d.left+f}" y1="${v}" y2="${v}"/>`),_.push(k`<text class="tick-label" x="${d.left-6}" y="${v+3}" text-anchor="end">${b.toLocaleString()}</text>`)}if(t==="line"||t==="area"){let $=f/Math.max(1,n.length-1),b=n.map((S,T)=>{let C=d.left+T*$,E=j(S.value);return`${T===0?"M":"L"} ${C} ${E}`}).join(" "),v=t==="area"?b+` L ${d.left+f} ${d.top+x} L ${d.left} ${d.top+x} Z`:"",P=a||"var(--forgeui-color-primary)";R=p`
          <g>${_}</g>
          ${t==="area"?k`<path class="area" d="${v}" style="fill:${P};opacity:0.15"/>`:h}
          ${k`<path class="line" d="${b}" style="stroke:${P}"/>`}
          ${n.map((S,T)=>{let C=d.left+T*$,E=j(S.value);return k`<circle class="point" cx="${C}" cy="${E}" r="3" style="fill:${P}"/>
              <text class="tick-label" x="${C}" y="${d.top+x+14}" text-anchor="middle">${S.label}</text>`})}
        `}else{let $=n.length,b=f/$,v=Math.max(2,b*.7),P=b-v;R=p`
          <g>${_}</g>
          ${n.map((S,T)=>{let C=d.left+T*b+P/2,E=j(S.value),O=Math.max(0,d.top+x-E),q=S.color||a||"var(--forgeui-color-primary)";return k`<rect class="bar" x="${C}" y="${E}" width="${v}" height="${O}" rx="2" style="fill:${q}">
                <title>${S.label}: ${S.value}</title>
              </rect>
              <text class="tick-label" x="${C+v/2}" y="${d.top+x+14}" text-anchor="middle">${S.label}</text>`})}
        `}}return p`
      ${i?p`<div class="title">${i}</div>`:h}
      <div class="wrap">
    ...
    `}};customElements.define("forgeui-chart",ze);var Le=class extends m{static get styles(){return y`
  ...
  `}render(){let t=this.getString("title",""),o=this.getBool("open"),i=`${this._instanceId}-title`;return o?this.setAttribute("open",""):this.removeAttribute("open"),o?p`
      ...
      >
        ${t?p`<h2 id="${i}" class="title">${t}</h2>`:h}
        <slot></slot>
      </div>
  ...
  `}render(){let e=this.getProp("value"),t=this.getNumber("max",100),o=e==null,i=o?0:Math.max(0,Math.min(Number(e),t)),r=o?0:i/t*100,l=this.getString("label",""),a=this.getBool("showValue");return p`
      ${l||a?p`
        <div class="meta">
          ${l?p`<span>${l}</span>`:p`<span></span>`}
          ${a?p`<span class="value">${Math.round(r)}%</span>`:h}
        </div>
      `:h}
      <div
  ...
  `}render(){let e=this.getString("message","");return e?p`<div class="toast">${e}</div>`:p`${h}`}};customElements.define("forgeui-toast",De);var Ve=class extends m{static get styles(){return y`
  ...
  `}render(){let e=this.getProp("items")||[];return p`${e.map((t,o)=>{let i=o===e.length-1,r=typeof t=="string"?t:t.label,l=typeof t=="string"?"#":t.href;return p`
        ${o>0?p`<span class="sep">/</span>`:h}
        ${i?p`<span class="current">${r}</span>`:p`<a href="${l}">${r}</a>`}
      `})}`}};customElements.define("forgeui-breadcrumb",Ve);var Fe=class extends m{static get styles(){return y`
  ...
  `}render(){let e=this.getProp("steps")||[],t=this.getNumber("active",0);return p`${e.map((o,i)=>{let r=typeof o=="string"?o:o.label||o.title||`Step ${i+1}`,l=i===t,a=i<t;return p`<div class="step" ?active=${l} ?completed=${a}>
        <div class="circle">${a?"\u2713":i+1}</div>
        <div class="label">${r}</div>
      </div>`})}`}};customElements.define("forgeui-stepper",Fe);var qe=class extends m{static get styles(){return y`
    :host { display:block; }
  ...
  `}render(){let e=this.getString("msg","Unknown error");return p`<div class="error" role="alert">⚠ ${e}</div>`}};customElements.define("forgeui-error",qe);var Be=class extends m{static get properties(){return{props:{type:Object}}}static get styles(){return y`
    :host { display:block; }
    svg { display:block; }
  `}render(){let e=this.getNumber("width",400),t=this.getNumber("height",300),o=this.getString("background","transparent"),i=this.getProp("shapes")||[];return k`
      <svg width="${e}" height="${t}" style="background:${o}" viewBox="0 0 ${e} ${t}">
        ${i.map(r=>this.renderShape(r))}
      </svg>
    `}renderShape(e){let t={fill:e.fill??void 0,stroke:e.stroke??void 0,"stroke-width":e.strokeWidth??void 0,opacity:e.opacity??void 0},o=e.action?()=>{this.onAction&&this.onAction(e.action)}:void 0,i=e.action?"cursor:pointer":void 0;switch(e.type){case"rect":return k`<rect
          x="${e.x??0}" y="${e.y??0}"
        ...
        />`;case"text":return k`<text
          ...
          style="${i}"
          @click=${o}
        >${e.content??""}</text>`;case"path":return k`<path
          d="${e.d??""}"
          fill="${t.fill??"none"}"
          stroke="${t.stroke??"none"}"
          stroke-width="${t["stroke-width"]??1}"
