...
`;function Ae(s="default"){switch(s){case"minimal":return zt();case"default":return Ot();case"full":return Pr()}}function zt(){let t=Object.entries(_t).map(([e,o])=>`  ${e}: ${o.join(", ")}`).join(`
...
`)}function Pe(){return{type:"object",required:["manifest","id","elements"],properties:{manifest:{type:"string",const:"0.1.0"},id:{type:"string",pattern:"^[a-z0-9][a-z0-9-]*$"},root:{type:"string"},title:{type:"string"},schema:{type:"object",properties:{version:{type:"number"},tables:{type:"object",additionalProperties:{type:"object",properties:{columns:{type:"object",additionalProperties:{type:"object",properties:{type:{type:"string",enum:["string","number","boolean"]},default:{}},required:["type"]}}},required:["columns"]}}}},state:{type:"object",additionalProperties:{}},elements:{type:"object",additionalProperties:{type:"object",properties:{type:{type:"string",enum:be},props:{type:"object"},children:{type:"array",items:{type:"string"}},visible:{type:"object",properties:{$when:{type:"object",properties:{path:{type:"string"},eq:{},neq:{},gt:{type:"number"},lt:{type:"number"},gte:{type:"number"},lte:{type:"number"},in:{type:"array"},notIn:{type:"array"},exists:{type:"boolean"}},required:["path"]}}}},required:["type"]}},actions:{type:"object",additionalProperties:{type:"object",properties:{type:{type:"string",enum:["mutateState","navigate","openDialog","closeDialog","callApi","toast","custom"]},path:{type:"string"},value:{},operation:{type:"string",enum:["set","append","update","delete","increment","decrement","toggle"]},set:{type:"object"},data:{type:"object"},key:{type:"string"},formId:{type:"string"},action:{type:"string"},target:{type:"string"},url:{type:"string"},method:{type:"string"},body:{type:"object"}},required:["type"]}},dataAccess:{type:"object",properties:{enabled:{type:"boolean",default:!1},readable:{type:"array",items:{type:"string"}},restricted:{type:"array",items:{type:"string"}},summaries:{type:"boolean",default:!1}}},persistState:{type:"boolean"},skipPersistState:{type:"boolean"}}}}var Er={container:"Stack",row:"Stack",column:"Stack",text:"Text",heading:"Text",label:"Text",image:"Image",icon:"Icon",button:"Button",link:"Link",textInput:"TextInput",numberInput:"NumberInput",checkbox:"Checkbox",toggle:"Toggle",select:"Select",divider:"Divider",spacer:"Spacer",card:"Card",table:"Table",list:"List",alert:"Alert",progress:"Progress",badge:"Badge",avatar:"Avatar"};function Mr(s){let t={},e=0;function o(n){let a=n.id||`a2ui-${e++}`,u=Er[n.type];if(!u||!oe(u))return t[a]={type:"Text",props:{content:`[Unsupported A2UI type: ${n.type}]`,variant:"caption",colorScheme:"secondary"}},a;let c=Ir(n.type,n.props||{}),d=(n.children||[]).map(p=>o(p));return t[a]={type:u,props:c,children:d.length>0?d:void 0},a}let r=(s.components||s.content||[]).map(n=>o(n)),l=r.length===1?r[0]:"a2ui-root";return r.length>1&&(t[l]={type:"Stack",props:{gap:"md"},children:r}),{manifest:"0.1.0",id:`a2ui-${Date.now()}`,root:l,elements:t,actions:{}}}function Ir(s,t){let e={...t};switch(s){case"heading":e.variant="heading",e.content=t.text||t.content||"",delete e.text;break;case"text":case"label":e.content=t.text||t.content||"",delete e.text;break;case"row":e.direction="row",e.gap=t.gap||t.spacing||"md";break;case"column":e.direction="column",e.gap=t.gap||t.spacing||"md";break;case"container":e.gap=t.gap||t.spacing||"md";break;case"image":e.src=t.src||t.url||"",e.alt=t.alt||"";break;case"button":e.label=t.text||t.label||"",e.variant=t.variant||t.style||"default";break;case"textInput":e.placeholder=t.placeholder||"",e.value=t.value||"",e.label=t.label||"";break;case"numberInput":e.placeholder=t.placeholder||"",e.value=t.value??0,e.label=t.label||"";break;case"select":t.options&&Array.isArray(t.options)&&(e.options=t.options.map(o=>typeof o=="string"?{value:o,label:o}:o));break;case"alert":e.variant=t.type||t.severity||"info",e.content=t.message||t.text||t.content||"",delete e.message,delete e.text;break;case"progress":e.value=t.value??t.percent??0;break}return e}function Ut(s){if(!s||typeof s!="object")return!1;let t=s;return!!(Array.isArray(t.components)||Array.isArray(t.content)||typeof t.type=="string"&&(t.type==="adaptive-card"||t.type.startsWith("a2ui"))||typeof t.version=="string"&&t.version.startsWith("a2ui")||!t.elements&&!t.manifest&&Array.isArray(t.content))}function Ee(s){return Ut(s)?Mr(s):s}function Nt(s){return`forgeui_${(s||"global").replace(/[^a-zA-Z0-9-]/g,"_")}`}var Me="f";function Tr(s){return new Promise((t,e)=>{let o=r=>e(r??new Error("IDB"));if(!globalThis.indexedDB)return o();let i=indexedDB.open(`${Nt(s)}_f`,1);i.onupgradeneeded=()=>{i.result.createObjectStore(Me)},i.onsuccess=()=>t(i.result),i.onerror=i.onblocked=()=>o(i.error)})}async function Ft(s,t){if(s.length===0)return;let e=null;try{e=await Tr(t),await new Promise((o,i)=>{let r=e.transaction(Me,"readwrite"),l=r.objectStore(Me);for(let{file:n,id:a}of s)l.put(n,a);r.oncomplete=()=>o(),r.onerror=r.onabort=()=>i(r.error)})}catch{return}finally{e?.close()}}function Vt(s,t,e="none"){let o=Nt(t),i=e,r=null,l=!1,n=!1,a=null,u=null,c=null,d=[];function p(){let m=y();for(let x of d)try{x(m)}catch{}}function y(){return{mode:i,isPersisting:r!=null&&l,lastSaved:a,lastLoaded:u,error:c,dbName:i==="indexeddb"?o:null}}async function L(){if(r)return r;try{let{createIndexedDbPersister:m}=await import("tinybase/persisters/persister-indexed-db");return r=m(s,o),c=null,r}catch(m){throw c=`IndexedDB unavailable: ${m.message}`,p(),m}}async function _(){if(i==="none")return;let m=await L();try{await m.load(),u=Date.now()}catch(x){c=`Load failed: ${x.message}`,p()}try{m.startAutoSave(),l=!0}catch(x){c=`Auto-save failed: ${x.message}`}try{m.startAutoLoad(1),n=!0}catch(x){c=`Auto-load failed: ${x.message}`}p()}async function v(){if(r){l&&(r.stopAutoSave(),l=!1),n&&(r.stopAutoLoad(),n=!1);try{await r.save(),a=Date.now()}catch(m){c=`Final save failed: ${m.message}`}p()}}async function M(){if(!(i==="none"||!r)){try{await r.save(),a=Date.now(),c=null}catch(m){c=`Save failed: ${m.message}`}p()}}async function k(){if(!(i==="none"||!r)){try{await r.load(),u=Date.now(),c=null}catch(m){c=`Load failed: ${m.message}`}p()}}async function P(){if(await v(),r){try{r.destroy()}catch{}r=null}d=[],p()}function $(m){if(m===i)return;let x=i;i=m,x==="indexeddb"&&m==="none"&&(r&&l&&(r.stopAutoSave(),l=!1),r&&n&&(r.stopAutoLoad(),n=!1)),m==="indexeddb"&&x==="none"&&_().catch(()=>{}),p()}function S(m){return d.push(m),()=>{d=d.filter(x=>x!==m)}}return{start:_,stop:v,save:M,load:k,setMode:$,getStatus:y,addListener:S,destroy:P}}var le=class{constructor(t=50){this._stack=[];this._position=-1;this._listeners=[];this._maxDepth=t}push(t){if(this._position<this._stack.length-1&&(this._stack=this._stack.slice(0,this._position+1)),this._position>=0){let e=this._stack[this._position];if(JSON.stringify(e)===JSON.stringify(t))return}this._stack.push(structuredClone(t)),this._stack.length>this._maxDepth?this._stack.shift():this._position++,this._notifyListeners()}undo(){return this._position<=0?null:(this._position--,this._notifyListeners(),structuredClone(this._stack[this._position]))}redo(){return this._position>=this._stack.length-1?null:(this._position++,this._notifyListeners(),structuredClone(this._stack[this._position]))}current(){return this._position<0?null:structuredClone(this._stack[this._position])}jumpTo(t){return t<0||t>=this._stack.length?null:(this._position=t,this._notifyListeners(),structuredClone(this._stack[this._position]))}getState(){return{canUndo:this._position>0,canRedo:this._position<this._stack.length-1,position:this._position,total:this._stack.length}}getTimeline(){return this._stack.map((t,e)=>({position:e,id:t.id}))}clear(){this._stack=[],this._position=-1,this._notifyListeners()}addListener(t){return this._listeners.push(t),()=>{this._listeners=this._listeners.filter(e=>e!==t)}}_notifyListeners(){let t=this.getState();for(let e of this._listeners)try{e(t)}catch{}}};function Bt(s){if(!Y(s))return s;let t=typeof s.manifest=="string"?s.manifest:"";if(!(/^0\.0(?:\.|$)/.test(t)||!t&&("version"in s||"manifestVersion"in s||"components"in s||"rootElement"in s||Cr(s.elements))))return s;let r={...s,manifest:"0.1.0"};if(delete r.version,delete r.manifestVersion,!r.elements&&Y(r.components)&&(r.elements=r.components,delete r.components),!r.root&&typeof r.rootElement=="string"&&(r.root=r.rootElement,delete r.rootElement),Y(r.elements)){let l={};for(let[n,a]of Object.entries(r.elements)){if(!Y(a))continue;let u={...a};!u.type&&typeof u.component=="string"&&(u.type=u.component,delete u.component),l[n]=u}r.elements=l}return r}function Cr(s){return Y(s)?Object.values(s).some(t=>Y(t)&&"component"in t):!1}function Y(s){return s!==null&&typeof s=="object"&&!Array.isArray(s)}import{html as je,css as Re,nothing as Rr}from"lit";import{LitElement as jr}from"lit";var ce=class ce extends jr{constructor(){super(...arguments);this._instanceId=`forge-${++ce._instanceCounter}`;this.props={};this.store=null;this.onAction=null;this.itemContext=null}static get properties(){return{props:{type:Object}}}connectedCallback(){super.connectedCallback()}resolve(e){if(!this.store)return e;this.itemContext&&J(this.itemContext);try{return E(this.store,e)}finally{J(null)}}getProp(e){let o=this.props?.[e];return typeof o=="string"&&(o.startsWith("$state:")||o.startsWith("$computed:")||o.startsWith("$item:")||o.startsWith("$expr:")||o.includes("{{")&&o.includes("}}"))?this.resolve(o):o}getArray(e){let o=this.getProp(e);return Array.isArray(o)?o:o&&typeof o=="object"?Object.values(o):[]}getString(e,o=""){let i=this.getProp(e);return typeof i=="string"?i:String(i??o)}getNumber(e,o=0){let i=this.getProp(e);return typeof i=="number"?i:Number(i)||o}getBool(e,o=!1){let i=this.getProp(e);return typeof i=="boolean"?i:o}getBoundProp(e,o){let i=typeof this.props?.bind=="string"?this.props.bind:"";if(i){let l=this.resolve(i);if(l!==void 0)return l}let r=this.getProp(e);return r===void 0?o:r}dispatchAction(e,o){let i=typeof this.props?.bind=="string"?this.props.bind:"",r=i?{...o||{},bind:i}:o;this.onAction&&this.onAction(e,r),this.dispatchEvent(new CustomEvent("forgeui-action",{detail:{action:e,payload:r},bubbles:!0,composed:!0}))}handleAction(e){let o=this.getString("action");o&&this.dispatchAction(o,this.props)}prop(e){return this.getProp(e)}static get sharedStyles(){return[_e]}gapValue(e){let o={none:"0",0:"0","3xs":"var(--forgeui-space-3xs)","2xs":"var(--forgeui-space-2xs)",xs:"var(--forgeui-space-xs)",sm:"var(--forgeui-space-sm)",md:"var(--forgeui-space-md)",lg:"var(--forgeui-space-lg)",xl:"var(--forgeui-space-xl)","2xl":"var(--forgeui-space-2xl)"};if(e==null||e==="")return"var(--forgeui-space-md)";let i=String(e);return i in o?o[i]:/^\d+(\.\d+)?$/.test(i)?`${i}px`:/^\d+(\.\d+)?(px|rem|em|%|vw|vh|ch)$/.test(i)?i:"var(--forgeui-space-md)"}static get styles(){return[_e]}};ce._instanceCounter=0;var g=ce;var Ie=class extends g{static get styles(){return Re`
  ...
  `}render(){let t=this.getString("label","Button"),e=this.getString("variant","primary"),o=this.getString("size",""),i=this.getBool("disabled"),r=this.getProp("pressed");return je`<button class="${e} ${o}" ?disabled=${i}
      aria-pressed=${r==null?Rr:String(!!r)}
  ...
  `}render(){let t=this.getString("label",""),e=this.getString("href","#");return je`<a href="${e}">${t}<slot></slot></a>`}};customElements.define("forgeui-link",Ce);import{html as D,css as X,nothing as qt}from"lit";var De=class extends g{static get properties(){return{props:{type:Object}}}static get styles(){return X`
  ...
  `}render(){let t=this.getString("src",""),e=this.getString("alt",""),o=this.getString("fit","contain");return t?D`<img src="${t}" alt="${e}" style="object-fit:${o}" loading="lazy">`:D`${qt}`}};customElements.define("forgeui-image",Le);var ze=class extends g{static get styles(){return X`
  ...
  `}render(){let t=this.getString("title","Nothing here"),e=this.getString("description","");return D`
      ...
      ${e?D`<div class="desc">${e}</div>`:qt}
      <slot></slot>
    `}};customElements.define("forgeui-empty-state",Ne);import{html as b,css as ue,svg as N,nothing as A}from"lit";var Fe=class extends g{constructor(){super(...arguments);this._query="";this._sortKey="";this._sortDir="asc";this._page=0}static get styles(){return ue`
  ...
  `}_colKey(e){return typeof e=="string"?e:String(e?.key??"")}_colLabel(e){return typeof e=="string"?e:String(e?.label??e?.key??"")}_statusClass(e){let o=String(e??"").toLowerCase().trim();return["done","complete","completed","success","active","ok","approved","paid"].includes(o)?"success":["in progress","in-progress","pending","warning","waiting","review"].includes(o)?"warning":["to do","to-do","todo","backlog","draft","new","inactive"].includes(o)?"neutral":["high","urgent","critical"].includes(o)?"error":["medium","med"].includes(o)?"warning":["low"].includes(o)?"info":["failed","error","rejected","blocked","overdue"].includes(o)?"error":"neutral"}_renderCell(e,o){let i=this._colKey(e),r=o[i],l=e&&typeof e=="object"?e.type:void 0;if(r==null||r==="")return b`<span style="color:var(--forgeui-color-text-tertiary)">—</span>`;if(l==="badge"||l==="status"){let n=(e.variant&&typeof e.variant=="object"?e.variant[String(r).toLowerCase()]:null)||this._statusClass(r);return b`<span class="badge ${n}">${String(r)}</span>`}if(l==="number")return typeof r=="number"?r.toLocaleString():String(r);if(l==="date"){let n=typeof r=="string"||typeof r=="number"?new Date(r):r;return n instanceof Date&&!isNaN(n.getTime())?n.toLocaleDateString():String(r)}if(l==="currency"){let n=Number(r);return isNaN(n)?String(r):n.toLocaleString(void 0,{style:"currency",currency:e.currency||"USD"})}return l==="boolean"?r?"\u2713":"\u2717":String(r)}_sortBy(e){e&&(this._sortKey===e?this._sortDir=this._sortDir==="asc"?"desc":"asc":(this._sortKey=e,this._sortDir="asc"),this._page=0,this.requestUpdate())}_setQuery(e){this._query=e,this._page=0,this.requestUpdate()}_filteredRows(e,o){let i=this._query.trim().toLowerCase();return i?e.filter(r=>o.some(l=>String(r[this._colKey(l)]??"").toLowerCase().includes(i))):e}_sortedRows(e){if(!this._sortKey)return e;let o=this._sortDir==="asc"?1:-1;return[...e].sort((i,r)=>{let l=i?.[this._sortKey],n=r?.[this._sortKey];if(typeof l=="number"&&typeof n=="number")return(l-n)*o;let a=String(l??"").toLowerCase(),u=String(n??"").toLowerCase();return(a>u?1:a<u?-1:0)*o})}_setPage(e,o){this._page=Math.max(0,Math.min(e,Math.max(0,o-1))),this.requestUpdate()}render(){let e=this.getProp("data"),o=this.getProp("columns")||[],i=this.getString("emptyMessage","No data yet"),r=this.getString("rowAction",""),l=this.getString("caption",""),n=this.getBool("searchable",!1),a=Math.max(0,Math.floor(this.getNumber("pageSize",0)));if(!Array.isArray(e))return b`<div class="empty">${i}</div>`;let u=o.length>0?o:e.length>0?Object.keys(e[0]):[];if(u.length===0)return b`<div class="empty">${i}</div>`;let c=this._filteredRows(e,u),d=this._sortedRows(c),p=a>0?Math.max(1,Math.ceil(d.length/a)):1,y=Math.min(this._page,p-1);y!==this._page&&(this._page=y);let L=a>0?d.slice(y*a,y*a+a):d;return b`
      ...
      <table>
        ...
        <thead><tr>${u.map(_=>{let v=this._colLabel(_),M=this._colKey(_),k=typeof _=="object"?_.align:void 0,P=typeof _=="object"?_.width:void 0,$=k==="right"?"align-right":k==="center"?"align-center":"",S=typeof _=="object"&&_.sortable===!0,m=this._sortKey===M;return b`<th class="${$}" style="${P?`width:${P}`:""}" aria-sort=${m?this._sortDir==="asc"?"ascending":"descending":A}>
            ${S?b`
              <button type="button" @click=${()=>this._sortBy(M)}>
                <span>${v}</span>${m?b`<span class="sort" aria-hidden="true">${this._sortDir==="asc"?"\u25B2":"\u25BC"}</span>`:A}
              </button>
            `:v}
          </th>`})}</tr></thead>
        <tbody>${L.length===0?b`<tr><td colspan=${u.length} class="empty">${i}</td></tr>`:L.map((_,v)=>{let M=a>0?y*a+v:v,k=!!r,P=k?String(_[typeof u[0]=="string"?u[0]:u[0]?.key]??`Row ${v+1}`):"";return b`<tr class="${k?"row-action":""}"
                tabindex=${k?0:A}
                role=${k?"button":A}
                aria-label=${k?P:A}
  ...
  `}_niceMax(e){if(e<=0)return 1;let o=Math.pow(10,Math.floor(Math.log10(e))),i=e/o;return(i<=1?1:i<=2?2:i<=5?5:10)*o}render(){let e=this.getString("chartType","bar"),o=this.getProp("data")||[],i=this.getString("title",""),r=this.getString("xKey","label")||this.getString("labelKey","label"),l=this.getString("yKey","value")||this.getString("valueKey","value"),n=this.getString("color","");if(!o||o.length===0)return b`
      ...
      `;let a=o.map(v=>typeof v=="number"?{label:"",value:v}:v&&typeof v=="object"?{label:String(v[r]??v.label??v.name??v.x??""),value:Number(v[l]??v.value??v.y??0)||0,color:v.color}:{label:String(v),value:0}),u=600,c=260,d={top:8,right:16,bottom:36,left:48},p=u-d.left-d.right,y=c-d.top-d.bottom,L,_=A;if(e==="pie"||e==="donut"){let v=a.reduce((C,I)=>C+Math.max(0,I.value),0)||1,M=u/2,k=c/2,P=Math.min(p,y)/2-8,$=e==="donut"?P*.55:0,S=-Math.PI/2,m=[],x=[];a.forEach((C,I)=>{let B=Math.max(0,C.value)/v,j=S,R=S+B*Math.PI*2;S=R;let Q=R-j>Math.PI?1:0,ae=M+P*Math.cos(j),vt=k+P*Math.sin(j),xt=M+P*Math.cos(R),wt=k+P*Math.sin(R),de=C.color||this._palette[I%this._palette.length];if(x.push(de),$>0){let Wt=M+$*Math.cos(j),Ht=k+$*Math.sin(j),Gt=M+$*Math.cos(R),Jt=k+$*Math.sin(R);m.push(N`<path class="slice" fill="${de}" d="M ${ae} ${vt} A ${P} ${P} 0 ${Q} 1 ${xt} ${wt} L ${Gt} ${Jt} A ${$} ${$} 0 ${Q} 0 ${Wt} ${Ht} Z"/>`)}else m.push(N`<path class="slice" fill="${de}" d="M ${M} ${k} L ${ae} ${vt} A ${P} ${P} 0 ${Q} 1 ${xt} ${wt} Z"/>`)}),L=N`<g>${m}</g>`,_=b`<div class="legend">${a.map((C,I)=>b`
        <span class="legend-item"><span class="swatch" style="background:${x[I]}"></span>${C.label} (${C.value})</span>
      `)}</div>`}else{let v=Math.max(...a.map(S=>S.value),0),M=this._niceMax(v),k=S=>d.top+y-S/M*y,P=4,$=[];for(let S=0;S<=P;S++){let m=M*S/P,x=k(m);$.push(N`<line class="grid" x1="${d.left}" x2="${d.left+p}" y1="${x}" y2="${x}"/>`),$.push(N`<text class="tick-label" x="${d.left-6}" y="${x+3}" text-anchor="end">${m.toLocaleString()}</text>`)}if(e==="line"||e==="area"){let S=p/Math.max(1,a.length-1),m=a.map((I,B)=>{let j=d.left+B*S,R=k(I.value);return`${B===0?"M":"L"} ${j} ${R}`}).join(" "),x=e==="area"?m+` L ${d.left+p} ${d.top+y} L ${d.left} ${d.top+y} Z`:"",C=n||"var(--forgeui-color-primary)";L=b`
          <g>${$}</g>
          ${e==="area"?N`<path class="area" d="${x}" style="fill:${C};opacity:0.15"/>`:A}
          ${N`<path class="line" d="${m}" style="stroke:${C}"/>`}
          ${a.map((I,B)=>{let j=d.left+B*S,R=k(I.value);return N`<circle class="point" cx="${j}" cy="${R}" r="3" style="fill:${C}"/>
              <text class="tick-label" x="${j}" y="${d.top+y+14}" text-anchor="middle">${I.label}</text>`})}
        `}else{let S=a.length,m=p/S,x=Math.max(2,m*.7),C=m-x;L=b`
          <g>${$}</g>
          ${a.map((I,B)=>{let j=d.left+B*m+C/2,R=k(I.value),Q=Math.max(0,d.top+y-R),ae=I.color||n||"var(--forgeui-color-primary)";return N`<rect class="bar" x="${j}" y="${R}" width="${x}" height="${Q}" rx="2" style="fill:${ae}">
                <title>${I.label}: ${I.value}</title>
              </rect>
              <text class="tick-label" x="${j+x/2}" y="${d.top+y+14}" text-anchor="middle">${I.label}</text>`})}
        `}}return b`
      ${i?b`<div class="title">${i}</div>`:A}
      <div class="wrap">
  ...
  `}render(){let t=this.getNumber("width",400),e=this.getNumber("height",300),o=this.getString("background","transparent"),i=this.getProp("shapes")||[];return q`
    ...
    `}renderShape(t){let e={fill:t.fill??void 0,stroke:t.stroke??void 0,"stroke-width":t.strokeWidth??void 0,opacity:t.opacity??void 0},o=t.action?()=>{this.onAction&&this.onAction(t.action)}:void 0,i=t.action?"cursor:pointer":void 0;switch(t.type){case"rect":return q`<rect
        ...
        >${t.content??""}</text>`;case"path":return q`<path
          ...
          style="${i}"
          @click=${o}
        />`;default:return q``}}};customElements.define("forgeui-drawing",Ke);import{html as z,css as ie,nothing as K}from"lit";var We=class extends g{static get styles(){return ie`
  ...
  `}render(){let t=this.getString("variant","info"),e=this.getString("title",""),o=this.getString("message","");return z`<div class="alert ${t}" role=${t==="error"||t==="warning"?"alert":"status"}>
      ${e?z`<strong>${e}</strong> `:K}${o}<slot></slot>
  ...
  `}render(){let e=this.getString("title",""),o=this.getBool("open"),i=`${this._instanceId}-title`;return o?this.setAttribute("open",""):this.removeAttribute("open"),o?z`
      ...
      >
        ${e?z`<h2 id="${i}" class="title">${e}</h2>`:K}
        <slot></slot>
      </div>
  ...
  `}render(){let t=this.getProp("value"),e=this.getNumber("max",100),o=t==null,i=o?0:Math.max(0,Math.min(Number(t),e)),r=o?0:i/e*100,l=this.getString("label",""),n=this.getBool("showValue");return z`
      ${l||n?z`
        <div class="meta">
          ${l?z`<span>${l}</span>`:z`<span></span>`}
          ${n?z`<span class="value">${Math.round(r)}%</span>`:K}
        </div>
      `:K}
      <div
  ...
  `}render(){let t=this.getString("message","");return t?z`<div class="toast">${t}</div>`:z`${K}`}};customElements.define("forgeui-toast",Je);var Ze=class extends g{static get styles(){return ie`
    :host { display:block; }
  ...
  `}render(){let t=this.getString("msg","Unknown error");return z`<div class="error" role="alert">⚠ ${t}</div>`}};customElements.define("forgeui-error",Ze);import{html as w,css as F,nothing as O}from"lit";var Ye=class extends g{static get styles(){return F`
  ...
  `}render(){let t=this.getString("label",""),e=this.getString("placeholder",""),o=this.getString("hint",""),i=this.getString("error",""),r=this.getString("inputType","")||this.getString("type","text"),l=this.getBool("multiline"),n=String(this.getBoundProp("value","")??""),a=this._instanceId;return w`
      ${t?w`<label for="${a}">${t}</label>`:O}
      ${l?w`<textarea id="${a}" placeholder="${e}" .value=${n} @input=${u=>this.dispatchAction("change",{value:u.target.value})}></textarea>`:w`<input id="${a}" type="${r}" placeholder="${e}" .value=${n} @input=${u=>this.dispatchAction("change",{value:u.target.value})}>`}
      ${o&&!i?w`<div class="hint">${o}</div>`:O}
      ${i?w`<div class="error">${i}</div>`:O}
    `}};customElements.define("forgeui-text-input",Ye);var Xe=class extends g{static get styles(){return F`
    :host { display:block; flex:1 1 auto; min-width:0; max-width:100%; margin-bottom:var(--forgeui-space-sm); }
  ...
  `}render(){let t=this.getString("label",""),e=this.getProp("min"),o=this.getProp("max"),i=this.getProp("step"),r=this.getBoundProp("value"),l=this._instanceId;return w`
      ${t?w`<label for="${l}">${t}</label>`:O}
      <input id="${l}" type="number" min=${e} max=${o} step=${i} .value=${r??""}
        @input=${n=>this.dispatchAction("change",{value:Number(n.target.value)})}>
    `}};customElements.define("forgeui-number-input",Xe);var Qe=class extends g{static get styles(){return F`
    :host { display:block; flex:1 1 auto; min-width:0; max-width:100%; margin-bottom:var(--forgeui-space-sm); }
    label { display:block; font-size:var(--forgeui-text-sm); font-weight:var(--forgeui-weight-medium); margin-bottom:var(--forgeui-space-2xs); overflow-wrap:break-word; }
  ...
  `}render(){let t=this.getString("label",""),e=this.getProp("options")||[],o=String(this.getBoundProp("value","")??""),i=this._instanceId;return w`
      ${t?w`<label for="${i}">${t}</label>`:O}
      <select id="${i}" .value=${o} @change=${r=>this.dispatchAction("change",{value:r.target.value})}>
        ${e.map(r=>w`<option value=${typeof r=="string"?r:r.value} ?selected=${(typeof r=="string"?r:r.value)===o}>
          ${typeof r=="string"?r:r.label||r.value}
        </option>`)}
      </select>
    `}};customElements.define("forgeui-select",Qe);var et=class extends g{static get styles(){return F`
  ...
  `}render(){let t=this.getString("label",""),e=this.getProp("options")||[],o=this.getBoundProp("value",this.getProp("selected")??[]),i=Number(this.getProp("maxSelections")),r=Number.isFinite(i)&&i>=0?i:1/0,l=(Array.isArray(o)?o.map(c=>String(c)):[]).slice(0,r),n=this.getBool("disabled"),a=this._instanceId,u=c=>{let d=l.filter(p=>p!==c);this.dispatchAction("remove",{value:c}),this.dispatchAction("change",{value:d,selected:d})};return w`
      ...
      <select id="${a}" multiple ?disabled=${n}
        ...
        ${e.map(c=>{let d=String(typeof c=="string"?c:c?.value??c?.label??"");return w`<option value=${d} ?selected=${l.includes(d)}>
            ${typeof c=="string"?c:c?.label??d}
          </option>`})}
      </select>
      <div class="tags">
        ${l.map(c=>w`<span class="tag">${c}<button type="button" aria-label=${`Remove ${c}`} @click=${()=>u(c)}>×</button></span>`)}
        <slot></slot>
      </div>
    `}};customElements.define("forgeui-multi-select",et);var tt=class extends g{static get styles(){return F`
  ...
  `}render(){let t=this.getString("label",""),e=!!this.getBoundProp("checked",this.getProp("value")??!1),o=this._instanceId;return w`
      <input id="${o}" type="checkbox" ?checked=${e} @change=${i=>this.dispatchAction("change",{checked:i.target.checked})}>
      ${t?w`<label for="${o}">${t}</label>`:O}
    `}};customElements.define("forgeui-checkbox",tt);var rt=class extends g{constructor(){super(...arguments);this._toggle=()=>{if(this.getBool("disabled"))return;let e=!!this.getBoundProp("on",this.getProp("value")??!1);this.dispatchAction("change",{value:!e,checked:!e})};this._onKeydown=e=>{(e.key==="Enter"||e.key===" "||e.key==="Spacebar")&&(e.preventDefault(),this._toggle())}}static get styles(){return F`
  ...
  `}render(){let e=!!this.getBoundProp("on",this.getProp("value")??!1),o=this.getString("label",""),i=this.getBool("disabled"),r=this._instanceId;return w`
      <label for="${r}" class="toggle-label">
        ...
        ></button>
        ${o?w`<span class="toggle-text">${o}</span>`:O}
      </label>
    `}};customElements.define("forgeui-toggle",rt);var ot=class extends g{static get styles(){return F`
    :host { display:block; flex:1 1 auto; min-width:0; max-width:100%; margin-bottom:var(--forgeui-space-sm); }
    label { display:block; font-size:var(--forgeui-text-sm); font-weight:var(--forgeui-weight-medium); margin-bottom:var(--forgeui-space-2xs); overflow-wrap:break-word; }
    input { width:100%; padding:var(--forgeui-space-xs) var(--forgeui-space-sm); border:1px solid var(--forgeui-color-border);
      border-radius:var(--forgeui-radius-md); font:inherit; height:var(--forgeui-input-height);
      background:var(--forgeui-color-surface); color:var(--forgeui-color-text); box-sizing:border-box; min-width:0; }
    input:focus { outline:none; border-color:var(--forgeui-color-primary); box-shadow:0 0 0 3px var(--forgeui-color-primary-subtle); }
  `}render(){let t=this.getString("label",""),e=this.getString("value",""),o=this._instanceId;return w`
      ${t?w`<label for="${o}">${t}</label>`:O}
      <input id="${o}" type="date" .value=${e} @change=${i=>this.dispatchAction("change",{value:i.target.value})}>
    `}};customElements.define("forgeui-date-picker",ot);var it=class extends g{static get styles(){return F`
    :host { display:block; flex:1 1 auto; min-width:0; max-width:100%; margin-bottom:var(--forgeui-space-sm); }
    label { display:block; font-size:var(--forgeui-text-sm); font-weight:var(--forgeui-weight-medium); margin-bottom:var(--forgeui-space-2xs); overflow-wrap:break-word; }
  ...
  `}render(){let t=this.getString("label",""),e=this.getNumber("min",0),o=this.getNumber("max",100),i=this.getNumber("step",1),r=this.getBoundProp("value",e),l=Number(r);Number.isFinite(l)||(l=e);let n=this._instanceId;return w`
      ${t?w`<label for="${n}">${t}</label>`:O}
      <input id="${n}" type="range" min=${e} max=${o} step=${i} .value=${l}
        @input=${a=>this.dispatchAction("change",{value:Number(a.target.value)})}>
      <div class="value">${l}</div>
    `}};customElements.define("forgeui-slider",it);var st=class extends g{constructor(){super(...arguments);this._dragging=!1;this._openFilePicker=()=>{this.shadowRoot?.querySelector('input[type="file"]')?.click()};this._onDropzoneKeydown=e=>{e.key!=="Enter"&&e.key!==" "||(e.preventDefault(),this._openFilePicker())};this._onFileChange=e=>{let o=Array.from(e.target.files??[]);this._processFiles(o)};this._onDragOver=e=>{e.preventDefault(),!this._dragging&&(this._dragging=!0,this.requestUpdate())};this._onDragLeave=e=>{e.currentTarget===e.target&&(this._dragging=!1,this.requestUpdate())};this._onDrop=e=>{e.preventDefault(),this._dragging=!1,this.requestUpdate(),this._processFiles(Array.from(e.dataTransfer?.files??[]))}}static get styles(){return F`
    :host { display:block; margin-bottom:var(--forgeui-space-sm); }
  ...
  `}_maxSizeBytes(){let e=this.getProp("maxSize");if(typeof e=="number"&&Number.isFinite(e)&&e>=0)return Math.floor(e);if(typeof e!="string")return null;let o=e.trim().match(/^(\d+(?:\.\d+)?)\s*(b|kb|mb|gb)?$/i);if(!o)return null;let i=Number(o[1]),r=(o[2]||"b").toLowerCase(),l=r==="gb"?1<<30:r==="mb"?1<<20:r==="kb"?1024:1,n=i*l;return Number.isFinite(n)?Math.floor(n):null}_newFileId(){return globalThis.crypto?.randomUUID?.()??`${Date.now()}_${Math.random().toString(36).slice(2)}`}_processFiles(e){let o=this.getBool("multiple"),i=this._maxSizeBytes(),r=(o?e:e.slice(0,1)).map(d=>{let p=this._newFileId(),y=i==null||d.size<=i,L={id:p,name:d.name,size:d.size,type:d.type,lastModified:d.lastModified,accepted:y,storageKey:y?p:null};return y||(L.error="maxSize"),[d,L]}),l=r.filter(([,d])=>d.accepted),n=r.map(([,d])=>d),a=l.map(([,d])=>d),u=o?a:a[0]??null,c=a[0]??null;this.dispatchAction("change",{id:c?.id??null,uuid:c?.id??null,name:c?.name??null,size:c?.size??null,type:c?.type??null,lastModified:c?.lastModified??null,storageKey:c?.storageKey??null,value:u,files:n,rejected:n.filter(d=>!d.accepted),multiple:o,maxSize:i}),Ft(l.map(([d,p])=>({file:d,id:p.id})))}render(){let e=this.getString("label","Upload file"),o=this.getString("accept","*"),i=this.getBool("multiple");return w`
    ...
    `}};customElements.define("forgeui-file-upload",st);import{html as T,css as V,nothing as nt}from"lit";var at=class extends g{static get properties(){return{props:{type:Object}}}static get styles(){return V`
  ...
  `}render(){let t=this.getProp("columns"),e;typeof t=="number"?e=String(t):typeof t=="string"&&t?e=t:e="1";let o=/^\d+$/.test(e)?`repeat(${e}, minmax(0, 1fr))`:e,i=this.getString("gap","md"),r=this.gapValue(i),l=this.getString("padding",""),n=l?this.gapValue(l):"0";return this.style.gridTemplateColumns=o,this.style.gap=r,this.style.padding=n,/^\d+$/.test(e)&&Number(e)>=2&&this.setAttribute("responsive",""),T`<slot></slot>`}};customElements.define("forgeui-grid",lt);var ct=class extends g{static get properties(){return{props:{type:Object}}}static get styles(){return V`
  ...
  `}render(){let t=this.getString("variant",""),e=this.getString("title",""),o=this.getString("subtitle","");return t&&this.setAttribute("variant",t),T`
      ${e||o?T`
        <div class="header">
          ${e?T`<div class="title">${e}</div>`:nt}
          ${o?T`<div class="subtitle">${o}</div>`:nt}
        </div>
      `:nt}
      <div class="body"><slot></slot></div>
    `}};customElements.define("forgeui-card",ct);var ut=class extends g{static get properties(){return{props:{type:Object}}}static get styles(){return V`:host { display:block; margin-inline:auto; width:100%; box-sizing:border-box; }`}render(){let t=this.getString("maxWidth",""),e={sm:"640px",md:"768px",lg:"1024px",xl:"1280px","2xl":"1536px",full:"100%",none:"none","":""},o=t in e?e[t]:t,i=this.getString("padding","");return o&&o!=="none"?this.style.maxWidth=o:this.style.maxWidth="",this.style.padding=i?this.gapValue(i):"",T`<slot></slot>`}};customElements.define("forgeui-container",ut);var dt=class extends g{static get properties(){return{props:{type:Object}}}constructor(){super(),this._active=""}static get styles(){return V`
    :host { display:block; }
  ...
  `}_itemKey(t){return typeof t=="string"?t:String(t&&typeof t=="object"?t.id??t.key??t.value??t.label??"":t??"")}_itemLabel(t){return typeof t=="string"?t:String(t&&typeof t=="object"?t.label??t.title??t.value??"":t??"")}updated(){Array.from(this.children).filter(e=>!(e instanceof HTMLScriptElement)).forEach((e,o)=>{let i=(e.props||{}).slot??e.getAttribute("slot");String(o)===this._active||i===this._active?e.setAttribute("data-active",""):e.removeAttribute("data-active")})}_moveTo(t,e){let o=this._itemKey(e[t])||String(t);this._active=o,this.requestUpdate(),this.dispatchAction("tab-change",{active:o,value:o}),this.updateComplete.then(()=>{this.shadowRoot?.querySelector(`#${this._instanceId}-tab-${t}`)?.focus()})}render(){let t=this.getProp("items")||this.getProp("tabs")||[],e=Array.isArray(t)?t:[],o=this.getBoundProp("activeTab",this.getProp("value"));o!==void 0&&String(o)!==this._active&&(this._active=String(o)),!this._active&&e.length>0&&(this._active=this._itemKey(e[0])||"0");let i=e.findIndex((l,n)=>(this._itemKey(l)||String(n))===this._active),r=(l,n)=>{let a=-1;l.key==="ArrowRight"?a=(n+1)%e.length:l.key==="ArrowLeft"?a=(n-1+e.length)%e.length:l.key==="Home"?a=0:l.key==="End"&&(a=e.length-1),a!==-1&&(l.preventDefault(),this._moveTo(a,e))};return T`
    ...
    `}};customElements.define("forgeui-tabs",dt);var pt=class extends g{static get properties(){return{props:{type:Object}}}static get styles(){return V`
    :host { display:block; }
  ...
  `}render(){let t=this.getString("title","Section");return T`<details><summary>${t}</summary><div class="content"><slot></slot></div></details>`}};customElements.define("forgeui-accordion",pt);var gt=class extends g{static get styles(){return V`
    :host { display:block; }
    hr { border:none; border-top:1px solid var(--forgeui-color-border); margin:var(--forgeui-space-sm) 0; }
  `}render(){return T`<hr>`}};customElements.define("forgeui-divider",gt);var ft=class extends g{static get styles(){return V`:host { display:block; }`}render(){let t=this.getString("size","md"),e=this.getString("height",""),o=this.getString("width",""),i=e?this.gapValue(e):this.gapValue(t),r=o?/^\d+(\.\d+)?%$/.test(o)?o:this.gapValue(o):"";return T`<div style="height:${i};${r?`width:${r}`:""}"></div>`}};customElements.define("forgeui-spacer",ft);var mt=class extends g{static get properties(){return{props:{type:Object}}}static get styles(){return V`
    :host { display:flex; flex-direction:column; gap:var(--forgeui-space-md); min-width:0; }
    :host([direction="row"]) { flex-direction:row; flex-wrap:wrap; }
    .empty { padding:var(--forgeui-space-lg); text-align:center; color:var(--forgeui-color-text-tertiary); font-size:var(--forgeui-text-sm); }
  `}render(){let t=this.getArray("data"),e=this.getString("emptyMessage",""),o=this.getString("direction","column");(o==="row"||o==="horizontal")&&this.setAttribute("direction","row");let i=this.getString("gap","md");return this.style.gap=this.gapValue(i),t.length===0&&e?T`<div class="empty">${e}</div>`:T`<slot></slot>`}};customElements.define("forgeui-repeater",mt);import{html as H,css as Kt,nothing as Lr}from"lit";var ht=class extends g{static get styles(){return Kt`
  ...
  `}render(){let t=this.getProp("items")||[];return H`${t.map((e,o)=>{let i=o===t.length-1,r=typeof e=="string"?e:e.label,l=typeof e=="string"?"#":e.href;return H`
        ${o>0?H`<span class="sep">/</span>`:Lr}
        ${i?H`<span class="current">${r}</span>`:H`<a href="${l}">${r}</a>`}
      `})}`}};customElements.define("forgeui-breadcrumb",ht);var bt=class extends g{static get styles(){return Kt`
