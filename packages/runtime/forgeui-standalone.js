...
`)}function Te(){return{type:"object",required:["manifest","id","elements"],properties:{manifest:{type:"string",const:"0.1.0"},id:{type:"string",pattern:"^[a-z0-9][a-z0-9-]*$"},root:{type:"string"},title:{type:"string"},schema:{type:"object",properties:{version:{type:"number"},tables:{type:"object",additionalProperties:{type:"object",properties:{columns:{type:"object",additionalProperties:{type:"object",properties:{type:{type:"string",enum:["string","number","boolean"]},default:{}},required:["type"]}}},required:["columns"]}}}},state:{type:"object",additionalProperties:{}},elements:{type:"object",additionalProperties:{type:"object",properties:{type:{type:"string",enum:de},props:{type:"object"},children:{type:"array",items:{type:"string"}},visible:{type:"object",properties:{$when:{type:"object",properties:{path:{type:"string"},eq:{},neq:{},gt:{type:"number"},lt:{type:"number"},gte:{type:"number"},lte:{type:"number"},in:{type:"array"},notIn:{type:"array"},exists:{type:"boolean"}},required:["path"]}}}},required:["type"]}},actions:{type:"object",additionalProperties:{type:"object",properties:{type:{type:"string",enum:[...Jt]},path:{type:"string"},value:{},operation:{type:"string",enum:[...Yt]},set:{type:"object"},data:{type:"object"},key:{type:"string"},formId:{type:"string"},action:{type:"string"},target:{type:"string"},url:{type:"string"},method:{type:"string",enum:[...Zt]},body:{type:"object"},message:{type:"string"},duration:{type:"number",minimum:0}},required:["type"]}},dataAccess:{type:"object",properties:{enabled:{type:"boolean",default:!1},readable:{type:"array",items:{type:"string"}},restricted:{type:"array",items:{type:"string"}},summaries:{type:"boolean",default:!1}}},persistState:{type:"boolean"},skipPersistState:{type:"boolean"}}}}var Br={container:"Stack",row:"Stack",column:"Stack",text:"Text",heading:"Text",label:"Text",image:"Image",icon:"Icon",button:"Button",link:"Link",textInput:"TextInput",textarea:"Textarea",textArea:"Textarea",numberInput:"NumberInput",checkbox:"Checkbox",toggle:"Toggle",select:"Select",divider:"Divider",spacer:"Spacer",card:"Card",table:"Table",list:"List",alert:"Alert",progress:"Progress",badge:"Badge",avatar:"Avatar"};function Vr(s){let t={},e=0;function o(n){let a=n.id||`a2ui-${e++}`,u=Br[n.type];if(!u||!ie(u))return t[a]={type:"Text",props:{content:`[Unsupported A2UI type: ${n.type}]`,variant:"caption",colorScheme:"secondary"}},a;let l=qr(n.type,n.props||{}),d=(n.children||[]).map(p=>o(p));return t[a]={type:u,props:l,children:d.length>0?d:void 0},a}let r=(s.components||s.content||[]).map(n=>o(n)),c=r.length===1?r[0]:"a2ui-root";return r.length>1&&(t[c]={type:"Stack",props:{gap:"md"},children:r}),{manifest:"0.1.0",id:`a2ui-${Date.now()}`,root:c,elements:t,actions:{}}}function qr(s,t){let e={...t};switch(s){case"heading":e.variant="heading",e.content=t.text||t.content||"",delete e.text;break;case"text":case"label":e.content=t.text||t.content||"",delete e.text;break;case"row":e.direction="row",e.gap=t.gap||t.spacing||"md";break;case"column":e.direction="column",e.gap=t.gap||t.spacing||"md";break;case"container":e.gap=t.gap||t.spacing||"md";break;case"image":e.src=t.src||t.url||"",e.alt=t.alt||"";break;case"button":e.label=t.text||t.label||"",e.variant=t.variant||t.style||"default";break;case"textInput":case"textarea":case"textArea":e.placeholder=t.placeholder||"",e.value=t.value||"",e.label=t.label||"";break;case"numberInput":e.placeholder=t.placeholder||"",e.value=t.value??0,e.label=t.label||"";break;case"select":t.options&&Array.isArray(t.options)&&(e.options=t.options.map(o=>typeof o=="string"?{value:o,label:o}:o));break;case"alert":e.variant=t.type||t.severity||"info",e.content=t.message||t.text||t.content||"",delete e.message,delete e.text;break;case"progress":e.value=t.value??t.percent??0;break}return e}function er(s){if(!s||typeof s!="object")return!1;let t=s;return!!(Array.isArray(t.components)||Array.isArray(t.content)||typeof t.type=="string"&&(t.type==="adaptive-card"||t.type.startsWith("a2ui"))||typeof t.version=="string"&&t.version.startsWith("a2ui")||!t.elements&&!t.manifest&&Array.isArray(t.content))}function Ie(s){return er(s)?Vr(s):s}function tr(s){return`forgeui_${(s||"global").replace(/[^a-zA-Z0-9-]/g,"_")}`}var Me="f";function Kr(s){return new Promise((t,e)=>{let o=r=>e(r??new Error("IDB"));if(!globalThis.indexedDB)return o();let i=indexedDB.open(`${tr(s)}_f`,1);i.onupgradeneeded=()=>{i.result.createObjectStore(Me)},i.onsuccess=()=>t(i.result),i.onerror=i.onblocked=()=>o(i.error)})}async function rr(s,t){if(s.length===0)return;let e=null;try{e=await Kr(t),await new Promise((o,i)=>{let r=e.transaction(Me,"readwrite"),c=r.objectStore(Me);for(let{file:n,id:a}of s)c.put(n,a);r.oncomplete=()=>o(),r.onerror=r.onabort=()=>i(r.error)})}catch{return}finally{e?.close()}}function or(s,t,e="none"){let o=tr(t),i=e,r=null,c=!1,n=!1,a=null,u=null,l=null,d=[];function p(){let b=v();for(let $ of d)try{$(b)}catch{}}function v(){return{mode:i,isPersisting:r!=null&&c,lastSaved:a,lastLoaded:u,error:l,dbName:i==="indexeddb"?o:null}}async function N(){if(r)return r;try{let{createIndexedDbPersister:b}=await import("tinybase/persisters/persister-indexed-db");return r=b(s,o),l=null,r}catch(b){throw l=`IndexedDB unavailable: ${b.message}`,p(),b}}async function P(){if(i==="none")return;let b=await N();try{await b.load(),u=Date.now()}catch($){l=`Load failed: ${$.message}`,p()}try{b.startAutoSave(),c=!0}catch($){l=`Auto-save failed: ${$.message}`}try{b.startAutoLoad(1),n=!0}catch($){l=`Auto-load failed: ${$.message}`}p()}async function x(){if(r){c&&(r.stopAutoSave(),c=!1),n&&(r.stopAutoLoad(),n=!1);try{await r.save(),a=Date.now()}catch(b){l=`Final save failed: ${b.message}`}p()}}async function I(){if(!(i==="none"||!r)){try{await r.save(),a=Date.now(),l=null}catch(b){l=`Save failed: ${b.message}`}p()}}async function _(){if(!(i==="none"||!r)){try{await r.load(),u=Date.now(),l=null}catch(b){l=`Load failed: ${b.message}`}p()}}async function E(){if(await x(),r){try{r.destroy()}catch{}r=null}d=[],p()}function k(b){if(b===i)return;let $=i;i=b,$==="indexeddb"&&b==="none"&&(r&&c&&(r.stopAutoSave(),c=!1),r&&n&&(r.stopAutoLoad(),n=!1)),b==="indexeddb"&&$==="none"&&P().catch(()=>{}),p()}function A(b){return d.push(b),()=>{d=d.filter($=>$!==b)}}return{start:P,stop:x,save:I,load:_,setMode:k,getStatus:v,addListener:A,destroy:E}}var ge=class{constructor(t=50){this._stack=[];this._position=-1;this._listeners=[];this._maxDepth=t}push(t){if(this._position<this._stack.length-1&&(this._stack=this._stack.slice(0,this._position+1)),this._position>=0){let e=this._stack[this._position];if(JSON.stringify(e)===JSON.stringify(t))return}this._stack.push(structuredClone(t)),this._stack.length>this._maxDepth?this._stack.shift():this._position++,this._notifyListeners()}undo(){return this._position<=0?null:(this._position--,this._notifyListeners(),structuredClone(this._stack[this._position]))}redo(){return this._position>=this._stack.length-1?null:(this._position++,this._notifyListeners(),structuredClone(this._stack[this._position]))}current(){return this._position<0?null:structuredClone(this._stack[this._position])}jumpTo(t){return t<0||t>=this._stack.length?null:(this._position=t,this._notifyListeners(),structuredClone(this._stack[this._position]))}getState(){return{canUndo:this._position>0,canRedo:this._position<this._stack.length-1,position:this._position,total:this._stack.length}}getTimeline(){return this._stack.map((t,e)=>({position:e,id:t.id}))}clear(){this._stack=[],this._position=-1,this._notifyListeners()}addListener(t){return this._listeners.push(t),()=>{this._listeners=this._listeners.filter(e=>e!==t)}}_notifyListeners(){let t=this.getState();for(let e of this._listeners)try{e(t)}catch{}}};function ir(s){if(!Z(s))return s;let t=typeof s.manifest=="string"?s.manifest:"";if(!(/^0\.0(?:\.|$)/.test(t)||!t&&("version"in s||"manifestVersion"in s||"components"in s||"rootElement"in s||Hr(s.elements))))return s;let r={...s,manifest:"0.1.0"};if(delete r.version,delete r.manifestVersion,!r.elements&&Z(r.components)&&(r.elements=r.components,delete r.components),!r.root&&typeof r.rootElement=="string"&&(r.root=r.rootElement,delete r.rootElement),Z(r.elements)){let c={};for(let[n,a]of Object.entries(r.elements)){if(!Z(a))continue;let u={...a};!u.type&&typeof u.component=="string"&&(u.type=u.component,delete u.component),c[n]=u}r.elements=c}return r}function Hr(s){return Z(s)?Object.values(s).some(t=>Z(t)&&"component"in t):!1}function Z(s){return s!==null&&typeof s=="object"&&!Array.isArray(s)}import{html as Le,css as De,nothing as Gr}from"lit";import{LitElement as Wr}from"lit";var fe=class fe extends Wr{constructor(){super(...arguments);this._instanceId=`forge-${++fe._instanceCounter}`;this.props={};this.store=null;this.onAction=null;this.itemContext=null}static get properties(){return{props:{type:Object}}}connectedCallback(){super.connectedCallback()}resolve(e){if(!this.store)return e;this.itemContext&&J(this.itemContext);try{return T(this.store,e)}finally{J(null)}}getProp(e){let o=this.props?.[e];return typeof o=="string"&&(o.startsWith("$state:")||o.startsWith("$computed:")||o.startsWith("$item:")||o.startsWith("$expr:")||o.includes("{{")&&o.includes("}}"))?this.resolve(o):o}getArray(e){let o=this.getProp(e);return Array.isArray(o)?o:o&&typeof o=="object"?Object.values(o):[]}getString(e,o=""){let i=this.getProp(e);return typeof i=="string"?i:String(i??o)}getNumber(e,o=0){let i=this.getProp(e);return typeof i=="number"?i:Number(i)||o}getBool(e,o=!1){let i=this.getProp(e);return typeof i=="boolean"?i:o}getBoundProp(e,o){let i=typeof this.props?.bind=="string"?this.props.bind:"";if(i){let c=this.resolve(i);if(c!==void 0)return c}let r=this.getProp(e);return r===void 0?o:r}dispatchAction(e,o){let i=typeof this.props?.bind=="string"?this.props.bind:"",r=i?{...o||{},bind:i}:o;this.onAction&&this.onAction(e,r),this.dispatchEvent(new CustomEvent("forgeui-action",{detail:{action:e,payload:r},bubbles:!0,composed:!0}))}handleAction(e){let o=this.getString("action");o&&this.dispatchAction(o,this.props)}prop(e){return this.getProp(e)}static get sharedStyles(){return[Pe]}gapValue(e){let o={none:"0",0:"0","3xs":"var(--forgeui-space-3xs)","2xs":"var(--forgeui-space-2xs)",xs:"var(--forgeui-space-xs)",sm:"var(--forgeui-space-sm)",md:"var(--forgeui-space-md)",lg:"var(--forgeui-space-lg)",xl:"var(--forgeui-space-xl)","2xl":"var(--forgeui-space-2xl)"};if(e==null||e==="")return"var(--forgeui-space-md)";let i=String(e);return i in o?o[i]:/^\d+(\.\d+)?$/.test(i)?`${i}px`:/^\d+(\.\d+)?(px|rem|em|%|vw|vh|ch)$/.test(i)?i:"var(--forgeui-space-md)"}static get styles(){return[Pe]}};fe._instanceCounter=0;var g=fe;var Ce=class extends g{static get styles(){return De`
  ...
  `}render(){let t=this.getString("label","Button"),e=this.getString("variant","primary"),o=this.getString("size",""),i=this.getBool("disabled"),r=this.getProp("pressed");return Le`<button class="${e} ${o}" ?disabled=${i}
      aria-pressed=${r==null?Gr:String(!!r)}
      @click=${c=>this.handleAction(c)}>${t}<slot></slot></button>`}};customElements.define("forgeui-button",Ce);var je=class extends g{static get styles(){return De`
    :host { display:flex; gap:var(--forgeui-space-xs); }
  `}render(){return Le`<slot></slot>`}};customElements.define("forgeui-button-group",je);var Re=class extends g{static get styles(){return De`
    :host { display:inline-flex; }
  ...
  `}render(){let t=this.getString("label",""),e=this.getString("href","#");return Le`<a href="${e}">${t}<slot></slot></a>`}};customElements.define("forgeui-link",Re);import{html as O,css as X,nothing as sr}from"lit";var ze=class extends g{static get properties(){return{props:{type:Object}}}static get styles(){return X`
  ...
  `}render(){let t=this.getString("title","Nothing here"),e=this.getString("description","");return O`
      ...
      ${e?O`<div class="desc">${e}</div>`:sr}
      <slot></slot>
    `}};customElements.define("forgeui-empty-state",Be);import{html as y,css as Q,svg as B,nothing as S}from"lit";var Ve=class extends g{constructor(){super(...arguments);this._query="";this._sortKey="";this._sortDir="asc";this._page=0}static get styles(){return Q`
  ...
  `}_colKey(e){return typeof e=="string"?e:String(e?.key??"")}_colLabel(e){return typeof e=="string"?e:String(e?.label??e?.key??"")}_statusClass(e){let o=String(e??"").toLowerCase().trim();return["done","complete","completed","success","active","ok","approved","paid"].includes(o)?"success":["in progress","in-progress","pending","warning","waiting","review"].includes(o)?"warning":["to do","to-do","todo","backlog","draft","new","inactive"].includes(o)?"neutral":["high","urgent","critical"].includes(o)?"error":["medium","med"].includes(o)?"warning":["low"].includes(o)?"info":["failed","error","rejected","blocked","overdue"].includes(o)?"error":"neutral"}_renderCell(e,o){let i=this._colKey(e),r=o[i],c=e&&typeof e=="object"?e.type:void 0;if(r==null||r==="")return y`<span style="color:var(--forgeui-color-text-tertiary)">—</span>`;if(c==="badge"||c==="status"){let n=(e.variant&&typeof e.variant=="object"?e.variant[String(r).toLowerCase()]:null)||this._statusClass(r);return y`<span class="badge ${n}">${String(r)}</span>`}if(c==="number")return typeof r=="number"?r.toLocaleString():String(r);if(c==="date"){let n=typeof r=="string"||typeof r=="number"?new Date(r):r;return n instanceof Date&&!isNaN(n.getTime())?n.toLocaleDateString():String(r)}if(c==="currency"){let n=Number(r);return isNaN(n)?String(r):n.toLocaleString(void 0,{style:"currency",currency:e.currency||"USD"})}return c==="boolean"?r?"\u2713":"\u2717":String(r)}_sortBy(e){e&&(this._sortKey===e?this._sortDir=this._sortDir==="asc"?"desc":"asc":(this._sortKey=e,this._sortDir="asc"),this._page=0,this.requestUpdate())}_setQuery(e){this._query=e,this._page=0,this.requestUpdate()}_filteredRows(e,o){let i=this._query.trim().toLowerCase();return i?e.filter(r=>o.some(c=>String(r[this._colKey(c)]??"").toLowerCase().includes(i))):e}_sortedRows(e){if(!this._sortKey)return e;let o=this._sortDir==="asc"?1:-1;return[...e].sort((i,r)=>{let c=i?.[this._sortKey],n=r?.[this._sortKey];if(typeof c=="number"&&typeof n=="number")return(c-n)*o;let a=String(c??"").toLowerCase(),u=String(n??"").toLowerCase();return(a>u?1:a<u?-1:0)*o})}_setPage(e,o){this._page=Math.max(0,Math.min(e,Math.max(0,o-1))),this.requestUpdate()}render(){let e=this.getProp("data"),o=this.getProp("columns")||[],i=this.getString("emptyMessage","No data yet"),r=this.getString("rowAction",""),c=this.getString("caption",""),n=this.getBool("searchable",!1),a=Math.max(0,Math.floor(this.getNumber("pageSize",0)));if(!Array.isArray(e))return y`<div class="empty">${i}</div>`;let u=o.length>0?o:e.length>0?Object.keys(e[0]):[];if(u.length===0)return y`<div class="empty">${i}</div>`;let l=this._filteredRows(e,u),d=this._sortedRows(l),p=a>0?Math.max(1,Math.ceil(d.length/a)):1,v=Math.min(this._page,p-1);v!==this._page&&(this._page=v);let N=a>0?d.slice(v*a,v*a+a):d;return y`
      ...
      <table>
        ...
        <thead><tr>${u.map(P=>{let x=this._colLabel(P),I=this._colKey(P),_=typeof P=="object"?P.align:void 0,E=typeof P=="object"?P.width:void 0,k=_==="right"?"align-right":_==="center"?"align-center":"",A=typeof P=="object"&&P.sortable===!0,b=this._sortKey===I;return y`<th class="${k}" style="${E?`width:${E}`:""}" aria-sort=${b?this._sortDir==="asc"?"ascending":"descending":S}>
            ${A?y`
        ...
        <tbody>${N.length===0?y`<tr><td colspan=${u.length} class="empty">${i}</td></tr>`:N.map((P,x)=>{let I=a>0?v*a+x:x,_=!!r,E=_?String(P[typeof u[0]=="string"?u[0]:u[0]?.key]??`Row ${x+1}`):"";return y`<tr class="${_?"row-action":""}"
                tabindex=${_?0:S}
                role=${_?"button":S}
                aria-label=${_?E:S}
                @click=${_?()=>this.dispatchAction(r,{row:P,index:I}):void 0}
                @keydown=${_?k=>{(k.key==="Enter"||k.key===" ")&&(k.preventDefault(),this.dispatchAction(r,{row:P,index:I}))}:void 0}>
  ...
  `}_niceMax(e){if(e<=0)return 1;let o=Math.pow(10,Math.floor(Math.log10(e))),i=e/o;return(i<=1?1:i<=2?2:i<=5?5:10)*o}render(){let e=this.getString("chartType","bar"),o=this.getProp("data")||[],i=this.getString("title",""),r=this.getString("xKey","label")||this.getString("labelKey","label"),c=this.getString("yKey","value")||this.getString("valueKey","value"),n=this.getString("color","");if(!o||o.length===0)return y`
      ...
      `;let a=o.map(x=>typeof x=="number"?{label:"",value:x}:x&&typeof x=="object"?{label:String(x[r]??x.label??x.name??x.x??""),value:Number(x[c]??x.value??x.y??0)||0,color:x.color}:{label:String(x),value:0}),u=600,l=260,d={top:8,right:16,bottom:36,left:48},p=u-d.left-d.right,v=l-d.top-d.bottom,N,P=S;if(e==="pie"||e==="donut"){let x=a.reduce((L,M)=>L+Math.max(0,M.value),0)||1,I=u/2,_=l/2,E=Math.min(p,v)/2-8,k=e==="donut"?E*.55:0,A=-Math.PI/2,b=[],$=[];a.forEach((L,M)=>{let q=Math.max(0,L.value)/x,D=A,z=A+q*Math.PI*2;A=z;let ee=z-D>Math.PI?1:0,ue=I+E*Math.cos(D),Ct=_+E*Math.sin(D),jt=I+E*Math.cos(z),Rt=_+E*Math.sin(z),me=L.color||this._palette[M%this._palette.length];if($.push(me),k>0){let ar=I+k*Math.cos(D),nr=_+k*Math.sin(D),lr=I+k*Math.cos(z),cr=_+k*Math.sin(z);b.push(B`<path class="slice" fill="${me}" d="M ${ue} ${Ct} A ${E} ${E} 0 ${ee} 1 ${jt} ${Rt} L ${lr} ${cr} A ${k} ${k} 0 ${ee} 0 ${ar} ${nr} Z"/>`)}else b.push(B`<path class="slice" fill="${me}" d="M ${I} ${_} L ${ue} ${Ct} A ${E} ${E} 0 ${ee} 1 ${jt} ${Rt} Z"/>`)}),N=B`<g>${b}</g>`,P=y`<div class="legend">${a.map((L,M)=>y`
        <span class="legend-item"><span class="swatch" style="background:${$[M]}"></span>${L.label} (${L.value})</span>
      `)}</div>`}else{let x=Math.max(...a.map(A=>A.value),0),I=this._niceMax(x),_=A=>d.top+v-A/I*v,E=4,k=[];for(let A=0;A<=E;A++){let b=I*A/E,$=_(b);k.push(B`<line class="grid" x1="${d.left}" x2="${d.left+p}" y1="${$}" y2="${$}"/>`),k.push(B`<text class="tick-label" x="${d.left-6}" y="${$+3}" text-anchor="end">${b.toLocaleString()}</text>`)}if(e==="line"||e==="area"){let A=p/Math.max(1,a.length-1),b=a.map((M,q)=>{let D=d.left+q*A,z=_(M.value);return`${q===0?"M":"L"} ${D} ${z}`}).join(" "),$=e==="area"?b+` L ${d.left+p} ${d.top+v} L ${d.left} ${d.top+v} Z`:"",L=n||"var(--forgeui-color-primary)";N=y`
          <g>${k}</g>
          ${e==="area"?B`<path class="area" d="${$}" style="fill:${L};opacity:0.15"/>`:S}
          ${B`<path class="line" d="${b}" style="stroke:${L}"/>`}
          ${a.map((M,q)=>{let D=d.left+q*A,z=_(M.value);return B`<circle class="point" cx="${D}" cy="${z}" r="3" style="fill:${L}"/>
              <text class="tick-label" x="${D}" y="${d.top+v+14}" text-anchor="middle">${M.label}</text>`})}
        `}else{let A=a.length,b=p/A,$=Math.max(2,b*.7),L=b-$;N=y`
          <g>${k}</g>
          ${a.map((M,q)=>{let D=d.left+q*b+L/2,z=_(M.value),ee=Math.max(0,d.top+v-z),ue=M.color||n||"var(--forgeui-color-primary)";return B`<rect class="bar" x="${D}" y="${z}" width="${$}" height="${ee}" rx="2" style="fill:${ue}">
                <title>${M.label}: ${M.value}</title>
              </rect>
              <text class="tick-label" x="${D+$/2}" y="${d.top+v+14}" text-anchor="middle">${M.label}</text>`})}
        `}}return y`
      ${i?y`<div class="title">${i}</div>`:S}
      <div class="wrap">
    ...
    `}};customElements.define("forgeui-chart",Ke);var He=class extends g{static get styles(){return Q`
    :host { display:block; min-width:0; }
  ...
  `}_trendClass(t){let e=String(t??"").toLowerCase();return e==="up"||e==="positive"||e.startsWith("+")?"up":e==="down"||e==="negative"||e.startsWith("-")?"down":"neutral"}render(){let t=this.getString("label",""),e=this.getProp("value"),o=this.getProp("trend"),i=this.getString("trendLabel",""),r=this.getString("subtitle",""),c=this.getString("unit",""),n=typeof e=="number"?e.toLocaleString():e==null||e===""?"\u2014":String(e),a=i||(o==null?"":String(o)),u=this._trendClass(o);return y`<div class="card">
    ...
    </div>`}};customElements.define("forgeui-stat-card",He);var We=class extends g{static get styles(){return Q`
  ...
  `}_trendMeta(t){if(t==null||t==="")return null;if(typeof t=="number")return t>0?{dir:"up",arrow:"\u2191",display:`${Math.abs(t)}%`}:t<0?{dir:"down",arrow:"\u2193",display:`${Math.abs(t)}%`}:{dir:"neutral",arrow:"\u2192",display:"0%"};if(typeof t=="string"){let e=t.toLowerCase(),o=t.match(/^\s*([+-]?\d+(?:\.\d+)?)\s*(%?)\s*$/);if(o){let i=parseFloat(o[1]),r=o[2];return i>0?{dir:"up",arrow:"\u2191",display:`${Math.abs(i)}${r}`}:i<0?{dir:"down",arrow:"\u2193",display:`${Math.abs(i)}${r}`}:{dir:"neutral",arrow:"\u2192",display:`0${r}`}}return e==="up"||e==="positive"||e==="increase"?{dir:"up",arrow:"\u2191",display:""}:e==="down"||e==="negative"||e==="decrease"?{dir:"down",arrow:"\u2193",display:""}:e==="flat"||e==="neutral"||e==="same"?{dir:"neutral",arrow:"\u2192",display:""}:{dir:"neutral",arrow:"",display:t}}return null}render(){let t=this.getString("label",""),e=this.getProp("value"),o=this.getProp("trend"),i=this.getString("trendLabel",""),r=this.getProp("goal"),c=this.getString("unit",""),n=this.getString("suffix",""),a=this.getString("subtitle",""),u=this.getString("variant","");u&&this.setAttribute("variant",u);let l=typeof e=="number"?e.toLocaleString():e==null||e===""?"\u2014":String(e),d=this._trendMeta(o);return y`
    ...
    `}};customElements.define("forgeui-metric",Ge);import{css as Jr,svg as K}from"lit";var Je=class extends g{static get properties(){return{props:{type:Object}}}static get styles(){return Jr`
    :host { display:block; }
    svg { display:block; }
  `}render(){let t=this.getNumber("width",400),e=this.getNumber("height",300),o=this.getString("background","transparent"),i=this.getProp("shapes")||[];return K`
    ...
    `}renderShape(t){let e={fill:t.fill??void 0,stroke:t.stroke??void 0,"stroke-width":t.strokeWidth??void 0,opacity:t.opacity??void 0},o=t.action?()=>{this.onAction&&this.onAction(t.action)}:void 0,i=t.action?"cursor:pointer":void 0;switch(t.type){case"rect":return K`<rect
        ...
        >${t.content??""}</text>`;case"path":return K`<path
          ...
          style="${i}"
          @click=${o}
        />`;default:return K``}}};customElements.define("forgeui-drawing",Je);import{html as U,css as ae,nothing as H}from"lit";var Ye=class extends g{static get styles(){return ae`
  ...
  `}render(){let t=this.getString("message","");return t?U`<div class="toast">${t}</div>`:U`${H}`}};customElements.define("forgeui-toast",Qe);var et=class extends g{static get styles(){return ae`
    :host { display:block; }
  ...
  `}render(){let t=this.getString("msg","Unknown error");return U`<div class="error" role="alert">⚠ ${t}</div>`}};customElements.define("forgeui-error",et);import{html as h,css as C,nothing as w}from"lit";var tt=class extends g{static get styles(){return C`
  ...
  `}render(){let t=this.getString("label",""),e=this.getBoundProp("value",this.getProp("value")),o=e&&typeof e=="object"&&!Array.isArray(e)?e:{},i=String(o.start??this.getProp("start")??""),r=String(o.end??this.getProp("end")??""),c=this.getString("min",""),n=this.getString("max",""),a=this.getBool("disabled"),u=this.getString("action","change"),l=`${this._instanceId}-start`,d=`${this._instanceId}-end`,p=v=>{this.dispatchAction(u,{value:v,start:v.start,end:v.end})};return h`
    ...
    `}};customElements.define("forgeui-date-range-picker",gt);var ft=class extends g{static get styles(){return C`
    :host { display:block; flex:1 1 auto; min-width:0; max-width:100%; margin-bottom:var(--forgeui-space-sm); }
    label { display:block; font-size:var(--forgeui-text-sm); font-weight:var(--forgeui-weight-medium); margin-bottom:var(--forgeui-space-2xs); overflow-wrap:break-word; }
  ...
  `}render(){let t=this.getString("label",""),e=this.getNumber("min",0),o=this.getNumber("max",100),i=this.getNumber("step",1),r=this.getBoundProp("value",e),c=Number(r);Number.isFinite(c)||(c=e);let n=this._instanceId;return h`
    ...
    `}};customElements.define("forgeui-slider",ft);var mt=class extends g{constructor(){super(...arguments);this._dragging=!1;this._openFilePicker=()=>{this.shadowRoot?.querySelector('input[type="file"]')?.click()};this._onDropzoneKeydown=e=>{e.key!=="Enter"&&e.key!==" "||(e.preventDefault(),this._openFilePicker())};this._onFileChange=e=>{let o=Array.from(e.target.files??[]);this._processFiles(o)};this._onDragOver=e=>{e.preventDefault(),!this._dragging&&(this._dragging=!0,this.requestUpdate())};this._onDragLeave=e=>{e.currentTarget===e.target&&(this._dragging=!1,this.requestUpdate())};this._onDrop=e=>{e.preventDefault(),this._dragging=!1,this.requestUpdate(),this._processFiles(Array.from(e.dataTransfer?.files??[]))}}static get styles(){return C`
    :host { display:block; margin-bottom:var(--forgeui-space-sm); }
  ...
  `}_maxSizeBytes(){let e=this.getProp("maxSize");if(typeof e=="number"&&Number.isFinite(e)&&e>=0)return Math.floor(e);if(typeof e!="string")return null;let o=e.trim().match(/^(\d+(?:\.\d+)?)\s*(b|kb|mb|gb)?$/i);if(!o)return null;let i=Number(o[1]),r=(o[2]||"b").toLowerCase(),c=r==="gb"?1<<30:r==="mb"?1<<20:r==="kb"?1024:1,n=i*c;return Number.isFinite(n)?Math.floor(n):null}_newFileId(){return globalThis.crypto?.randomUUID?.()??`${Date.now()}_${Math.random().toString(36).slice(2)}`}_processFiles(e){let o=this.getBool("multiple"),i=this._maxSizeBytes(),r=(o?e:e.slice(0,1)).map(d=>{let p=this._newFileId(),v=i==null||d.size<=i,N={id:p,name:d.name,size:d.size,type:d.type,lastModified:d.lastModified,accepted:v,storageKey:v?p:null};return v||(N.error="maxSize"),[d,N]}),c=r.filter(([,d])=>d.accepted),n=r.map(([,d])=>d),a=c.map(([,d])=>d),u=o?a:a[0]??null,l=a[0]??null;this.dispatchAction("change",{id:l?.id??null,uuid:l?.id??null,name:l?.name??null,size:l?.size??null,type:l?.type??null,lastModified:l?.lastModified??null,storageKey:l?.storageKey??null,value:u,files:n,rejected:n.filter(d=>!d.accepted),multiple:o,maxSize:i}),rr(c.map(([d,p])=>({file:d,id:p.id})))}render(){let e=this.getString("label","Upload file"),o=this.getString("accept","*"),i=this.getBool("multiple");return h`
    ...
    `}};customElements.define("forgeui-file-upload",mt);import{html as R,css as V,nothing as ht}from"lit";var bt=class extends g{static get properties(){return{props:{type:Object}}}static get styles(){return V`
  ...
  `}render(){let t=this.getProp("columns"),e;typeof t=="number"?e=String(t):typeof t=="string"&&t?e=t:e="1";let o=/^\d+$/.test(e)?`repeat(${e}, minmax(0, 1fr))`:e,i=this.getString("gap","md"),r=this.gapValue(i),c=this.getString("padding",""),n=c?this.gapValue(c):"0";return this.style.gridTemplateColumns=o,this.style.gap=r,this.style.padding=n,/^\d+$/.test(e)&&Number(e)>=2&&this.setAttribute("responsive",""),R`<slot></slot>`}};customElements.define("forgeui-grid",yt);var vt=class extends g{static get properties(){return{props:{type:Object}}}static get styles(){return V`
  ...
  `}render(){let t=this.getString("variant",""),e=this.getString("title",""),o=this.getString("subtitle","");return t&&this.setAttribute("variant",t),R`
      ${e||o?R`
        <div class="header">
          ${e?R`<div class="title">${e}</div>`:ht}
          ${o?R`<div class="subtitle">${o}</div>`:ht}
        </div>
      `:ht}
      <div class="body"><slot></slot></div>
    `}};customElements.define("forgeui-card",vt);var xt=class extends g{static get properties(){return{props:{type:Object}}}static get styles(){return V`:host { display:block; margin-inline:auto; width:100%; box-sizing:border-box; }`}render(){let t=this.getString("maxWidth",""),e={sm:"640px",md:"768px",lg:"1024px",xl:"1280px","2xl":"1536px",full:"100%",none:"none","":""},o=t in e?e[t]:t,i=this.getString("padding","");return o&&o!=="none"?this.style.maxWidth=o:this.style.maxWidth="",this.style.padding=i?this.gapValue(i):"",R`<slot></slot>`}};customElements.define("forgeui-container",xt);var wt=class extends g{static get properties(){return{props:{type:Object}}}constructor(){super(),this._active=""}static get styles(){return V`
    :host { display:block; }
  ...
  `}_itemKey(t){return typeof t=="string"?t:String(t&&typeof t=="object"?t.id??t.key??t.value??t.label??"":t??"")}_itemLabel(t){return typeof t=="string"?t:String(t&&typeof t=="object"?t.label??t.title??t.value??"":t??"")}updated(){Array.from(this.children).filter(e=>!(e instanceof HTMLScriptElement)).forEach((e,o)=>{let i=(e.props||{}).slot??e.getAttribute("slot");String(o)===this._active||i===this._active?e.setAttribute("data-active",""):e.removeAttribute("data-active")})}_moveTo(t,e){let o=this._itemKey(e[t])||String(t);this._active=o,this.requestUpdate(),this.dispatchAction("tab-change",{active:o,value:o}),this.updateComplete.then(()=>{this.shadowRoot?.querySelector(`#${this._instanceId}-tab-${t}`)?.focus()})}render(){let t=this.getProp("items")||this.getProp("tabs")||[],e=Array.isArray(t)?t:[],o=this.getBoundProp("activeTab",this.getProp("value"));o!==void 0&&String(o)!==this._active&&(this._active=String(o)),!this._active&&e.length>0&&(this._active=this._itemKey(e[0])||"0");let i=e.findIndex((c,n)=>(this._itemKey(c)||String(n))===this._active),r=(c,n)=>{let a=-1;c.key==="ArrowRight"?a=(n+1)%e.length:c.key==="ArrowLeft"?a=(n-1+e.length)%e.length:c.key==="Home"?a=0:c.key==="End"&&(a=e.length-1),a!==-1&&(c.preventDefault(),this._moveTo(a,e))};return R`
    ...
    `}};customElements.define("forgeui-tabs",wt);var $t=class extends g{static get properties(){return{props:{type:Object}}}static get styles(){return V`
    :host { display:block; }
  ...
  `}render(){let t=this.getString("title","Section");return R`<details><summary>${t}</summary><div class="content"><slot></slot></div></details>`}};customElements.define("forgeui-accordion",$t);var kt=class extends g{static get styles(){return V`
    :host { display:block; }
    hr { border:none; border-top:1px solid var(--forgeui-color-border); margin:var(--forgeui-space-sm) 0; }
  `}render(){return R`<hr>`}};customElements.define("forgeui-divider",kt);var St=class extends g{static get styles(){return V`:host { display:block; }`}render(){let t=this.getString("size","md"),e=this.getString("height",""),o=this.getString("width",""),i=e?this.gapValue(e):this.gapValue(t),r=o?/^\d+(\.\d+)?%$/.test(o)?o:this.gapValue(o):"";return R`<div style="height:${i};${r?`width:${r}`:""}"></div>`}};customElements.define("forgeui-spacer",St);var _t=class extends g{static get properties(){return{props:{type:Object}}}static get styles(){return V`
    :host { display:flex; flex-direction:column; gap:var(--forgeui-space-md); min-width:0; }
    :host([direction="row"]) { flex-direction:row; flex-wrap:wrap; }
    .empty { padding:var(--forgeui-space-lg); text-align:center; color:var(--forgeui-color-text-tertiary); font-size:var(--forgeui-text-sm); }
  `}render(){let t=this.getArray("data"),e=this.getString("emptyMessage",""),o=this.getString("direction","column");(o==="row"||o==="horizontal")&&this.setAttribute("direction","row");let i=this.getString("gap","md");return this.style.gap=this.gapValue(i),t.length===0&&e?R`<div class="empty">${e}</div>`:R`<slot></slot>`}};customElements.define("forgeui-repeater",_t);import{html as F,css as ne,nothing as Yr}from"lit";var At=class extends g{static get styles(){return ne`
  ...
  `}render(){let t=this.getProp("items")||[];return F`${t.map((e,o)=>{let i=o===t.length-1,r=typeof e=="string"?e:e.label,c=typeof e=="string"?"#":e.href;return F`
        ${o>0?F`<span class="sep">/</span>`:Yr}
        ${i?F`<span class="current">${r}</span>`:F`<a href="${c}">${r}</a>`}
      `})}`}};customElements.define("forgeui-breadcrumb",At);var Pt=class extends g{static get styles(){return ne`
  ...
  `}render(){let t=this.getProp("steps")||[],e=this.getBoundProp("active",this.getProp("activeStep")??0),o=Number(e)||0;return F`${t.map((i,r)=>{let c=typeof i=="string"?i:i.label||i.title||`Step ${r+1}`,n=r===o,a=r<o;return F`<div class="step" ?active=${n} ?completed=${a}>
      ...
      </div>`})}`}};customElements.define("forgeui-stepper",Pt);var Et=class extends g{static get styles(){return ne`
    :host { display:block; min-width:0; }
  ...
  `}render(){let t=this.getString("label","Search"),e=this.getString("placeholder","Search"),o=String(this.getBoundProp("value","")??""),i=this.getBool("disabled"),r=this.getString("action","change"),c=this._instanceId;return F`<div class="field">
    ...
    </div>`}};customElements.define("forgeui-search-box",Et);var Tt=class extends g{static get styles(){return ne`
  ...
  `}render(){let t=this.getArray("options"),e=String(this.getBoundProp("value",this.getProp("value")??this._optionValue(t[0]))??""),o=this.getBool("disabled"),i=this.getString("action","change");return F`
    ...
    `}_optionValue(t){if(t&&typeof t=="object"){let e=t;return String(e.value??e.label??"")}return String(t??"")}_optionLabel(t,e){if(t&&typeof t=="object"){let o=t;return String(o.label??o.value??e)}return e}};customElements.define("forgeui-segmented-control",Tt);var It=class extends g{static get styles(){return ne`
