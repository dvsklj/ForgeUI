...
`)}function ie(){return{type:"object",required:["manifest","id","elements"],properties:{manifest:{type:"string",const:"0.1.0"},id:{type:"string",pattern:"^[a-z0-9][a-z0-9-]*$"},root:{type:"string"},title:{type:"string"},schema:{type:"object",properties:{version:{type:"number"},tables:{type:"object",additionalProperties:{type:"object",properties:{columns:{type:"object",additionalProperties:{type:"object",properties:{type:{type:"string",enum:["string","number","boolean"]},default:{}},required:["type"]}}},required:["columns"]}}}},state:{type:"object",additionalProperties:{}},elements:{type:"object",additionalProperties:{type:"object",properties:{type:{type:"string",enum:Qe},props:{type:"object"},children:{type:"array",items:{type:"string"}},visible:{type:"object",properties:{$when:{type:"object",properties:{path:{type:"string"},eq:{},neq:{},gt:{type:"number"},lt:{type:"number"},gte:{type:"number"},lte:{type:"number"},in:{type:"array"},notIn:{type:"array"},exists:{type:"boolean"}},required:["path"]}}}},required:["type"]}},actions:{type:"object",additionalProperties:{type:"object",properties:{type:{type:"string",enum:["mutateState","setState","navigate","submitForm"]},path:{type:"string"},value:{},operation:{type:"string",enum:["append","update","delete"]},data:{type:"object"},key:{type:"string"},formId:{type:"string"},action:{type:"string"}},required:["type"]}},dataAccess:{type:"object",properties:{enabled:{type:"boolean",default:!1},readable:{type:"array",items:{type:"string"}},restricted:{type:"array",items:{type:"string"}},summaries:{type:"boolean",default:!1}}},persistState:{type:"boolean"},skipPersistState:{type:"boolean"}}}}var Dt={container:"Stack",row:"Stack",column:"Stack",text:"Text",heading:"Text",label:"Text",image:"Image",icon:"Icon",button:"Button",link:"Link",textInput:"TextInput",numberInput:"NumberInput",checkbox:"Checkbox",toggle:"Toggle",select:"Select",divider:"Divider",spacer:"Spacer",card:"Card",table:"Table",list:"List",alert:"Alert",progress:"Progress",badge:"Badge",avatar:"Avatar"};function Vt(s){let e={},t=0;function o(a){let n=a.id||`a2ui-${t++}`,u=Dt[a.type];if(!u||!V(u))return e[n]={type:"Text",props:{content:`[Unsupported A2UI type: ${a.type}]`,variant:"caption",colorScheme:"secondary"}},n;let c=Ft(a.type,a.props||{}),p=(a.children||[]).map(f=>o(f));return e[n]={type:u,props:c,children:p.length>0?p:void 0},n}let r=(s.components||s.content||[]).map(a=>o(a)),l=r.length===1?r[0]:"a2ui-root";return r.length>1&&(e[l]={type:"Stack",props:{gap:"md"},children:r}),{manifest:"0.1.0",id:`a2ui-${Date.now()}`,root:l,elements:e,actions:{}}}function Ft(s,e){let t={...e};switch(s){case"heading":t.variant="heading",t.content=e.text||e.content||"",delete t.text;break;case"text":case"label":t.content=e.text||e.content||"",delete t.text;break;case"row":t.direction="row",t.gap=e.gap||e.spacing||"md";break;case"column":t.direction="column",t.gap=e.gap||e.spacing||"md";break;case"container":t.gap=e.gap||e.spacing||"md";break;case"image":t.src=e.src||e.url||"",t.alt=e.alt||"";break;case"button":t.label=e.text||e.label||"",t.variant=e.variant||e.style||"default";break;case"textInput":t.placeholder=e.placeholder||"",t.value=e.value||"",t.label=e.label||"";break;case"numberInput":t.placeholder=e.placeholder||"",t.value=e.value??0,t.label=e.label||"";break;case"select":e.options&&Array.isArray(e.options)&&(t.options=e.options.map(o=>typeof o=="string"?{value:o,label:o}:o));break;case"alert":t.variant=e.type||e.severity||"info",t.content=e.message||e.text||e.content||"",delete t.message,delete t.text;break;case"progress":t.value=e.value??e.percent??0;break}return t}function ft(s){if(!s||typeof s!="object")return!1;let e=s;return!!(Array.isArray(e.components)||Array.isArray(e.content)||typeof e.type=="string"&&(e.type==="adaptive-card"||e.type.startsWith("a2ui"))||typeof e.version=="string"&&e.version.startsWith("a2ui")||!e.elements&&!e.manifest&&Array.isArray(e.content))}function se(s){return ft(s)?Vt(s):s}function qt(s){return`forgeui_${(s||"global").replace(/[^a-zA-Z0-9-]/g,"_")}`}function gt(s,e,t="none"){let o=qt(e),i=t,r=null,l=!1,a=!1,n=null,u=null,c=null,p=[];function f(){let y=k();for(let v of p)try{v(y)}catch{}}function k(){return{mode:i,isPersisting:r!=null&&l,lastSaved:n,lastLoaded:u,error:c,dbName:i==="indexeddb"?o:null}}async function R(){if(r)return r;try{let{createIndexedDbPersister:y}=await import("tinybase/persisters/persister-indexed-db");return r=y(s,o),c=null,r}catch(y){throw c=`IndexedDB unavailable: ${y.message}`,f(),y}}async function N(){if(i==="none")return;let y=await R();try{await y.load(),u=Date.now()}catch(v){c=`Load failed: ${v.message}`,f()}try{y.startAutoSave(),l=!0}catch(v){c=`Auto-save failed: ${v.message}`}try{y.startAutoLoad(1),a=!0}catch(v){c=`Auto-load failed: ${v.message}`}f()}async function x(){if(r){l&&(r.stopAutoSave(),l=!1),a&&(r.stopAutoLoad(),a=!1);try{await r.save(),n=Date.now()}catch(y){c=`Final save failed: ${y.message}`}f()}}async function I(){if(!(i==="none"||!r)){try{await r.save(),n=Date.now(),c=null}catch(y){c=`Save failed: ${y.message}`}f()}}async function j(){if(!(i==="none"||!r)){try{await r.load(),u=Date.now(),c=null}catch(y){c=`Load failed: ${y.message}`}f()}}async function A(){if(await x(),r){try{r.destroy()}catch{}r=null}p=[],f()}function _(y){if(y===i)return;let v=i;i=y,v==="indexeddb"&&y==="none"&&(r&&l&&(r.stopAutoSave(),l=!1),r&&a&&(r.stopAutoLoad(),a=!1)),y==="indexeddb"&&v==="none"&&N().catch(()=>{}),f()}function w(y){return p.push(y),()=>{p=p.filter(v=>v!==y)}}return{start:N,stop:x,save:I,load:j,setMode:_,getStatus:k,addListener:w,destroy:A}}var W=class{constructor(e=50){this._stack=[];this._position=-1;this._listeners=[];this._maxDepth=e}push(e){if(this._position<this._stack.length-1&&(this._stack=this._stack.slice(0,this._position+1)),this._position>=0){let t=this._stack[this._position];if(JSON.stringify(t)===JSON.stringify(e))return}this._stack.push(structuredClone(e)),this._stack.length>this._maxDepth?this._stack.shift():this._position++,this._notifyListeners()}undo(){return this._position<=0?null:(this._position--,this._notifyListeners(),structuredClone(this._stack[this._position]))}redo(){return this._position>=this._stack.length-1?null:(this._position++,this._notifyListeners(),structuredClone(this._stack[this._position]))}current(){return this._position<0?null:structuredClone(this._stack[this._position])}jumpTo(e){return e<0||e>=this._stack.length?null:(this._position=e,this._notifyListeners(),structuredClone(this._stack[this._position]))}getState(){return{canUndo:this._position>0,canRedo:this._position<this._stack.length-1,position:this._position,total:this._stack.length}}getTimeline(){return this._stack.map((e,t)=>({position:t,id:e.id}))}clear(){this._stack=[],this._position=-1,this._notifyListeners()}addListener(e){return this._listeners.push(e),()=>{this._listeners=this._listeners.filter(t=>t!==e)}}_notifyListeners(){let e=this.getState();for(let t of this._listeners)try{t(e)}catch{}}};import{html as d,css as b,svg as $,nothing as h}from"lit";import{LitElement as Bt}from"lit";var H=class H extends Bt{constructor(){super(...arguments);this._instanceId=`forge-${++H._instanceCounter}`;this.props={};this.store=null;this.onAction=null;this.itemContext=null}static get properties(){return{props:{type:Object}}}connectedCallback(){super.connectedCallback()}resolve(t){if(!this.store)return t;this.itemContext&&L(this.itemContext);try{return M(this.store,t)}finally{L(null)}}getProp(t){let o=this.props?.[t];return typeof o=="string"&&(o.startsWith("$state:")||o.startsWith("$computed:")||o.startsWith("$item:")||o.startsWith("$expr:")||o.includes("{{")&&o.includes("}}"))?this.resolve(o):o}getArray(t){let o=this.getProp(t);return Array.isArray(o)?o:o&&typeof o=="object"?Object.values(o):[]}getString(t,o=""){let i=this.getProp(t);return typeof i=="string"?i:String(i??o)}getNumber(t,o=0){let i=this.getProp(t);return typeof i=="number"?i:Number(i)||o}getBool(t,o=!1){let i=this.getProp(t);return typeof i=="boolean"?i:o}dispatchAction(t,o){this.onAction&&this.onAction(t,o),this.dispatchEvent(new CustomEvent("forgeui-action",{detail:{action:t,payload:o},bubbles:!0,composed:!0}))}handleAction(t){let o=this.getString("action");o&&this.dispatchAction(o,this.props)}prop(t){return this.getProp(t)}static get sharedStyles(){return[re]}gapValue(t){let o={none:"0",0:"0","3xs":"var(--forgeui-space-3xs)","2xs":"var(--forgeui-space-2xs)",xs:"var(--forgeui-space-xs)",sm:"var(--forgeui-space-sm)",md:"var(--forgeui-space-md)",lg:"var(--forgeui-space-lg)",xl:"var(--forgeui-space-xl)","2xl":"var(--forgeui-space-2xl)"};if(t==null||t==="")return"var(--forgeui-space-md)";let i=String(t);return i in o?o[i]:/^\d+(\.\d+)?$/.test(i)?`${i}px`:/^\d+(\.\d+)?(px|rem|em|%|vw|vh|ch)$/.test(i)?i:"var(--forgeui-space-md)"}static get styles(){return[re]}};H._instanceCounter=0;var m=H;var ae=class extends m{static get properties(){return{props:{type:Object}}}static get styles(){return b`
  ...
  `}render(){let e=this.getString("label",""),t=this.getString("placeholder",""),o=this.getString("hint",""),i=this.getString("error",""),r=this.getString("inputType","")||this.getString("type","text"),l=this.getBool("multiline"),a=this.getString("value",""),n=this._instanceId;return d`
  ...
  `}render(){let e=this.getString("label",""),t=this.getBool("checked"),o=this._instanceId;return d`
    ...
    `}};customElements.define("forgeui-checkbox",Ae);var _e=class extends m{constructor(){super(...arguments);this._toggle=()=>{if(this.getBool("disabled"))return;let t=this.getBool("on");this.dispatchEvent(new CustomEvent("forgeui-action",{detail:{actionId:"change",value:!t},bubbles:!0,composed:!0}))};this._onKeydown=t=>{(t.key==="Enter"||t.key===" "||t.key==="Spacebar")&&(t.preventDefault(),this._toggle())}}static get styles(){return b`
  ...
  `}render(){let e=this.getString("label",""),t=this.getNumber("min",0),o=this.getNumber("max",100),i=this.getNumber("step",1),r=this.getNumber("value",t);return d`
    ...
    `}};customElements.define("forgeui-slider",Ce);var Ee=class extends m{static get styles(){return b`
    :host { display:block; margin-bottom:var(--forgeui-space-sm); }
    label { display:block; font-size:var(--forgeui-text-sm); font-weight:var(--forgeui-weight-medium); margin-bottom:var(--forgeui-space-2xs); }
  ...
  `}render(){let e=this.getString("label","Upload file"),t=this.getString("accept","*");return d`
    ...
    `}};customElements.define("forgeui-file-upload",Ee);var je=class extends m{static get styles(){return b`
    :host { display:inline-flex; }
  ...
  `}render(){let e=this.getString("label","Button"),t=this.getString("variant","primary"),o=this.getString("size",""),i=this.getBool("disabled"),r=this.getProp("pressed");return d`<button class="${t} ${o}" ?disabled=${i}
      ...
      @click=${l=>this.handleAction(l)}>${e}<slot></slot></button>`}};customElements.define("forgeui-button",je);var Ie=class extends m{static get styles(){return b`
    :host { display:flex; gap:var(--forgeui-space-xs); }
  `}render(){return d`<slot></slot>`}};customElements.define("forgeui-button-group",Ie);var Me=class extends m{static get styles(){return b`
    :host { display:inline-flex; }
  ...
  `}render(){let e=this.getString("label",""),t=this.getString("href","#");return d`<a href="${t}">${e}<slot></slot></a>`}};customElements.define("forgeui-link",Me);var Te=class extends m{static get styles(){return b`
  ...
  `}render(){let e=this.getProp("data"),t=this.getString("emptyMessage","No items");return!Array.isArray(e)||e.length===0?d`<div class="empty">${t}</div>`:d`<div class="list">${e.map((o,i)=>d`
  ...
  `}_niceMax(t){if(t<=0)return 1;let o=Math.pow(10,Math.floor(Math.log10(t))),i=t/o;return(i<=1?1:i<=2?2:i<=5?5:10)*o}render(){let t=this.getString("chartType","bar"),o=this.getProp("data")||[],i=this.getString("title",""),r=this.getString("xKey","label")||this.getString("labelKey","label"),l=this.getString("yKey","value")||this.getString("valueKey","value"),a=this.getString("color","");if(!o||o.length===0)return d`
      ...
      `;let n=o.map(x=>typeof x=="number"?{label:"",value:x}:x&&typeof x=="object"?{label:String(x[r]??x.label??x.name??x.x??""),value:Number(x[l]??x.value??x.y??0)||0,color:x.color}:{label:String(x),value:0}),u=600,c=260,p={top:8,right:16,bottom:36,left:48},f=u-p.left-p.right,k=c-p.top-p.bottom,R,N=h;if(t==="pie"||t==="donut"){let x=n.reduce((P,S)=>P+Math.max(0,S.value),0)||1,I=u/2,j=c/2,A=Math.min(f,k)/2-8,_=t==="donut"?A*.55:0,w=-Math.PI/2,y=[],v=[];n.forEach((P,S)=>{let T=Math.max(0,P.value)/x,C=w,E=w+T*Math.PI*2;w=E;let O=E-C>Math.PI?1:0,q=I+A*Math.cos(C),He=j+A*Math.sin(C),Ke=I+A*Math.cos(E),Ge=j+A*Math.sin(E),K=P.color||this._palette[S%this._palette.length];if(v.push(K),_>0){let mt=I+_*Math.cos(C),ht=j+_*Math.sin(C),bt=I+_*Math.cos(E),yt=j+_*Math.sin(E);y.push($`<path class="slice" fill="${K}" d="M ${q} ${He} A ${A} ${A} 0 ${O} 1 ${Ke} ${Ge} L ${bt} ${yt} A ${_} ${_} 0 ${O} 0 ${mt} ${ht} Z"/>`)}else y.push($`<path class="slice" fill="${K}" d="M ${I} ${j} L ${q} ${He} A ${A} ${A} 0 ${O} 1 ${Ke} ${Ge} Z"/>`)}),R=$`<g>${y}</g>`,N=d`<div class="legend">${n.map((P,S)=>d`
        <span class="legend-item"><span class="swatch" style="background:${v[S]}"></span>${P.label} (${P.value})</span>
      `)}</div>`}else{let x=Math.max(...n.map(w=>w.value),0),I=this._niceMax(x),j=w=>p.top+k-w/I*k,A=4,_=[];for(let w=0;w<=A;w++){let y=I*w/A,v=j(y);_.push($`<line class="grid" x1="${p.left}" x2="${p.left+f}" y1="${v}" y2="${v}"/>`),_.push($`<text class="tick-label" x="${p.left-6}" y="${v+3}" text-anchor="end">${y.toLocaleString()}</text>`)}if(t==="line"||t==="area"){let w=f/Math.max(1,n.length-1),y=n.map((S,T)=>{let C=p.left+T*w,E=j(S.value);return`${T===0?"M":"L"} ${C} ${E}`}).join(" "),v=t==="area"?y+` L ${p.left+f} ${p.top+k} L ${p.left} ${p.top+k} Z`:"",P=a||"var(--forgeui-color-primary)";R=d`
          <g>${_}</g>
          ${t==="area"?$`<path class="area" d="${v}" style="fill:${P};opacity:0.15"/>`:h}
          ${$`<path class="line" d="${y}" style="stroke:${P}"/>`}
          ${n.map((S,T)=>{let C=p.left+T*w,E=j(S.value);return $`<circle class="point" cx="${C}" cy="${E}" r="3" style="fill:${P}"/>
              <text class="tick-label" x="${C}" y="${p.top+k+14}" text-anchor="middle">${S.label}</text>`})}
        `}else{let w=n.length,y=f/w,v=Math.max(2,y*.7),P=y-v;R=d`
          <g>${_}</g>
          ${n.map((S,T)=>{let C=p.left+T*y+P/2,E=j(S.value),O=Math.max(0,p.top+k-E),q=S.color||a||"var(--forgeui-color-primary)";return $`<rect class="bar" x="${C}" y="${E}" width="${v}" height="${O}" rx="2" style="fill:${q}">
                <title>${S.label}: ${S.value}</title>
              </rect>
              <text class="tick-label" x="${C+v/2}" y="${p.top+k+14}" text-anchor="middle">${S.label}</text>`})}
        `}}return d`
      ${i?d`<div class="title">${i}</div>`:h}
      <div class="wrap">
  ...
  `}render(){let e=this.getProp("items")||[];return d`${e.map((t,o)=>{let i=o===e.length-1,r=typeof t=="string"?t:t.label,l=typeof t=="string"?"#":t.href;return d`
      ...
      `})}`}};customElements.define("forgeui-breadcrumb",Ve);var Fe=class extends m{static get styles(){return b`
  ...
  `}render(){let e=this.getString("msg","Unknown error");return d`<div class="error" role="alert">⚠ ${e}</div>`}};customElements.define("forgeui-error",qe);var Be=class extends m{static get properties(){return{props:{type:Object}}}static get styles(){return b`
    :host { display:block; }
    svg { display:block; }
