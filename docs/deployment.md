...
| `FORGEUI_API_TOKEN` | *(unset)* | Bearer token for `/api/apps/*` write operations (POST, PUT, PATCH, DELETE). In production, writes are rejected when this is unset. |
...
| `NODE_ENV` | *(unset)* | When set to `production`, requires `FORGEUI_API_TOKEN` for writes. |
