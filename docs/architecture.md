...
Ajv compiler. The 56.5 KB gzip budget is enforced in CI via
...
the full ~28 KB ESM bundle (or ~56.5 KB IIFE on CDN). This is what makes growing the catalog cheap for us
and for them.
...
gzip with a 56.5 KB ceiling enforced in CI. The aspirational ~40 KB target
