...
The manifest catalog today is **51 components**. The runtime registers 52
...
- **Core (19):** the components the LLM is told about by default in
  ...
  base runtime.
- **Extended (26):** components lazy-loaded or opted-in via
...
| Core | Action | Button |
| Core | Data | Table, Chart, ChartLegend, Metric |
| Core | Feedback | Alert, Dialog, Progress, Error |
| Extended | Layout | Container, PageHeader, Accordion, ButtonGroup, Divider, Spacer |
...
| Extended | Navigation | Breadcrumb, Stepper, SearchBox, SegmentedControl, Pagination |
...
|-----------------------|--------|---------|-------------------------------|
| IIFE (CDN)            | 224 KB | 59.1 KB | `<script>`-tag, zero build    |
| ESM standalone        | 181 KB | 41.7 KB | Modern bundler, whole runtime |
| ESM per-component     | 107 KB | 23.2 KB | Tree-shaking consumers        |
...
Ajv compiler. The 62.0 KB gzip budget is enforced in CI via
...
the full ~41.7 KB ESM bundle (or ~59.1 KB IIFE on CDN). This is what makes growing the catalog cheap for us
and for them.
...
standalone validator function, saving ~34 KB gzip. The IIFE is now ~59.1 KB
gzip with a 62.0 KB ceiling enforced in CI. The aspirational ~40 KB target
...
Core runtime, Ring 2 server, MCP connector, 51 manifest components plus the
...
- Component a11y deep audit (WCAG 2.2 AA on all 51)
